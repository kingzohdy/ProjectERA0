...
int CBCGPEditCtrl::OnCreate(LPCREATESTRUCT lpCreateStruct) 
{
	...
	{
		m_wndToolTip.SetMaxTipWidth (m_nMaxToolTipWidth);
	}
		...
		if (m_bEnableHyperlinkSupport && GetCapture () != this)
		{
			...
			if (bCtrl && m_nCurrHyperlinkHot != -1)
			{
			#ifdef _BCGPEDIT_STANDALONE
				{
					...
					::SetCursor (AfxGetApp ()->LoadCursor (IDC_BCGBARRES_HAND));
				}
			#else
...
int CBCGPEditCtrl::HitTest (CPoint& pt, BOOL bNormalize, BOOL bIgnoreTextBounds)
{
	...
	for (int i = nStrartFrom; i <= nRow; i++)
	{
		...
		if (i == nRow)
		{
			...
			int nXRight = m_rectText.left + 1 + GetStringExtent (&dc, strRow, strRow.GetLength ()).cx;
...
BOOL CBCGPEditCtrl::OffsetToPoint (int nOffset, CPoint& pt, LPPOINT ptRowColumn, LPINT pnScrollOffset)
{
	...
	pt.x = max (pt.x, m_rectText.left + 1);
...
BOOL CBCGPEditCtrl::InsertTab (BOOL bRedraw)
{
	...
	if (m_bKeepTabs)
	{
		...
		UpdateOffsetRelatedData (m_nCurrOffset, m_nCurrOffset + strTab.GetLength ());
		OnUpdateAutoOutlining (m_nCurrOffset, strTab.GetLength (), FALSE);
		OnAfterTextChanged (m_nCurrOffset, strTab, TRUE);
		UpdateScrollBars ();
	...
	{
		CString strToInsert (_T(' '), m_nIndentSize);
		if (!OnBeforeTextInserted (strToInsert, m_nCurrOffset))
		{
			return FALSE;
		}
		int nSaveOffset = m_nCurrOffset;
		for (int i = 0; i < m_nIndentSize; i++)
		{
			strTab += _T(' ');
			m_strBuffer.Insert (m_nCurrOffset, _T(' '));
			UpdateScrollBars ();
			Right ();
		}

		UpdateOffsetRelatedData (nSaveOffset, nSaveOffset + m_nIndentSize);
		OnUpdateAutoOutlining (nSaveOffset, m_nIndentSize, FALSE);
		OnAfterTextChanged (m_nCurrOffset, strToInsert, TRUE);
	}
...
void CBCGPEditCtrl::OnFillBackground (CDC* pDC)
{
	...
	if (m_clrBack == -1)
	{
		::FillRect (pDC->GetSafeHdc (), &m_rectText, ::GetSysColorBrush (COLOR_WINDOW));
	}
	else
	{
		CBrush br (m_clrBack);
		pDC->FillRect (m_rectText, &br);
	}
}
//***************************************************************************************
...
void CBCGPEditCtrl::OnDrawLineNumbersBar (CDC* pDC, CRect rect)
{
	...
	COLORREF clrLineNumber = RGB (0, 130, 132); // magenta

	LOGBRUSH lb;
	lb.lbStyle = BS_SOLID;
	lb.lbColor = clrLineNumber;
	lb.lbHatch = 0;

	CPen pen(PS_ALTERNATE, 1, &lb);
...
void CBCGPEditCtrl::OnDrawLineNumber (CDC* pDC, CRect rect, const int nLineNumber)
{
	...
	{
		COLORREF clrLineNumber = RGB (0, 130, 132); // magenta
		COLORREF clrOld = pDC->SetTextColor (clrLineNumber);
		int nOldMode = pDC->SetBkMode (TRANSPARENT);
...
//////////////////////////////////////////////////////////////////////////
	...
	int nEndOffset = HitTest (ptBottomRight, FALSE, TRUE);
	...
	m_OutlineNodes.GetBlocksByStateInRange (nStartOffset, nEndOffset, lstCollapsedOutlines, TRUE, FALSE);
	...
	for (int i = nStartOffset; i <= nEndOffset; i++)
	{
		...
		if (chNext != g_chEOL && !bDrawSymbol && pHiddenText == NULL)
		{
			if (nNextDelimiter == -1) 
			{
				...
				if (!bColorFound && 
					!OnGetWordColor (strNextWord, clrNextWordFore, clrNextWordBk, i))
				{
		...
		if (pHiddenText != NULL)
		{
			...
			OnDrawOutlineSymbol (pDC, pHiddenText, clrOutline, clrBackground);
							   ...
							   COLORREF /*clrBack*/)
{
	...
	if (m_bKeepTabs)  
	{
		return pDC->TabbedTextOut (rect.left, rect.top, str, 1, &m_nTabLogicalSize, nOrigin).cx;
	}
	
	return pDC->DrawText (str, rect, DT_NOCLIP | DT_SINGLELINE | DT_NOPREFIX);
	
}
//**************************************************************************************
/*
CSize CBCGPEditCtrl::GetStringExtent (CDC* pDC, LPCTSTR str)
{
	CSize size;
	if (m_bKeepTabs)  
	{
		size = pDC->GetOutputTabbedTextExtent (str, 1, &m_nTabLogicalSize);	
	}
	else
	{
...
CSize CBCGPEditCtrl::GetStringExtent (CDC* pDC, LPCTSTR lpszString, int nCount)
{
	if (m_bEnableSymSupport)
	{
		...
		if (m_bKeepTabs)  
		{
			sizeResult = pDC->GetOutputTabbedTextExtent (pszString, j, 1, &m_nTabLogicalSize);	
		}
		else
		{
	...
	if (m_bKeepTabs)  
	{
		return pDC->GetOutputTabbedTextExtent (lpszString, nCount, 1, &m_nTabLogicalSize);	
	}
									 ...
									 BOOL bRedraw)
{
	...
	CString strBuffer = m_strBuffer.Mid (nStartOffset, nEndOffset - nStartOffset);
	...
	int nBufferLen = strBuffer.GetLength ();
	...
	while (nNextStartOffset < nBufferLen)
	{
													 ...
													 m_lstColorBlocks.GetNext (pos))
		{
			...
			int nIdx = strBuffer.Find (colorBlock.m_strOpen, nNextStartOffset);
			if (nIdx == -1)
			{
				continue;
			}
		...
		if (!colorBlock.m_strClose.IsEmpty ())
		{
			do 
			{
				colorArea.m_nEnd = strBuffer.Find (colorBlock.m_strClose, nIdxMin + 1);
				if (colorArea.m_nEnd == -1)
				{
					break;
				}
				
				if (m_strContinueStringSymbols.Find (strBuffer.GetAt (colorArea.m_nEnd - 1)) >= 0)
				{
					nIdxMin = nIdxMin + 1;
					colorArea.m_nEnd = strBuffer.Find (colorBlock.m_strClose, nIdxMin + 1);
				}
		...
		nNextStartOffset = colorArea.m_nEnd + max (1, colorBlock.m_strClose.GetLength ());

		colorArea.m_nStart += nStartOffset;
		if (colorBlock.m_strClose.GetLength () > 0)
		{
			colorArea.m_nEnd += nStartOffset + colorBlock.m_strClose.GetLength () - 1;
		}
		else
		{
...
BOOL CBCGPEditCtrl::Paste (int iPos /*= -1*/)
{
	...
	BOOL bForceNextUndo = m_iStartSel != 0;
													...
													int nLen)
{
	try
	{
		HGLOBAL hClipbuffer = ExportBuffer (lpszText, nLen, 
											FALSE, m_bReplaceTabsAndEOLOnCopy);
		if (hClipbuffer != NULL)
		{
			::GlobalUnlock (hClipbuffer);
									 ...
									 BOOL bReplaceEOL)
{
	...
	{
		int nSpaceCount = 0;
		for (int nTextIdx = 0, nExpIdx = 0; nTextIdx < nTextLen; nTextIdx++, nExpIdx++)
		{
...
void CBCGPEditCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
	...
	if (IsPointInSelectionRect (point) && (nFlags & MK_LBUTTON))
	{
		...
		if (OpenClipboard ())
		{
			...
			_tcscpy (lpszBufferText, strText);
			...
			if (hClipbufferText != NULL)
			{
				...
				srcItem.CacheGlobalData (_TCF_TEXT, hClipbufferText);
				
				DROPEFFECT dropEffect = srcItem.DoDragDrop (DROPEFFECT_COPY | DROPEFFECT_MOVE);
	...
	if (m_bEnableOutlining && 
		ptSave.x >= m_nLeftMarginWidth + (m_bEnableLineNumbersMargin ? m_nLineNumbersMarginWidth : 0) && 
...
//*************************************************************************************
void CBCGPEditCtrl::OnTimer(UINT nIDEvent) 
{
	switch (nIDEvent)
	{
	case ID_TEXT_SCROLL:
...
void CBCGPEditCtrl::UpdateScrollBars ()
{
	...
	m_szTotalScroll = CSize (m_nMaxScrollWidth, m_nTotalLines - m_nHiddenLines);
	...
	siHorz.nPage = m_rectText.Width () / m_nMaxCharWidth;
	...
	siVert.nPage = m_rectText.Height () / m_nLineHeight;
...
BOOL CBCGPEditCtrl::OnScroll (int fnBar, UINT nSBCode, int nPos)
{
	...
	GetScrollInfo (fnBar, &scrollInfo);
	...
	if (fnBar == SB_VERT)
	{
		...
		for (int iIdxRow = 0; iIdxRow < abs (nActualScroll); )
		{
			if (bDirection)
			{
				...
				{
					nNextOffset++;
					if (FindCollapsedBlock (nNextOffset) == NULL)
					{
						iIdxRow++;
						continue;
					}
	...
	SetScrollPos (fnBar, nScrollOffset);	
								   ...
								   BOOL bForseInsertMode)
{
	...
	if (!m_bUndoCharMode && dwAction == g_dwUATInsertData && !GetOverrideMode ())
	{
			 ...
			 m_bEOL && m_posUndoBuffer == NULL)
		{
			...
			if ((pUndoAction->m_dwActionType & g_dwUATInsertData) &&
...
BOOL CBCGPEditCtrl::OnUndo (BOOL bSetCaret, BOOL bRedraw)
{
	...
	if (m_posUndoBuffer == NULL && m_bBOL)
	{
		m_bIsModified = FALSE;
	}
										   ...
										   BOOL bSetCaret)
{
		...
		(pUndoAction->m_dwActionType & g_dwUATDeleteData) && !bUndo)
	{
		...
		{
			CString strChar = pUndoAction->m_strActionText [0];
...
{
	int nRangeStart = max (0, nOffset - _MAX_COLOR_BLOCK_STR_LEN);
	int nRangeEnd = min (m_strBuffer.GetLength (), nOffset + _MAX_COLOR_BLOCK_STR_LEN);
...
{
	if (strBlockOpen.GetLength () > _MAX_COLOR_BLOCK_STR_LEN ||
		strBlockClose.GetLength () > _MAX_COLOR_BLOCK_STR_LEN)
	{
		ASSERT (FALSE);
	...
	colorBlock.m_clrForeground = clrFrgnd;
...
//************************************************************************************
void CBCGPEditCtrl::ReverseFindOneOf(const CString& strCharSet, const ciStartPos, int& nCurrOffset) const
{
	do
	{
		nCurrOffset--;
...
int CBCGPEditCtrl::GetOffsetOfColumnInRow (int nColumn, int nRowEndOffset, BOOL bEndOffsetSpecified, BOOL bSkipHidden)
{
	...
	{
		int nCurrColumn = 0;
		for (int iOffset = nStartRowOffset; iOffset < nRowEndOffset; iOffset++)
		{
			CBCGPOutlineBaseNode* pHiddenText = NULL;
	...
	{
		int nCurrColumn = 0;
		for (int iOffset = nStartRowOffset; iOffset < nRowEndOffset;)
		{
...
//************************************************************************************
void CBCGPEditCtrl::BuildTabString (CString& str)
{
	if (m_bKeepTabs)
	{
...
//***************************************************************************************
COLORREF GetColorFromBuffer (const CString strBuffer)
{
...
BOOL CBCGPEditCtrl::LoadXMLSettingsFromBuffer (const CString& strInBuffer)
{
	...
	if (ExcludeTag (strBuffer, _T("COLOR_DATA"), strColorData))
	{
		...
		if (ExcludeTag (strColorData, _T("WORDS"), strWords))
		{
			while (TRUE)
			{
				...
				if (ExcludeTag (strWordData, _T("CaseSensitive"), strCaseSensitive))
				{
					strCaseSensitive.MakeUpper ();
					bCaseSensitive = (strCaseSensitive == _T("TRUE"));
				}
		...
		if (ExcludeTag (strColorData, _T("BLOCKS"), strBlocks))
		{
			while (TRUE)
			{
				...
				CString strEnd;
				ExcludeTag (strBlockData, _T("End"), strEnd);
				...
				if (ExcludeTag (strBlockData, _T("WholeText"), strIsWholeText))
				{
	...
	if (ExcludeTag (strBuffer, _T("OUTLINE_DATA"), strOutlineData))
	{
	...
	if (ExcludeTag (strBuffer, _T("HYPERLINKS"), strHyperlinkSettings))
	{
...
BOOL CBCGPEditCtrl::OpenFile (CFile& file)
{
	try
	{
		...
		if (!IsTextUnicode((void*)pszFileBuffer, dwFileSize, NULL))
		{
			...
			TCHAR* pszTextBuffer = new TCHAR [dwFileSize + 1];
			memset (pszTextBuffer, 0, (dwFileSize + 1) * sizeof (TCHAR));

			const int nTextLen = MultiByteToWideChar(::GetACP (), 0, 
				pszFileBuffer, dwFileSize, pszTextBuffer, dwFileSize * sizeof (TCHAR));
	...
	try
	{
		...
		if (dwTextLen > 0)
		{
			...
			if (m_bSaveFileInUnicode) // Unicode encoding
			{
				...
				// Write to file:
				#if _MSC_VER >= 1300
				file.Write(lpszBuffer, _tcslen (lpszBuffer) * sizeof (TCHAR));
				#else
			...
			else // ANSI encoding
			{
				...
				dwTextLen = _tcslen (lpszBuffer);

				// Convert buffer to Ansi:
				char* pszTextBuffer = new char [dwTextLen + 1];
				memset (pszTextBuffer, 0, (dwTextLen+1) * sizeof(char));

				if (WideCharToMultiByte(::GetACP (), 0, lpszBuffer, dwTextLen, 
					pszTextBuffer, dwTextLen, NULL, NULL) == 0)
				{
					TRACE0 ("SaveFile failed - system convertion API failed\n");
					delete pszTextBuffer;
					GlobalFree (hGlobal);
				...
				pszTextBuffer [dwTextLen] = '\0';
				memcpy (lpszBuffer, pszTextBuffer, dwTextLen+1);

				delete pszTextBuffer;

				// Write to file:
				#if _MSC_VER >= 1300
				file.Write(lpszBuffer, strlen ((char*)lpszBuffer) * sizeof (char));
				#else
				file.WriteHuge(lpszBuffer, strlen ((char*)lpszBuffer) * sizeof (char));
				#endif
								...
								BOOL bNext, BOOL bCase, BOOL bWholeWord)
{
	...
	SetLastUndoReason (g_dwUATPaste);
	int nSaveOffset = m_nCurrOffset;
    InsertText (lpszReplace, -1, TRUE, FALSE, TRUE, FALSE);

	SetSel (nSaveOffset, m_nCurrOffset);
...
void CBCGPEditCtrl::InitColors ()
{
	...
	m_clrHyperlink = m_clrBackSelActive;
}
//**************************************************************************************
...
{
	return (m_clrText == -1) ? ::GetSysColor (COLOR_WINDOWTEXT) : m_clrText;
}
//**************************************************************************************
COLORREF CBCGPEditCtrl::GetDefaultBackColor () const
{
	return (m_clrBack == -1) ? ::GetSysColor (COLOR_WINDOW) : m_clrBack;
}
//**************************************************************************************
...
void CBCGPEditCtrl::OnPrint(CDC* pDC, CPrintInfo* pInfo)
{
	...
	if (GetRowStart (m_nScrollOffsetVert + nRowsInPage, TRUE) < 0)
	{
...
CBCGPOutlineNode* CBCGPOutlineNode::FindCollapsedBlock (int nOffset) const
{
	...
	for (POSITION pos = m_lstNodes.GetHeadPosition (); pos != NULL;)
	{
		...
		{
			CBCGPOutlineNode* pInnerOutlineNode = pOutlineNode->FindCollapsedBlock (nOffset);
...
//**************************************************************************************
BOOL CBCGPEditCtrl::GetHiddenTextFromPoint (CPoint pt, CString& strWord)
{
	int nOffset = HitTest (pt);
	if (nOffset == -1 || nOffset >= m_strBuffer.GetLength ())
	{
...
void CBCGPEditCtrl::OnOutlineChanges (BCGP_EDIT_OUTLINE_CHANGES& changes, BOOL bRedraw)
{
	...
	// Notify all changes:
	// -------------------
	for (POSITION pos = changes.m_lstInserted.GetHeadPosition (); pos != NULL; )
	{
...
BOOL CBCGPEditCtrl::LoadOutlineParserXMLSettings (CString& strInBuffer)
{
	if (m_pOutlineParser != NULL)
	{
		...
		//-----------------------
		if (ExcludeTag (strInBuffer, _T("EscapeSequences"), strBlocks))
		{
			while (TRUE)
			{
				CString strEscapeSequence;
				if (!ExcludeTag (strBlocks, _T("EscapeSequence"), strEscapeSequence, TRUE))
				{
					break;
				}
		...
		// Read blocks:
		//-------------
		if (ExcludeTag (strInBuffer, _T("BLOCKS"), strBlocks))
		{
			while (TRUE)
			{
				CString strBlock;
				if (!ExcludeTag (strBlocks, _T("BLOCK"), strBlock))
				{
					break;
				}

				CString strOpen;
				if (!ExcludeTag (strBlock, _T("Start"), strOpen, TRUE))
				{
					ASSERT (FALSE);
					break;
				}
				...
				CString strClose;
				if (!ExcludeTag (strBlock, _T("End"), strClose, TRUE))
				{
					ASSERT (FALSE);
					break;
				}
				...
				CString	strReplace;
				if (!ExcludeTag (strBlock, _T("ReplaceString"), strReplace, TRUE))
				{
					strReplace = _T("...");
				}
				...
				CString strNested;
				if (ExcludeTag (strBlock, _T("AllowNestedBlocks"), strNested))
				{
					strNested.MakeUpper ();
				...
				BOOL bIgnore = FALSE;
				CString strIgnore;
				if (ExcludeTag (strBlock, _T("Ignore"), strIgnore))
				{
					strIgnore.MakeUpper ();
				...
				CString strKeywords;
				if (ExcludeTag (strBlock, _T("KEYWORDS"), strKeywords))
				{
					while (TRUE)
					{
						CString strKeyword;
		...
		BOOL bCaseSensitive = TRUE;
		CString strCaseSensitive;
		if (ExcludeTag (strInBuffer, _T("CaseSensitive"), strCaseSensitive))
		{
			strCaseSensitive.MakeUpper ();
			bCaseSensitive = (strCaseSensitive == _T("TRUE"));
		}
		...
		if (ExcludeTag (strInBuffer, _T("WholeWords"), strWholeWords))
		{
...
void CBCGPEditCtrl::UpdateHiddenLines (BCGP_EDIT_OUTLINE_CHANGES& changes)
{
	...
	for (POSITION pos = changes.m_lstRemoved.GetHeadPosition (); pos != NULL; )
	{
		CBCGPOutlineNode* pNode = (CBCGPOutlineNode*) changes.m_lstRemoved.GetNext (pos);
		ASSERT_VALID (pNode);
		...
		{
			CBCGPOutlineBaseNode* pNode = (CBCGPOutlineBaseNode*) changes.m_lstRemoved.GetNext (pos);
			ASSERT_VALID (pNode);
...
void CBCGPEditCtrl::SetHyperlinkColor (BOOL bUseColor, COLORREF clr)
{
	...
	m_clrHyperlink = (clr != -1) ? clr : m_clrBackSelActive;
}
//************************************************************************************
...
BOOL CBCGPEditCtrl::LoadHyperlinkXMLSettings (CString& strInBuffer)
{
	...
	m_bEnableHyperlinkSupport = FALSE;
	CString strEnable;
	if (ExcludeTag (strInBuffer, _T("EnableHyperlinkSupport"), strEnable))
	{
		strEnable.MakeUpper ();
	...
	if (ExcludeTag (strInBuffer, _T("HighlightWithColor"), strColorHyperlink))
	{
	...
	m_clrHyperlink = m_clrBackSelActive;
	CString strColor;
	if (ExcludeTag (strInBuffer, _T("Color"), strColor))
	{
	...
	if (ExcludeTag (strInBuffer, _T("URLPrefixes"), strURLPrefixes))
	{
		...
		while (TRUE)
		{
			...
			if (!ExcludeTag (strURLPrefixes, _T("URLPrefix"), strURLPrefix))
			{
				break;
			}
