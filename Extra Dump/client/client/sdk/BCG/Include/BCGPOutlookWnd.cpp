...
CBCGPPopupMenu* COutlookCustomizeButton::CreatePopupMenu ()
{
...
void COutlookCustomizeButton::OnDraw (CDC* pDC, const CRect& rect, CBCGPToolBarImages* /*pImages*/,
...
BOOL CBCGPOutlookBarToolBar::OnSendCommand (const CBCGPToolbarButton* pButton)
{
	...
	if (nIndex >= 0)
	{
		...
		{
			m_pParentBar->SetActiveTab (iTab);
			return TRUE;
		}
	}
...
void CBCGPOutlookBarToolBar::AdjustLocations ()
{
	...
	int x = rectToolbar.right - (sizeButton.cx - 2) * nCount - sizeCustomizeButton.cx + 2;
	...
	for (POSITION pos = m_Buttons.GetHeadPosition (); pos != NULL;)
	{
		...
		if (x < rectToolbar.left)
		{
			m_pCustomizeBtn->AddInvisibleButton (pButton);
...
void CBCGPOutlookWnd::RecalcLayout ()
{
	...
	if (bIsMode2003)
	{
		...
		m_rectCaption.bottom = m_rectCaption.top + globalData.GetTextHeight () + 2 * CBCGPBaseTabWnd::TAB_TEXT_MARGIN;
	...
	if (nVisibleTabsNum > 1 || !IsHideSingleTab ())
	{
		for (int i = 0; i < m_iTabsNum; i ++)
		{
			...
			if (i == m_iActiveTab && !bIsMode2003)
			{
				...
				y = rectClient.bottom - m_nTabsHeight * (nVisibleTabsNum - i - 1) + 1;
...
void CBCGPOutlookWnd::OnPaint() 
{
	...
	if (!m_rectCaption.IsRectEmpty ())
	{
		...
		dc.FillRect (rectTop, &globalData.brBarFace);
		
		COLORREF clrText = globalData.clrBarText;
		CBCGPVisualManager::GetInstance ()->OnFillOutlookBarCaption (&dc,
			m_rectCaption, clrText);

		CString strActivePage;
		GetTabLabel (m_iActiveTab, strActivePage);

		CRect rcText = m_rectCaption;
		rcText.DeflateRect (5, 5);
	...
	{
		// Draw splitter:
		CBCGPVisualManager::GetInstance ()->OnDrawOutlookBarSplitter (&dc,
			m_rectSplitter);
	}
...
class CBCGPOutlookOptionsDlg : public CDialog
{
// Construction
public:
...
CBCGPOutlookOptionsDlg::CBCGPOutlookOptionsDlg(CBCGPOutlookWnd& parentBar)
	: CDialog(CBCGPOutlookOptionsDlg::IDD, &parentBar),
...
BEGIN_MESSAGE_MAP(CBCGPOutlookOptionsDlg, CDialog)
...
void CBCGPOutlookOptionsDlg::OnReset() 
{
	...
	for (int i = 0; i < pOutlookBar->GetDefaultTabsOrder ().GetSize (); i++)
	{
