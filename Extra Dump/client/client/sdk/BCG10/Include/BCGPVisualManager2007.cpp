										...
										BOOL bActive, BOOL bMDI/* = FALSE*/)
{
	...
	CBCGPToolBarImages::ImageAlignHorz horz = globalData.GetRibbonImageScale () != 1. ? CBCGPToolBarImages::ImageAlignHorzStretch : CBCGPToolBarImages::ImageAlignHorzCenter;
	CBCGPToolBarImages::ImageAlignVert vert = globalData.GetRibbonImageScale () != 1. ? CBCGPToolBarImages::ImageAlignVertStretch : CBCGPToolBarImages::ImageAlignVertCenter;
...
void CBCGPVisualManager2007::OnDrawCaptionBarInfoArea (CDC* pDC, CBCGPCaptionBar* pBar, CRect rect)
{
	...
	CBCGPDrawManager dm (*pDC);
	dm.FillGradient (rect, globalData.clrBarFace, RGB (255, 255, 255));
										 ...
										 BOOL bPressed)
{
	if (!CanDrawImage ())
	{
		return CBCGPVisualManager2003::DrawCheckBox (pDC, rect, nState, bHighlighted, bPressed, bEnabled);
	}
											  ...
											  CRect rectCaption, CRect rectText)
{
...

    {
		...
		if (bExtra)
		{
			...
			if (pCaptionQA->IsValid ())
			{
				...
				rectQAFrame.right   = pBar->GetQATCommandsLocation ().right + GetRibbonQATRightMargin () + 1;
	...
	DrawNcText (pDC, rectText, strTitle, strDocument, bPrefix, bActive, 
		bIsRTL, m_bNcTextCenter, bGlass, pWnd->IsZoomed () ? 0 : 10, 
