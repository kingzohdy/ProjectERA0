...
CBCGPRibbonStatusBarPane::CBCGPRibbonStatusBarPane (UINT nCmdID, 
	LPCTSTR lpszText, BOOL bIsStatic, HICON hIcon, LPCTSTR lpszAlmostLargeText) :
	CBCGPRibbonButton (nCmdID, lpszText, hIcon)
{
	CommonInit ();
...
CBCGPRibbonStatusBarPane::CBCGPRibbonStatusBarPane (UINT nCmdID, 
	...
	COLORREF clrTransp, HICON hIcon, BOOL bIsStatic) :
	CBCGPRibbonButton (nCmdID, lpszText, hIcon)
{
	CommonInit ();
...
//*****************************************************************************
int CBCGPRibbonStatusBarPane::DrawText (CDC* pDC, const CString& strText, CRect rectText, UINT /*uiDTFlags*/)
{
	ASSERT_VALID (this);
	ASSERT_VALID (pDC);
	...
	return CBCGPRibbonButton::DrawText (pDC, strText, rectText, uiDTFlags);
}
//******************************************************************************
												 ...
												 COLORREF clrTransp)
{
	...
	SetAnimationList (hbmp, cxAnimation, clrTransp);
	return TRUE;
}
//*******************************************************************************
CSize CBCGPRibbonStatusBarPane::GetIntermediateSize (CDC* pDC)
{
	...
	if (m_AnimImages.GetCount () > 0)
	{
		...
		return CSize (
			imageSize.cx + 2 * m_szMargin.cx,
			imageSize.cy + 2 * m_szMargin.cy);
	}
...
void CBCGPRibbonStatusBarPane::OnDraw (CDC* pDC)
{
	...
	else
	{
		if (globalData.GetRibbonImageScale () != 1.)
		{
			...
			CSize sizeDest = CSize (
					(int) (.5 + globalData.GetRibbonImageScale () * sizeImage.cx), 
					(int) (.5 + globalData.GetRibbonImageScale () * sizeImage.cy));
			...
			ptImage.Offset (
				max (0, (m_rect.Width () - sizeDest.cx) / 2), 
				max (0, (m_rect.Height () - sizeDest.cy) / 2));
