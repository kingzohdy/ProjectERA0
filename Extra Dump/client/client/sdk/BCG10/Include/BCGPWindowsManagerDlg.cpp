...
void CBCGPWindowsManagerDlg::OnDrawItem(int nIDCtl, LPDRAWITEMSTRUCT lpDIS) 
{
	...
	CBrush&		brFill = (lpDIS->itemState & ODS_SELECTED) ? globalData.brHilite : globalData.brWindow;
	COLORREF	clText = (lpDIS->itemState & ODS_SELECTED) ? globalData.clrTextHilite : globalData.clrWindowText;
	...
	if (hIcon != NULL)
	{
		...
		::DrawIconEx (pDC->GetSafeHdc (), rectIcon.left, rectIcon.top, 
			hIcon, rectIcon.Height (), rectIcon.Height (),
			0, NULL, DI_NORMAL);
	}
