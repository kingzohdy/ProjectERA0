...
BOOL CBCGPPlannerViewDay::EnsureVisible(const CBCGPAppointment* pApp, BOOL bPartialOK)
{
	...
	{
		if (bPartialOK || 
			(rect.top >= m_ViewRects[0].top && 
			 rect.bottom <= m_ViewRects[0].bottom))
		{
			return TRUE;
		}
	}
	...
	int nPos = 0;
	if (rect.top < m_ViewRects[0].top)
	{
		//scroll up
		nPos = (rect.top - m_ViewRects[0].top) / m_nRowHeight;
	}
	else
	{
		//scroll down
		...
		{
			nPos = (rect.bottom - m_ViewRects[0].top) / m_nRowHeight - m_nScrollPage;
		}
	}
