...
void CBCGPEditCtrl::Initialize ()
{
	...
	ModifyStyle (WS_BORDER, WS_HSCROLL | WS_VSCROLL);
...
void CBCGPEditCtrl::RecalcLayout ()
{
	...
	if (m_bEnableLineNumbersMargin)
	{
		m_rectText.left += m_nLineNumbersMarginAutoWidth;
	}
	if (m_bEnableOutlineMargin)
	{
...
void CBCGPEditCtrl::OnDrawSideBar (CDC* pDC, CRect rect)
{
	...
	pDC->FillSolidRect (&rectBar, m_clrBackSidebar);
...
void CBCGPEditCtrl::OnDrawLineNumber (CDC* pDC, CRect rect, const int nLineNumber)
{
	...
	{
		COLORREF clrOld = pDC->SetTextColor (m_clrTextLineNumber);
...
BOOL CBCGPEditCtrl::IsIqual (const CString& strBuffer, int nOffset, BOOL bDirForward, const CString& str) const
{
	...
	if (bDirForward)
	{
		...
		if (nLeftOffset + str.GetLength () - 1 < strBuffer.GetLength ())
		{
			...
			ASSERT (nLeftOffset + i < strBuffer.GetLength ()); // to check
									 ...
									 BOOL bRedraw)
{
	...
	while (nNextStartOffset < nBufferLen)
	{
													 ...
													 m_lstColorBlocks.GetNext (pos))
		{
			...
			{
				// on demand prepare buffer for case sensitive search
			...
			int nIdx = (colorBlock.m_bCaseSensitive) ?
				strBufferUpper.Find (colorBlock.m_strOpen, nNextStartOffset) :
				strBuffer.Find (colorBlock.m_strOpen, nNextStartOffset);
			if (nIdx == -1)
			{
				continue;
			}
		...
		if (!colorBlock.m_strClose.IsEmpty ())
		{
			...
			do 
			{
				colorArea.m_nEnd = (colorBlock.m_bCaseSensitive) ?
					strBufferUpper.Find (colorBlock.m_strClose, nIdxMin + 1) :
					strBuffer.Find (colorBlock.m_strClose, nIdxMin + 1);
				...
				{
					if (IsEscapeSequence (strBuffer, colorArea.m_nEnd - 1, FALSE))
					{
						break;
					}
...
BOOL CBCGPEditCtrl::CopyTextToClipboard (LPCTSTR lpszText, int nLen)
{
	...
	try
	{
		...
		if (bCopyRTFToClipboard)
		{
			...
			if (uFormat != 0)
			{
				if (!CopyTextToClipboardInternal (strRTF, (CLIPFORMAT) uFormat, strRTF.GetLength ()))
				{
					::CloseClipboard ();
					return FALSE;
				}
...
void CBCGPEditCtrl::UpdateScrollBars ()
{
	...
	siVert.nMax = m_szTotalScroll.cy + siVert.nPage;
...
void CBCGPEditCtrl::OnVScroll(UINT nSBCode, UINT nPos, CScrollBar* /*pScrollBar*/) 
{
	...
	if (bScroll)
	{
		...
		ScrollWindow (0, (nScrollOffsetVertOld - m_nScrollOffsetVert) * m_nLineHeight);
...
BOOL CBCGPEditCtrl::LoadXMLSettingsFromBuffer (const CString& strInBuffer)
{
	...
	if (globalData.ExcludeTag (strBuffer, _T("OPTIONS"), strOptions))
	{
		...
		if (globalData.ExcludeTag (strBuffer, _T("EscapeSequences"), strEscapeSequencesList))
		{
			while (TRUE)
			{
	...
	if (globalData.ExcludeTag (strBuffer, _T("COLOR_DATA"), strColorData))
	{
		...
		if (globalData.ExcludeTag (strColorData, _T("BLOCKS"), strBlocks))
		{
			while (TRUE)
			{
				...
				CString strStart;
				if (!globalData.ExcludeTag (strBlockData, _T("Start"), strStart))
				{
					ASSERT (FALSE);
					break;
				}
				...
				CString strEnd;
				globalData.ExcludeTag (strBlockData, _T("End"), strEnd);
				...
				SetBlockColor (strStart, strEnd, bIsWholeText, color, colorBack, bCaseSensitive);
			}
		}
	}
										 ...
										 BOOL bInsideOpenBlockAtStart, BOOL bInsideOpenBlockAtEnd, BOOL bEndOfBlock)
{
	...
	CPen pen (PS_SOLID, 1, m_clrLineOutline);

	CPen* pPenOld = pDC->SelectObject (&pen);

	CBrush br (m_clrBackOutline);
...
int CBCGPEditCtrl::GetLine (int nLine, LPTSTR lpszBuffer) const
{
	...
	if (nEndOffset > nStartOffset && nEndOffset <= m_strBuffer.GetLength ())
	{
		...
		lstrcpyn (lpszBuffer, (LPCTSTR) strLine, min (nLineLen, nMaxLength));

		return min (nLineLen, nMaxLength);
	}
...
int CBCGPEditCtrl::GetLine (int nLine, LPTSTR lpszBuffer, int nMaxLength) const
{
	...
	*((WORD*) lpszBuffer) = (WORD) nMaxLength;
	...
	return GetLine (nLine, lpszBuffer);
}
//************************************************************************************
...
void CBCGPEditCtrl::OnNcPaint() 
{
	...
	//------------------------
	if ((GetStyle () & WS_THICKFRAME) == 0 &&
		GetScrollBarCtrl (SB_HORZ) != NULL &&
		GetScrollBarCtrl (SB_VERT) != NULL)
	{
		CRect rectBox = rect;
	
		rectBox.left = rectBox.right - ::GetSystemMetrics (SM_CXVSCROLL) - 1;
		rectBox.top = rectBox.bottom - ::GetSystemMetrics (SM_CYHSCROLL) - 1;

		COLORREF clrBkOld = dc.GetBkColor ();
		dc.FillSolidRect (rectBox, ::GetSysColor(COLOR_BTNFACE));
