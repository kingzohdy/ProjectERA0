...
void CBCGPPopupWindow::OnTimer(UINT_PTR nIDEvent) 
{
	...
	case iAnimTimerId:
		if (!m_bAnimationIsDone)
		{
			...
			if ((m_AnimationType != CBCGPPopupMenu::FADE && m_AnimSize.cy >= m_FinalSize.cy && m_AnimSize.cx >= m_FinalSize.cx) ||
				(m_AnimationType == CBCGPPopupMenu::UNFOLD && m_AnimSize.cx >= m_FinalSize.cx) ||
