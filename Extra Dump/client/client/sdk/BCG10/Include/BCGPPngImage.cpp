...
static void png_read_data(png_structp png_ptr, png_bytep data, png_size_t length)
{
    ...
    {
        check = pFile->Read(n_data, length);
    }
    else
    {
...
void png_write_flush(png_structp png_ptr, png_bytep /*data*/, png_size_t /*length*/)
{
	CFile* pFile = (CFile*) png_ptr->io_ptr;
	ASSERT_VALID (pFile);
	pFile->Flush ();
...
#pragma warning (disable : 4244)
    ...
    WORD wDIRowBytes = (WORD) (cImgChannels == 4) ? (cImgChannels * cxImgSize) : 
		(WORD) (((3 * cxImgSize + 3L) >> 2) << 2);
    ...
    for (LONG yImg = 0; yImg < height; yImg++)
    {
		...
		else
		{
			for (LONG xImg = 0; xImg < bih.biWidth; xImg++)
			{
				...
				double alpha = 255.0 / (double)quad->rgbReserved;

				*dst++ = (BYTE) min (quad->rgbBlue * alpha, 255.0); /* note the reverse order */
				*dst++ = (BYTE) min (quad->rgbGreen * alpha, 255.0);
				*dst++ = (BYTE) min (quad->rgbRed * alpha, 255.0);
...
static BOOL PngLoadImage (CFile* pFile, png_byte **ppbImageData,
...
BOOL PngSaveImage (CFile* pFile, png_byte **ppbImageData,
                   int iWidth, int iHeight, int iChannels, COLORREF BkgColor)
{
	try
	{
        ...
        png_set_write_fn(png_ptr, (png_voidp)pFile, png_write_data, png_write_flush);
...
BOOL CBCGPPngImage::LoadFromFile (CFile* pFile)
{
	...
	try
	{
		...
		if (!PngLoadImage (pFile, &pbImageData, &cxImgSize, &cyImgSize, &nChannels, &bkgColor))
		{
			if (pbImageData != NULL)
			{
				delete [] pbImageData;
...
BOOL CBCGPPngImage::SaveToFile (LPCTSTR lpszPath)
{
	...
	return SUCCEEDED (m_image.Save (lpszPath));
#endif
#endif // BCGP_EXCLUDE_PNG_SUPPORT
}
//*******************************************************************************
...
BOOL CBCGPPngImage::SaveToFile (CFile* pFile)
{
	...
	try
	{
		...
		if (PngSaveImage (pFile, pbImageData, bih.biWidth, abs (bih.biHeight), 
