...
//****************************************************************************
void CBCGPRibbonPanel::CommonInit (LPCTSTR lpszName, HICON hIcon, BOOL bAutoDestroyIcon)
{
...
int CBCGPRibbonPanel::GetHeight (CDC* pDC) const
{
	...
	int nMaxHeight = max (nRowHeight * 3, ((CBCGPRibbonPanel*)this)->m_btnDefault.GetRegularSize (pDC).cy);
		...
		if (m_pHighlighted != NULL)
		{
				...
				m_pScrollBar->GetSafeHwnd () != NULL)
			{
				...
				if (pIcon != NULL)
				{
					...
					if (nChar == VK_DOWN)
					{
						if (rectIcon.bottom >= rectPalette.bottom &&
					...
					{
						if (rectIcon.top <= rectPalette.top &&
