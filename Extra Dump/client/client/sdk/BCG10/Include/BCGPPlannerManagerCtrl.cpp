...
void CBCGPPlannerManagerCtrl::SetImages (LPCTSTR szResID, int cxImage, COLORREF clrTransparent)
{
	...
	HBITMAP hBmp = (HBITMAP)::LoadImage (AfxFindResourceHandle (RT_BITMAP, szResID), 
...
BOOL CBCGPPlannerManagerCtrl::RemoveAppointment (CBCGPAppointment* pApp, BOOL bQuery, BOOL bRedraw)
{
	...
	if (pApp->IsRecurrenceClone () && lRes == 1)
	{
		...
		pApp = pAppsStorage->GetRecurrence (ID);
...
void CBCGPPlannerManagerCtrl::OnLButtonDown(UINT nFlags, CPoint point) 
{
		...
		hit == CBCGPPlannerView::BCGP_PLANNER_HITTEST_WEEK_CAPTION)
	{
		if (m_bNotifyParent && GetSafeHwnd () != NULL)
		{
			...
			if (GetParent ()->SendMessage (message, wparam, 0) != 0L)
			{
				return;
			}
		}
		...
		while (m_lsSelectedApps.GetTailPosition () != NULL)
		{
			...
			if (bNotify)
			{
				GetParent ()->SendMessage 
					(BCGP_PLANNER_SELECT_APPOINTMENT, (WPARAM) FALSE, (LPARAM) pApp);
			}
		}
												 ...
												 BOOL bSelect, BOOL bRedraw)
{
	...
	{
		bOperation = GetParent ()->SendMessage 
			(BCGP_PLANNER_BEFORE_SELECT_APPOINTMENT, (WPARAM) bSelect, (LPARAM) pApp) == 0L;
	}
	...
	if (bOperation)
	{
		...
		{
			GetParent ()->SendMessage 
				(BCGP_PLANNER_SELECT_APPOINTMENT, (WPARAM) bSelect, (LPARAM) pApp);
		}
	}
	...
	try
	{
		...
		if (bRes)
		{
			...
			if (m_bNotifyParent)
			{
				...
				{
					GetParent ()->SendMessage (BCGP_PLANNER_RESOURCEID_CHANGED, 0, 0);
				}
			}
		}
	}
...
void CBCGPPlannerManagerCtrl::UpdateCalendarsSelection (CBCGPCalendar* pWndCalendar /*= NULL*/)
{
	...
	else if (m_Type == BCGP_PLANNER_TYPE_MONTH && IsCompressWeekend () && GetFirstDayOfWeek () == 0)
	{
		dt -= COleDateTimeSpan (1, 0, 0, 0);
	}
...
BOOL CBCGPPlannerManagerCtrl::SetCurrentResourceID (UINT nResourceID, BOOL bRedraw/* = TRUE*/)
{
	...
	bRes = pStorage->SetCurrentResourceID (nResourceID);
