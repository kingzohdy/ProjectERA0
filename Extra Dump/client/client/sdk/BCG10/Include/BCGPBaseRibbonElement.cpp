...
int CBCGPBaseRibbonElement::GetDropDownImageWidth () const
{
	...
	int cxImage = CBCGPMenuImages::Size ().cx;

	if (globalData.GetRibbonImageScale () > 1.)
	{
		cxImage = (int) (.5 + globalData.GetRibbonImageScale () * cxImage);
	}
...
//******************************************************************************
int CBCGPBaseRibbonElement::DrawText (CDC* pDC, const CString& strText, CRect rectText, UINT uiDTFlags,
								 COLORREF clrText)
{
	ASSERT_VALID (this);
	ASSERT_VALID (pDC);
	...
	if (CBCGPToolBarImages::m_bIsDrawOnGlass)
	{
		...
		CBCGPVisualManager::GetInstance ()->DrawTextOnGlass (pDC, strText, rectText, uiDTFlags, 0, 
			bIsZoomed ? RGB (255, 255, 255) : clrText);
