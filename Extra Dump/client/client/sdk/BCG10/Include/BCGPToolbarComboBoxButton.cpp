	...
	if (m_bFlat)
	{
		...
		if (!m_strEdit.IsEmpty ())
		{
			...
			if (m_pWndEdit == NULL)
			{
				...
				{
					COLORREF cltTextOld = pDC->SetTextColor (globalData.clrWindowText);
					DrawButtonText (pDC, m_strEdit, &rectText, DT_VCENTER | DT_SINGLELINE, globalData.clrWindowText, (int)CBCGPVisualManager::ButtonsIsRegular);
...
void CBCGPComboEdit::OnPaint() 
{
	...
	{
		dc.FillRect (rect, &globalData.brWindow);
