...
**  $Id: tlock.h,v 1.5 2007/12/29 09:45:45 steve Exp $
**  @author $Author: steve $
...
*********************************************************************/
int tlockopen(IN const char* a_pszPath, IN int a_iFlags, IN int a_iType);
...
*	@retval -1  if failed, use errno to determine the reason.
*********************************************************************/
int tlockclose(IN int a_iLock, IN int a_iFlags);
...
*	@param a_iLock Input, the descriptor of the lock.
*	@param a_iIsLock Input, TRUE for locking operation, FALSE for unlocking operation.
*	@return whether the operation is succeeded.
*	@retval 0  if sucess.
*	@retval -1  if failed, use errno to determine the reason.
*********************************************************************/
int tlockop(IN int a_iLock, IN int a_iIsLocking);
