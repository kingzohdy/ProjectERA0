...
#ifndef __TECH_STDMTL_H__
#define __TECH_STDMTL_H__

#include "OgreD3D9Technique.h"

namespace Ogre
{
	class D3DTech_stdmtl_lod0 : public D3DTech
	{
	public:
		enum
		{
	...
	public:
		D3DTech *clone()
		{
			return new D3DTech_stdmtl_lod0;
		}
		...
		void init(const ShaderEnvFlags &env, const MaterialMacro &macros)
		{
			...
			for(int i=0; i<MaterialMacro::MAX_MACRO; i++)
			{
				...
				else if(indx == MTLPARAM_USE_MASK) m_UseMask = macros.value[i];
			}
		...
		virtual void beginPass( size_t ipass )
		{
			...
			else
			{
				...
				{
					if(m_UseSelfIllumTex==SELFILLUM_ALPHA || m_UseSelfIllumTex==SELFILLUM_ALL) SetBlendState(m_pDevice, BLEND_ALPHATEST, 0);
					else SetBlendState(m_pDevice, m_BlendMode, 85);
				}
	...
	class D3DTech_stdmtl_shadowgen : public D3DTech
	{
	public:
		enum
		{
	...
	public:
		D3DTech *clone()
		{
			return new D3DTech_stdmtl_shadowgen;
		}
