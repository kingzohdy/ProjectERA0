#ifndef _ARK_HASH_JENKINS_H
#define _ARK_HASH_JENKINS_H
/*
...
/*
   ...
   in unsigned ints instead of bytes.  It is much faster than hash().  It 
   requires
   ...
   --------------------------------------------------------------------
   */
inline unsigned int hash2( 
		register unsigned int *k,        /* the key */
		register unsigned int  length,   /* the length of the key, in unsigned ints */
		register unsigned int  initval  /* the previous hash, or an arbitrary value */
		)
{
	register unsigned int a,b,c,len;
	...
	/*-------------------------------------- handle the last 2 unsigned int's */
	c += length;
	switch(len)              /* all the case statements fall through */
	{
 ...
 * from http://www.concentric.net/~Ttwang/tech/inthash.htm
 */
inline unsigned int hash2(unsigned int key)
{
	key += ~(key << 15);
	key ^=  (key >> 10);
	key +=  (key << 3);
