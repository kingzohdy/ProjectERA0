...
namespace TE
{
	...
	GrassGroupManager    *g_PlantMgr;

	extern Ogre::Vector4 g_WorldAmbientLight;


	GrassGroup::GrassGroup(void)
	{
		m_nCurVecNum = 0;
		m_nCurIndexNum = 0;
		...
		for( int i = 0 ; i < 4 ; i++)
		{
			m_MovePos[i] = Ogre::Vector3(0 ,0 , 0);
		}
	...
	void   GrassGroup::ClearLightColor()
	{
		if ( m_nCurVecNum == 0 )
			return;
	...
	void   GrassGroup::CountLightColor(int lighttype, Ogre::Vector3 pos, Ogre::ColourValue color, Ogre::ColourValue ambient, float range, float shadowdensity)
	{
		unsigned char r,g,b ;
		...
		for( int i = 0 ; i < m_nCurVecNum / 4  ; i ++ )
		{
			...
			if(lighttype == 0)
			{
				Ogre::Vector3 pos0 = m_SavePosSet[i];
	...
	void   GrassGroup::UpdateLightColor()
	{
	...
	void GrassGroup::UpdateHeight(EditorTerrainBlock* pChunk)
	{
		for(int i=0;i<m_nCurVecNum/4;i++)
		{
			float newheight;
	...
	void   GrassGroup::UpdateDataEx_1()
	{
		if( m_nCurVecNum <= 0)
		{
			m_nCurVecNum = 0;
	...
	void   GrassGroup::UpdateDataEx(int mode)
	{
		if( m_nCurVecNum <= 0)
			return;
	...
	void   GrassGroup::Reset()
	{
		m_nCurVecNum = 0 ;
	}

	void   GrassGroup::UpdateData()
	{
		if( m_nCurVecNum <= 0)
			return;
	...
	bool   GrassGroup::InitEx_1()
	{
		...
		{	
			vdata[i*4+0].texcoord = Ogre::Vector2(1,1);
			vdata[i*4+1].texcoord = Ogre::Vector2(0,1);
			vdata[i*4+2].texcoord = Ogre::Vector2(1,0);
			vdata[i*4+3].texcoord = Ogre::Vector2(0,0);
		}
	...
	bool   GrassGroup::SetTexture(Ogre::Texture *  tex)
	{
	...
	bool   GrassGroup::SetTexture(const char* filename )
	{
	...
	void GrassGroup::render(Ogre::SceneRenderer *prenderer, const Ogre::ShaderEnvData &envdata)
	{
		if(m_nCurIndexNum == 0)
		{
			return;
	...
	void   GrassGroup::RenderEx(Ogre::SceneRenderer *prenderer, const Ogre::ShaderEnvData &envdata)
	{
		if( m_nCurIndexNum == 0)
			return;
	...
	void   GrassGroup::UpdateEx_1(float tick)
	{
	...
	void  GrassGroup::Update(float  tick)
	{	
	...
	void   GrassGroup::DelPos( Ogre::Vector3 pos , float fSize )
	{
	...
	bool   GrassGroup::AddPosEx_1(Ogre::Vector3 pos , float fScale , unsigned int color )
	{
		if( m_nCurVecNum / 4 >= ( PLANT_MAX_NUM - 12)  || m_nCurVecNum < 0 )
			return false;

		Ogre::Vector3  posSet[4];
	...
	void   GrassGroup::SetPos(Ogre::Vector3 pos)
	{
		Ogre::Vector3  posSet[8];
	...
	//植物管理，根据信息生成大量植被
	GrassGroupManager::GrassGroupManager()
	{
		memset( m_PlantMap,0,1024*1024 );
		m_strPlantInfoFile[0] = 0 ;
	}

	void    GrassGroupManager::Init()
	{
	...
	//根据灰度图文件生成大量植被
	void    GrassGroupManager::CreateChunkPlant()
	{
		...
		{		
			...
			{
				LOG_WARNING( "加载植物灰度图不是24位，根据灰度图创建植被失败！");
				delete[] pSizeInfo;
				return;
			}
		}
		...
		for( int i = 0 ; i < TERRTILE_NUMBLOCK_Z ; i ++ )
		{
			for( int j = 0 ; j < TERRTILE_NUMBLOCK_X ; j ++)
			{
				...
				EditorTerrainBlock *pChunk = &g_TerrainEdit.GetEditData() ->chunkSet [i][j];
					...
					for( int l = 0 ; l < 64 ; l++)
					{
						...
						for( unsigned int p = 0 ; p < m_PlantInfoSet.size () ; p++ )
						{
							...
							{	
								...
								Ogre::Vector3  pos(0,0,0) , dir(0,0,0) ;
								...
								if( m_PlantInfoSet[p].nAutoNormal == 1 )
								{
									...
									dir.x = atan ( fry ) / Ogre::ONE_PI *180;
									dir.y = -atan( frx ) / Ogre::ONE_PI*180;
	...
	void    GrassGroupManager::UpdateDataPlant()
	{
		return;
	};

	void    GrassGroupManager::Render()
	{
	}

	void    GrassGroupManager::Release()
	{
