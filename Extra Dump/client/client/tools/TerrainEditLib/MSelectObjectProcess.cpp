...
MVec3  ChangeScenePointToRay( int x , int y )
{
	MMatrix mat = g_pCamera->getProjectMatrix();
...
bool   TestNormal(MVec3 &nor1 , MVec3 &nor2,MVec3 &nor3 , MVec3 &nor4,MVec3 testnor)
{
...
MSelectObjectProcess::MSelectObjectProcess(void)
{	
...
bool MSelectObjectProcess::Process( MTerrainEdit* pEditor , ActionEvent *event )
{
	...
	case MouseLDown:
		{	
			...
			pEditor->m_nSaveMouseX = event->mx  ;
			pEditor->m_nSaveMouseY = event->my  ;
			...
			else
			{
				...
				{
					pEditor->m_pCurSelectModel->GetModelData ()->pDrawbox ->SetColor (  0xffaaaaaa  ); 
				}
				for( size_t i = 0 ; i < pEditor->m_CurSelectModelSet.size () ; i++)
				{
					pEditor->m_CurSelectModelSet[i]->GetModelData ()->pDrawbox ->SetColor ( 0xffaaaaaa   );  
				}
...
bool   MSelectObjectProcess::AddSelectScreenObject( MTerrainEdit* pEditor ,MVec3 &dir,MVec3 &pos, int mode )
{	
	...
	MTerrainCull *pcull = pEditor->m_Cull;
	...
	{	
		...
		if(pModel->GetShowModel()->intersectRay(INTERSECT_PICK, ray, NULL))
		{
			...
			pEditor->m_pCurSelectModel->GetModelData ()->pDrawbox ->SetColor ( 0xff00ff00 );  
...
bool  MSelectObjectProcess::SelectRangeObject( MTerrainEdit* pEditor ,int dx, int dy , int mode)
{
		...
		|| pos[0].y == pos[2].y )
	{
	 ...
	 for( size_t i = 0 ; i < pEditor->GetEditData()->modelData.modelSet.size () ; i ++)
	 {
		 ...
		 {
			MVec3  curPos = pModel->GetModelData ()->pos ;
			MVec3 curDir = curPos - g_Camera.GetPosition ();
			Normalize(curDir);
...
bool   MSelectObjectProcess::DelSelectScreenObject( MTerrainEdit* pEditor ,MVec3 &dir,MVec3 &pos, int mode )
{
	EditorModel* pModel;
	...
	for( iter = pEditor->m_CurSelectModelSet.begin () ; iter != pEditor->m_CurSelectModelSet.end() ; iter++ )
	{
		...
		if(pModel->GetShowModel()->intersectRay(INTERSECT_PICK, ray, NULL))
		{
			(*iter)->GetModelData ()->pDrawbox ->SetColor (  0xffaaaaaa  );
...
bool   MSelectObjectProcess::SelectScreenObject( MTerrainEdit* pEditor , MVec3 &dir,MVec3 &pos, int mode)
{	
	if( !pEditor->GetEditData() )
		return false;
	  ...
	  bool bFind = false;
       ...
       for(unsigned int i = 0 ; i < pEditor->GetEditData()->modelData .modelSet .size () ; i++)
	   {
		   ...
		   if(pModel->GetShowModel()->intersectRay(INTERSECT_PICK, ray, NULL) && !bFind)
		   {
			   ...
			   pEditor->GetEditData()->modelData .modelSet [i]->GetModelData ()->pDrawbox ->SetColor ( 0xff00ff00 );
		   ...
		   {
			   if( pEditor->GetEditData()->modelData .modelSet [i]->GetModelData ()->pDrawbox )
				   pEditor->GetEditData()->modelData .modelSet [i]->GetModelData ()->pDrawbox ->SetColor ( 0xffaaaaaa );
