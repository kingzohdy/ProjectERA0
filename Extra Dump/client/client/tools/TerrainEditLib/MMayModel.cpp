...
inline bool   TestBoxAndBox( MVec3 minSrc , MVec3 maxSrc , MVec3 minDes , MVec3 maxDes )
{
	//创建8个点，判断是否在在源包围盒
	MVec3  vec[8];
...
void   EditorModel::CountLightColor( LightInfo &lightInfo )
{
	...
	//	if( !TestBoxAndBox( minSrc , maxSrc , minDes , maxDes ) )
		...
		for ( size_t j = 0 ;j <m_pLinkMesh->getNumSubMesh(); j ++ )
		{
			...
			for(size_t i=0; i<pvd->getNumVertex(); i++)
			{
				MVec3 pos = *(Vector3 *)pvd->getPosition(i);
				MVec3 normal = *(Vector3 *)pvd->getNormal(i);

				MVec3 pos0 = m_wPos + pos ;
				MVec3 dst = lightInfo.pos - pos0 ;
				...
				r = unsigned char( ( k*lightInfo.color.x * fdot )*255 ) ;
				g = unsigned char( ( k*lightInfo.color.y * fdot )*255 ) ;
				b = unsigned char( ( k*lightInfo.color.z * fdot )*255 ) ;
...
void  EditorModel::UpdateMesh(MVec3 startPos)
{
	if ( m_pModel )
	{
		m_wSavePos = m_wPos ;
		...
		for ( size_t j = 0 ;j <m_pLinkMesh->getNumSubMesh(); j ++ )
		{
			...
			for(size_t i=0; i<pvd->getNumVertex(); i++)
			{
				MVec3 vPos = m_LinkMeshVec[index] + startPos ;
				float curHeight ;
				float fHeight ;
...
void EditorModel::UpdatePos()
{
 ...
 void EditorModel::SetPos(MVec3 pos)
 {
	 m_wPos = pos ;	 
...
void EditorModel::SetDir(MVec3 dir)
{
	m_Dir = dir;
...
void EditorModel::SetLinkMeshTexture(LMTex ptex)
{
	if(m_pLinkMesh)
	{
