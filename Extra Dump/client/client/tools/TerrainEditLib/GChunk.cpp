...
void MChunk::attachToScene(GameScene *pscene)
{
}

void MChunk::detachFromScene(GameScene *pscene)
{
}
...
void MChunk::drawChunkMesh(Ogre::ShaderContext *pcontext, int fillmode, int layer)
{
...
void MChunk::update(unsigned int dtick)
{
	if(m_pLiquid) m_pLiquid->update(dtick);

	MChunk::PLANT_MAP::iterator iter = m_PlantMap.begin() ;
...
uint MChunk::getRenderUsageBits()
{
	return RenderableObject::getRenderUsageBits();
}

void MChunk::getRenderPassRequired(RenderPassDesc &desc)
{
...
void MChunk::render(SceneRenderer *prenderer, const ShaderEnvData &envdata)
{
	int texnum = 0;
	...
	//render water
	if(GetChunkData()->bHaswater && prenderer->m_CurRenderUsage!=RU_REFLECT)
	{
...
void   MChunk::UpdateRenderHeight()
{
	if( m_ChunkRenderData.vb == NULL )
		return;

	if(m_pLiquid) createOrUpdateDepthTexture(m_pLiquid);
...
//更新对应格子高度	
void    MChunk::SetGridHeight(int mx ,int my , float height )
{
...
MVec3   MChunk::GetChunkVec( int mx ,int my )
{
	MVec3 pos(0,0,0);
...
bool MChunk::GetNormal(float x , float z , MVec3* normal)
{
...
bool MChunk::GetHeightEx(float x,float z,float* height0)
{
	if(x > m_ChunkData.maxVec.x || x < m_ChunkData.minVec .x )
		return false;
...
bool MChunk::GetHeight(float x,float z,float* height0, bool use_vb)
{
	...
	{
		if(m_pLiquid->getHeight() > height) height = m_pLiquid->getHeight();
	}
...
void  MChunk::SetLiquidHeight(float height)
{
	if( m_pLiquid )
	{
...
void MChunk::SetLiquidColor(float r, float g, float b, float a)
{
}
	
void  MChunk::UpdateQuadPath()
{	
...
bool  MChunk::CreateQuadPath()
{
	m_pQuadPath = new MPath;

   UpdateQuadPath();

	return true;
};
bool MChunk::CreateTextureData(MTitle* title)
{
	for( int i = 0 ; i < 4 ; i ++)
	{
...
bool MChunk::CreateOtherVecData()
{
	m_AABBox.setRange ( m_ChunkData.minVec , m_ChunkData.maxVec );
...
bool MChunk::CreateVecData()
{	
	return CreateOtherVecData();
}	

Ogre::IndexData *MChunk::createIndexBuffer(int layer)
{
...
void MChunk::CreateRenderPassEx()
{
	...
	m_ChunkRenderData.triNum = m_ChunkRenderData.ib[1]->getNumIndex();
}

void MChunk::CreateRenderPass()
{
    m_RenderPassSet.passSet .clear ();
...
bool MChunk::CreateRenderData(MTitle* title)
{
	...
	if(m_ChunkData.bHaswater != 0)
	{
		...
		m_pLiquid = new MLiquid;
		if(pliquid->type == 0) m_pLiquid->createGeneralLiquid(pliquid->name, 30);
		else m_pLiquid->createReflectLiquid("scene\\environment\\water\\waternormal.dds", "scene\\environment\\water\\bump.dds");
...
bool  MChunk::DelPlant(MVec3 pos , float fSize )
{
	PLANT_MAP::iterator iter = m_PlantMap.begin();
...
bool MChunk::AddPlant( const char* filename , MVec3 pos , float fScale , unsigned int color)
{
	...
	CPlantSet* pPlant = new CPlantSet;
...
void  MChunk::UpdateMapInfoToColorMap(int nSelectModel)
{
...
//更新地图信息（行走层等信息）
void  MChunk::UpdateMapInfoToColorMap()
{
...
void MChunk::createOrUpdateDepthTexture(Ogre::TLiquid *pliquid)
{
	...
	if(pdestbits)
	{
		...
		for(int ii=0;ii<64;ii++)
		{
			...
			{
				float dx = pdata->minVec.x + (ii+0.5f)*(pdata->maxVec.x - pdata->minVec.x)/64.0f;
				float dz = pdata->minVec.z + (jj+0.5f)*(pdata->maxVec.z - pdata->minVec.z)/64.0f;
				float y;
...
bool MChunk::UpdateLiquid(EditOptionWater &water)
{
	char filename[256];
	if(water.watertype == 0) filename[0] = 0;
	else sprintf(filename,"scene\\environment\\water\\%s\\%s", water.animtex, water.animtex);
	...
	if(water.watertype!=m_LiquidData.type || strcmp(filename, m_LiquidData.name)!=0)
	{
		...
		if(water.watertype < 0) return true;
		if(water.watertype==0 && filename[0]==0) return true;
		...
		else
		{
			...
			m_pLiquid->createReflectLiquid("scene\\environment\\water\\waternormal.dds", "scene\\environment\\water\\bump.dds");
		}
	...
	if(water.waterheight != m_LiquidData.waterheight)
	{
		m_LiquidData.waterheight = water.waterheight;
		//g_Terrain.m_pReflect->m_Height = water.waterheight;
		update_depth = true;
	}
...
void MChunk::Release() 
{
	...
	MSAFE_DELETE(m_pQuadPath);

	for( unsigned int i = 0 ; i < m_ChunkPlantData.size () ; i ++)
	{
		MSAFE_DELETE( m_ChunkPlantData[i] );
	}
	...
	for(  ; iter!=  m_PlantMap.end() ; iter ++ )
	{
		MSAFE_DELETE( iter->second ) ;
	}
	m_PlantMap.clear();
	
};

bool  MChunk::SetCullInfoEx(int x,int z, unsigned char nCull , bool bUpdate , int mode )
{
	if( !bUpdate )
		return false;
...
//mode == 1 ，表示恢复裁剪
void  MChunk::SetCullInfo(int x,int z,unsigned char bCull , int mode )
{
...
void   MChunk::UpdateTexRepeat( int nRepeat )
{
 ...
 float MChunk::GetGridPosX(float x)
 {
	 {
		 return x-m_NewVecData.vVer[0].pos.x ;
	 }
 }
 float MChunk::GetGridPosY(float y)
 {
	 
	 return y-m_NewVecData.vVer[0].pos.y ;
 }
 float MChunk::GetGridPosZ(float z)
 {
	 
	 return z-m_NewVecData.vVer[0].pos.z ;
 }
	
 bool  MChunk::SavePlantFile(const char* filename)
 {
...
extern  void ScaleMapPoint(MVec3 &pt);

 bool  MChunk::LoadPlantFile(const char* filename)
 {
	 MStream stream;
 ...
 void   MChunk::SetLightColor( unsigned int color )
 {	
 ...
 void  MChunk::UpdatePlantLightColor()
 {
     //如果有植物，则算植物的顶点颜色
	 MChunk::PLANT_MAP ::iterator iter = m_PlantMap.begin();
 ...
 void  MChunk::CountLightColor(int lighttype, MVec3 pos, Ogre::ColourValue color, Ogre::ColourValue ambient, float range, float shadowdensity)
 {
	 if(lighttype == 0)
	 {
		 if( range <= 0 )
	 ...
	 {
		 float strength;
		 MVec3 normal = m_NewVecData.vVer[i].normal ;
		 Normalize( normal );

		 if(lighttype == 0)
		 {
			 MVec3 pos0 = vdata[i].pos ;
			 MVec3 dst = pos - pos0 ;
 ...
 //判断每个格子是否有水
 bool  MChunk::GetGridIfWater( int mx , int my )
 {
	 if ( m_ChunkData.vecNum == 145 )
		 return false;
