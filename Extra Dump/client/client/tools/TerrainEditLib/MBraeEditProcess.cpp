...
MBraeEditProcess::MBraeEditProcess(void)
{
	//设置处理的类型
...
//增加模型; 参数( 文件名,位置 )
TerrModel*  MBraeEditProcess::AddModel( const char* filename , MVec3 pos )
{
...
//保存;参数( 文件名 ) ; 返回 ( 为真表示成功 )	
bool    MBraeEditProcess::Save( const char* filename )
{	
...
//动态引导; 参数( 编辑器指针 )
void    MBraeEditProcess::Loading( MTerrainEdit* pEditor )
{
	if( !m_bLoaded )
		return;
...
bool    MBraeEditProcess::Read( MTerrainEdit* pEditor ,const char* filename )
{
...
//删除当前斜坡; 参数( 编辑器指针 )
void   MBraeEditProcess::DelCurBrae( MTerrainEdit* pEditor )
{
...
//删除斜破; 参数( 编辑器指针, 斜坡信息 )
void  MBraeEditProcess::DelBrea(  MTerrainEdit* pEditor ,MBreaInfo*  pInfo)
{
...
void  MBraeEditProcess::AddBrea( MTerrainEdit* pEditor , MVec3 dir , MVec3 pos ) 
{		
	MVec3  pos0;
...
//设置斜坡模型; 参数( 文件名 )
void   MBraeEditProcess::SetBraeModelName( std::string &name )
{	
	TerrModel*  model = new TerrModel;
...
bool   MBraeEditProcess::FindeBrea( int gx ,int gy ,int mx ,int my )
{
	for( size_t i = 0 ; i < m_BreaInfoSet.size() ; i ++ )
	{
...
void   MBraeEditProcess::UpdateSaveChunkHeight( int gx ,int gy ,int mx ,int my , float fHeight )
{
	int indexx = mx - 4 ;
	int indexy = my - 4 ;
...
void  MBraeEditProcess::UpdateChunkHeight( int gx, int gy  ,int mx ,int my )
{
	int indexx = mx - 4 ;
	int indexy = my - 4 ;
...
bool MBraeEditProcess::Process( MTerrainEdit* pEditor , ActionEvent *event )
{
...
//后退操作 ; 参数( 编辑器指针 )
void  MBraeEditProcess::Undo( MTerrainEdit *pEditor )
{
...
//保存数据 ; 参数( 编辑器指针)
void  MBraeEditProcess::SaveData( MTerrainEdit* pEditor )
{
