...
namespace TE
{
	EditorMaskMap g_NavMap;

	EditorMaskMap::EditorMaskMap(void)
	{
		memset( &m_NavData , 0 ,sizeof( MaskMapData ));
		memset( &m_Head, 0 , sizeof( MaskMapHeader ));
	}
	...
	void    EditorMaskMap::Release()
	{
		OGRE_DELETE_ARRAY(m_NavData.pdata);
	};

	bool EditorMaskMap::SaveBmpFile( const char* filename, int layer)
	{
	...
	bool EditorMaskMap::LoadBmpFile(const char* filename)
	{
		...
		fread(&headInfo, sizeof(headInfo), 1, fp);

		unsigned char colorData[4*256];
		size_t xxx = sizeof(colorData);
		fread(colorData, sizeof(colorData), 1, fp);

		int num = headInfo.biSizeImage;
		int layer = 0;
		unsigned char *pdata = new unsigned char[num];
		m_NavData.pdata = new unsigned char[num];
		memset(m_NavData.pdata, 0, num);

		fread(pdata, num, 1, fp);
		for(int i=0; i<num; i++)
		{
			if ( 0 != pdata[i] )
			{
				m_NavData.pdata[i] = (1<<layer);
			}
		}
	...
	bool EditorMaskMap::SaveNavFile (const char* filename)
	{
		int ret_x, ret_z;
		...
		CQuadTree quadtree;
		quadtree.Create(m_NavData.pdata, m_Head.ngridx, m_Head.ngridz, m_Head.gridsizex);
		char treename[256];
		strcpy(treename, filename);
		char *pdot = strrchr(treename, '.');
		if(pdot) *pdot = 0;
		strcat(treename, ".mqt");

		quadtree.SaveQdtFile(treename);
		return true;
	}
}
