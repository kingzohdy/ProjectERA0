...
namespace TE
{
	...
	void MapMarkRound::SetDivideNum ( int num )
	{
	...
	void MapMark::SetPos(Ogre::Vector3 pos)
	{
		static  Ogre::Vector3 _pos(0,0,0);
	...
	void  MapMarkGroup::PushData(Ogre::Vector3 pos)
	{
	...
	void  MapMarkGroup::Release()
	{
		OGRE_DELETE( m_pSign );
	};

	void  MapMarkGroup::UpdateData(Ogre::Vector3 pos,float rad,int numRound)
	{
		...
		for( int i = 0 ; i < numRound ; i ++)
		{
			...
			Ogre::Vector2  curPos = m_Round.m_RoundPosSet [i];
	...
	void  MapMarkGroup::UpdateData()
	{
		m_nColorIndex = ( rand()/255 ) % 8;
	}

	void  MapMarkGroup::Render()
	{
		if( m_pSign )
		{
			if( m_PosSet.empty ())
