...
namespace TE
{
	...
	void Terrain::update(uint dtick)
	{
		g_dwCurTime += dtick;
	...
	void Terrain::onAttachObject(MovableObject *pobject)
	{
		pobject->addRef();
	...
	void Terrain::onDetachObject(MovableObject *pobject)
	{
		if(IS_KIND_OF(RenderableObject, pobject))
		{
			RenderableObject *prenderable = static_cast<RenderableObject *>(pobject);
	...
	void Terrain::onObjectPosChange(MovableObject *pobject)
	{
	}
	...
	//这个地方还有问题
	void Terrain::queryShaderEnvData(ShaderEnvData &envdata,const Ogre::BoxBound &aabb,const Vector3 &center,
	...
	void Terrain::caculateShadowCamera2(Camera *pcamera, Camera *pdepthcamera)
	{
		bool ptlight = false;
		Vector3 ptpos;
		...
		for(size_t i=0;i<m_EffectObjects.size();i++)
		{
			...
			if(IS_KIND_OF(Light,pobj) && static_cast<Light *>(pobj)->m_DoShadow)
			{
				if(static_cast<Light*>(pobj)->m_Type == LT_DIRECT)
				{
					...
					pobj->getRotation().rotate(Shadowmap::getSingletonPtr()->m_vLightDir,dir);								
				}
				else
				{
					Shadowmap::getSingletonPtr()->m_vLightDir = Vector3(0, -1.0f, 0);
		...
		Vector3 lightdir = Shadowmap::getSingletonPtr()->m_vLightDir;
		Vector3 lightup(0, 0, 1.0f);
		Normalize(lightdir);

		if(ptlight)
		{
			pdepthcamera->setFov(120.0f);
			//pdepthcamera->setFov(60.0f);
			pdepthcamera->setLookDirect(ptpos+Vector3(0,500.0f, 0), lightdir, lightup);
			pdepthcamera->setDepth(10.0f, 5000.0f);
		}
		else
		{
			Matrix4 mView;
			...
			Vector3 viewdir	=	Vector3(mCameraView._13,mCameraView._23,mCameraView._33);
			Normalize(viewdir);

			Vector3 pos;
			if(this->TestAndRay(viewdir, eye, pos))
			{
				m_Viewpoint = pos;
			}

			float dist = 0;
			if(getPhysicsScene()->pick(Ray(eye,viewdir), &dist))
			{
				Vector3 t = eye + viewdir*dist;
				if(t.y > pos.y) pos = t;
			}
			m_Viewpoint = pos;

			pdepthcamera->setOrtho( 5000.0f * Shadowmap::getSingletonPtr()->m_fAccuracy );
			pdepthcamera->setLookDirect(m_Viewpoint-lightdir*10000.0f, lightdir, lightup);
			pdepthcamera->setDepth(5000.0f, 20000.0f);
		}
		pdepthcamera->setRatio(1.0f);
		pdepthcamera->update(0);
		...
		mDepth._41 =  0.5f;
		mDepth._42 =  0.5f;
		Shadowmap::getSingletonPtr()->m_ProjMatrix = matV * matP * mDepth;
		*/

		Shadowmap::getSingletonPtr()->m_ProjMatrix = matV * matP;
	}

	void Terrain::caculateShadowCamera(Ogre::Camera *pcamera,Ogre::Camera *pdepthcamera)
	{
		caculateShadowCamera2(pcamera, pdepthcamera);
		return;
		...
		Vector3 vLightDir = Shadowmap::getSingletonPtr()->m_vLightDir;
		Normalize(vLightDir);
		...
		if(ptlight)
		{
			pdepthcamera->setFov(90.0f);
			pdepthcamera->setLookDirect(ptpos+Vector3(0,800.0f, 0), vLightDir, vLightUp);
	...
	void Terrain::onCull(Camera *pcamera)
	{
		s_RenderRecursive++;
		TerrainCull terrcull;
		terrcull.Cull(this);
		...
		{
			EditorTerrainBlock *pchunk = terrcull.m_ShowChunk[i];
			...
			{
				MapPath *ppath = pchunk->GetQuadLine();
				ppath->Render();
			}
		}
		...
		g_LightManager.onCull();
		g_TerrainEdit.getNpcManager()->onCull();

		pcamera->m_pCullresult->addRenderable(this, g_TerrainEdit.m_pLinesRenderer, 0, NULL);
	}

	void Terrain::onRender(Ogre::SceneRenderer *prenderer)
	{
		assert(0);	
		/*
		s_RenderRecursive++;
	...
	MovableObject *Terrain::pickObject(IntersectType type, const WorldRay &ray, float *collide_dist, uint pickflags)
	{
		Ray rray;
		...
		{		
			EditorTerrainTile *pTile = m_TitleSet[k];
			...
			{	
				...
				{
					if(!CanEditModelType(pmodel->cType) && pmodel->cType!=TYPE_LINK_MODEL)
						continue;
				}
				else
				{
					if(!CanEditModelType(pmodel->cType)) continue;
	...
	bool Terrain::pickGround(const WorldRay &worldray, float *collide_dist)
	{
	...
	bool Terrain::pickGround(WPOS_T x, WPOS_T z, WPOS_T *y, Ogre::Vector3 *pnormal, float *water_height)
	{
		assert(0);
		return false;
	}

	bool   Terrain::GetGridPos(Ogre::Vector3* pos,int gx,int gz)
	{
	...
	Ogre::Vector3  Terrain::CountPos(Ogre::Vector3  &dir,Ogre::Vector3  &pos,float dz,int mode)
	{
		Ogre::Vector3  pos0 = pos + dir*dz*(float)mode;
	...
	bool Terrain::InsterectRay(Ogre::Vector3 &dir , Ogre::Vector3 &pos , Ogre::Vector3 &result)
	{
		WorldRay wray;
		wray.m_Origin = pos;
		wray.m_Dir = dir;
	...
	bool Terrain::TestTerrainAndWater (Ogre::Vector3 &dir, Ogre::Vector3 &pos, Ogre::Vector3 &result, int mode)
	{
		if(IsUnderTerrain(pos))
		{
			result = pos;
		...
		{
			len += step;
			Ogre::Vector3 curpos = pos + len*dir;
			...
			if(IsUnderTerrain(curpos))
			{
				...
				if(TestTriangle(ray, gx1, gz1, gx2, gz2, t))
				{
					...
					if (GetWaterHeight(result.x, result.z, &waterHeight))
					{
						if (result.y < waterHeight)
							result.y = waterHeight;
					}
...
#define  TEST_RAY_NUM  300
	...
	bool Terrain::IsUnderTerrain(const Ogre::Vector3 &pos)
	{
		float h;
	...
	bool Terrain::TestTriangle(const Ogre::Ray &ray, int sx, int sz, int ex, int ez, float &t)
	{
		t = MAX_FLOAT;
	...
	bool  Terrain::TestAndRay(Ogre::Vector3 &dir , Ogre::Vector3 &pos , Ogre::Vector3 &result,int mode)
	{
		if(IsUnderTerrain(pos))
		{
			result = pos;
	...
	bool Terrain::LoadTitle(const char* filename) 
	{	
		Release();
		...
		{	
			...
			Ogre::Vector3 vFromPt   = Ogre::Vector3( 0.0f, 0.0f, -5.0f );
			Ogre::Vector3 vLookatPt = Ogre::Vector3( 0.0f, 0.0f, 0.0f ); 
			Ogre::Vector3 vUpVec    = Ogre::Vector3( 0.0f, 1.0f, 0.0f );
	...
	bool   Terrain::GetGridX( float posx , int* gx)
	{
		for(unsigned int i = 0 ; i < m_TitleSet.size () ; i++)
		{
			MTileData *ptiledata = ((EditorTerrainTile*)m_TitleSet[i])->GetTitleData();
	...
	bool   Terrain::AddTitle(EditorTerrainTile* title)
	{
		if(title == NULL)
			return false;
	...
	bool   Terrain::GetGridZ( float posz , int* gz)
	{
		for(unsigned int i = 0 ; i < m_TitleSet.size () ; i++)
		{
			MTileData *ptiledata = ((EditorTerrainTile*)m_TitleSet[i])->GetTitleData();
	...
	EditorTerrainTile*  Terrain::GetTitle(int index)
	{
	...
	bool    Terrain::SaveCurTile(const char* filename)
	{
		if(m_TitleSet.empty ())
			return false;

		((EditorTerrainTile*)m_TitleSet[0])->SaveFile ( filename );

		((EditorTerrainTile*)m_TitleSet[0])->BuildPhysicsScene2();

		std::string fileName(filename);
		std::string::size_type pos = fileName.find_last_of("\\");
		if (pos != std::string::npos)
		{
			std::string lastFileName = fileName.substr(0, pos+1);
			getPhysicsScene2()->saveData(lastFileName.c_str());
		}
		return true;
	}


	bool Terrain::GetNormal(float x, float z , Ogre::Vector3* normal)
	{
	...
	bool Terrain::GetHeightEx(float x,float z,float* height)
	{
		(*height) = 0;
		for(unsigned int i = 0 ; i < m_TitleSet.size() ; i++)
		{
	...
	bool Terrain::GetHeight(float x,float z,float* height, TerrPickType type)
	{
		(*height) = 0;
	...
	bool Terrain::GetWaterHeight (float x, float z, float* height)
	{
		(*height) = 0.0f;
		...
		for (unsigned int i=0; i<(int)m_TitleSet.size(); i++)
		{
			if (m_TitleSet[i]->GetWaterHeight(x, z, &h) && h>maxh)
			{
				maxh = h;
			}
		}
