...
static bool NewPackageFile( LPCTSTR path)
{
...
bool GameFilePackage::open( const std::wstring &packpath, bool readonly)
{
	m_ReadOnly = readonly;
	m_FP = _wfopen(packpath.c_str(), readonly?_T("rb"):_T("r+b"));
	if(m_FP == NULL)
	{
		if(readonly)
		{
		...
		else
		{
			...
			m_FP = _wfopen(packpath.c_str(), _T("r+b"));
...
bool GameFilePackage::appendOrReplaceFile(uint key1, uint key2, uint flags, const void *pdata, uint originsize, uint packsize)
{
	...
	else
	{
		if (packsize != 0 )
		{
			n = fwrite(pdata, packsize, 1, m_FP);
			info.originsize = originsize;
			info.packsize = packsize;
		}else{
