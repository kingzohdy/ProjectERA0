...
void CFrameCtrl::OnDraw( CDC* pDC )
{
	...
	for( int i = 0; i < ( ClienRect.Width() - SLIDERLENGHT )/m_PixPerGrid + 1; i++ )
	{
		...
		{
			TCHAR Buf[32];
			swprintf( Buf, 32, _T("%d"), i*m_FramesPerGrid );
	...
	TCHAR Buf[32];
	swprintf( Buf, _T("%d/%d"), (int)m_FrameCur, m_FrameEnd - m_FrameStart );
	pDC->TextOut( BlockRect.left + 3 +(int)( ( BlockRect.Width() - wcslen( Buf )*6.5f )/2 ), BlockRect.top, Buf ); 
}
...
//	CCtrlWnd::OnDraw( pDC );
	...
	for( map<float,DWORD>::iterator it = m_State.begin(); it != m_State.end(); ++it )
	{
		...
		CString str;
		str.Format( _T("%.1f"), (*it).first*m_Range );
		ClienRect.left += 3;
		ClienRect.top += 1;
		pDC->SelectObject( &m_Font );
		pDC->SetBkMode( TRANSPARENT );
...
void CCompBlock::OnLButtonDown(UINT nFlags, CPoint point)
{
	...
	if( m_CurPercent > 0.99f )
		m_CurPercent = 1.0f;
	map<float,DWORD>::iterator it = m_State.begin();
