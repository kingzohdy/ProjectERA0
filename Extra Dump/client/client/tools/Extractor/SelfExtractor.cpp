...
int CSelfExtractor::Create(UINT resource, CString Filename, funcPtr pFn /* = NULL */, void* userData /*=NULL*/)
{
	...
	HRSRC hrSrc = FindResource(AfxGetResourceHandle(), MAKEINTRESOURCE(resource), _T("SFX_EXE"));
	if(hrSrc == NULL)
...
CSEFileInfo* CSelfExtractor::GetItem( LPCTSTR fileName)
{
	for (int i = 0 ; i < m_nFiles ; i ++)
	{
		if (m_InfoArray[i].GetFilename().CompareNoCase(fileName) == 0)
		{
...
uint64 CSelfExtractor::GetAllFileSize()
{
	uint64 result = 0;
	for (int i = 0 ; i < m_nFiles; i++ )
	{
		result += (uint64)GetFileSize(i);
	}
	return result;
}