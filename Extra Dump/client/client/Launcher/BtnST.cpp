...
#define BST_TYPEMASK SS_TYPEMASK
...
void CButtonST::PreSubclassWindow() 
{
	...
	ModifyStyle(BST_TYPEMASK, BS_OWNERDRAW, SWP_FRAMECHANGED);
...
{
	UINT nNewType = UINT(wParam & BST_TYPEMASK);
	...
	return DefWindowProc(BM_SETSTYLE,
		(wParam & ~BST_TYPEMASK) | BS_OWNERDRAW, lParam);
} // End of OnSetStyle
...
void CButtonST::DrawTheBitmap(CDC* pDC, BOOL bHasTitle, RECT* rpItem, CRect* rpCaption, BOOL bIsPressed, BOOL bIsDisabled)
{
	...
	//***按下后使用第三种状态位图，不可用也使用第三种状态位图
	if(( bIsPressed ) || (bIsDisabled))
	{
		byIndex = 2;
	}
...
// 	::TransparentBlt( pDC->m_hDC, rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcMem, 0, 0, 
// 		m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, m_csBitmaps[byIndex].crTransparent );


 	//::BitBlt(pDC->m_hDC, rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, SRCPAINT);
	::TransparentBlt( pDC->m_hDC, rImage.left, rImage.top, m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, hdcBmpMem, 0, 0, 
		m_csBitmaps[byIndex].dwWidth, m_csBitmaps[byIndex].dwHeight, m_csBitmaps[byIndex].crTransparent );
...
DWORD CButtonST::SetBitmaps(int nBitmapIn, COLORREF crTransColorIn, int nBitmapOut, COLORREF crTransColorOut)
{
	...
	switch (nBitmapOut)
	{
		...
		case (int)BTNST_AUTO_GRAY:
			{
				LONG	bmp = BTNST_AUTO_GRAY;

				hBitmapOut = *((HBITMAP*)&bmp);
			}
			break;
		case (int)BTNST_AUTO_DARKER:
			{
				LONG	bmp = BTNST_AUTO_DARKER;
...
//			Failed creating mask bitmap.
//
DWORD CButtonST::SetBitmaps(HBITMAP hBitmapIn,  COLORREF crTransColorIn,
							HBITMAP hBitmapOut, COLORREF crTransColorOut,
							HBITMAP hBitmapOvr, COLORREF crTransColorOvr)
{
	int		nRetValue = 0;
	BITMAP	csBitmapSize;
	...
	if (hBitmapIn)
	{
		...
		if (hBitmapOvr)
		{
			m_csBitmaps[2].hBitmap = hBitmapOvr;
			m_csBitmaps[2].crTransparent = crTransColorOvr;
			// Get bitmap size
			nRetValue = ::GetObject(hBitmapOvr, sizeof(csBitmapSize), &csBitmapSize);
			if (nRetValue == 0)
			{
				FreeResources();
				return BTNST_INVALIDRESOURCE;
			...
			// Create mask for bitmap Out
			m_csBitmaps[2].hMask = CreateBitmapMask(hBitmapOvr, m_csBitmaps[2].dwWidth, m_csBitmaps[2].dwHeight, crTransColorOvr);
