...
const int MAX_CHECK_FOR_COMPLETE_FILE_NUM = 17;
...
static void KEY(char Out[8], char In[8], const PSubKey pSubKey, EnCodeOp Type);//标准KEY加/解密
...
bool Key_Go(char *Out, char *In, long datalen, const char *Key, int keylen, EnCodeOp Type)
{
	...
	if( Is3DES ) 
	{  
		// 3次KEY 加密:加(key0)-解(key1)-加(key0) 解密::解(key0)-加(key1)-解(key0)
		for ( long i = 0, j = datalen >> 3; i < j; ++i, Out += 8, In += 8 )
		{
			bool bTmpBuffer = Type;
			/*KEY(Out, In,  &SubKey[0], Type);
			KEY(Out, Out, &SubKey[1], (EnCodeOp)(!bTmpBuffer) );
			KEY(Out, Out, &SubKey[0], Type);*/
			
			KEY( Out, In, &SubKey[0], Type );
			KEY( Out, In, &SubKey[1], (EnCodeOp)(!bTmpBuffer) );
			KEY( Out, In, &SubKey[0], Type );
		}
	} 
	else
...
void SetKey(const char* Key, int len)
{
	...
	// 此为密钥Key--gStrKey
	memcpy(deskey, Key, len > 16 ? 16 : len /*min( len, sizeof( deskey ) /sizeof( char ) )*/ );
	SetSubKey(&SubKey[0], &deskey[0]);
	// TODO: 这步 deskey是否会越界
	Is3DES = len > 8 ? (SetSubKey(&SubKey[1], &deskey[8]), true) : false;
}

void KEY(char Out[8], char In[8], const PSubKey pSubKey, EnCodeOp Type)
{
    static bool M[64];
	static bool tmp[32]; 
	static bool* Li = &M[0]; 
...
void SetSubKey(PSubKey pSubKey, const char Key[8])
{
    ...
    ByteToBit(K, Key, 64);
    Transform(K, K, PC1_Table, sizeof( PC1_Table ) / sizeof( char ) );
