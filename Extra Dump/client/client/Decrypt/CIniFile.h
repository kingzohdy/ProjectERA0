...
public:
	...
	// 得到某个Section下的Key的值
	bool	GetKeyValue(const char*	lpSection, const char*	lpKeyName, char*	lpRString, unsigned int nSize); 

	// 得到某个Section下的字符串Key的值
	void	GetString(const char*	lpSection,	const char*	lpKeyName,	const char*	lpDefault,	char*	lpRString,	unsigned int	nSize);
