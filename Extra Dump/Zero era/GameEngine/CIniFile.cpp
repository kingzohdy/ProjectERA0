...
bool CIniFile::GetKeyValue(const char*	lpSection,const char*	lpKeyName,	char*	lpRString, uint32	nSize)
{
...
//	读取一个字符串
void CIniFile::GetString(const char* lpSection,	const char* lpKeyName,	const char* lpDefault,char* lpRString, uint32 nSize	)
{
...
// 删除一个 Section下的Key的值
void CIniFile::RemoveSection( const char* lpSection, const char* lpKeyName )
{
	map< string, map<string, string> >::iterator itSection = GetIniFileHandle( m_hFileHandle ).m_Struct.find( lpSection );
	if( itSection == GetIniFileHandle( m_hFileHandle ).m_Struct.end() )
		return ;
