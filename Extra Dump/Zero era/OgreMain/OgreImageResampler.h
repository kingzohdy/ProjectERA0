...
namespace Ogre {
	...
	template<unsigned int elemsize> struct NearestResampler {
		static void scale(const PixelBox& src, const PixelBox& dst) {
			...
			for (int z = dst.m_Front; z < dst.m_Back; z++, sz_48 += stepz) {
				int srczoff = (int)(sz_48 >> 48) * (int)src.slicePitch;

				uint64 sy_48 = (stepy >> 1) - 1;
				for (int y = dst.m_Top; y < dst.m_Bottom; y++, sy_48 += stepy) {
					int srcyoff = (int)(sy_48 >> 48) * (int)src.rowPitch;

					uint64 sx_48 = (stepx >> 1) - 1;
					for (int x = dst.m_Left; x < dst.m_Right; x++, sx_48 += stepx) {
						uchar* psrc = srcdata +
	...
	struct LinearResampler {
		static void scale(const PixelBox& src, const PixelBox& dst) {
			...
			for (int z = dst.m_Front; z < dst.m_Back; z++, sz_48+=stepz) {
				...
				int sz1 = int(temp >> 16);				 // src z, sample #1
				int sz2 = Min(sz1+1,src.getDepth()-1);// src z, sample #2
				...
				for (int y = dst.m_Top; y < dst.m_Bottom; y++, sy_48+=stepy) {
					temp = (uint)(sy_48 >> 32);
					temp = (temp > 0x8000)? temp - 0x8000 : 0;
					int sy1 = temp >> 16;					// src y #1
					int sy2 = Min(sy1+1,src.getHeight()-1);// src y #2
					...
					for (int x = dst.m_Left; x < dst.m_Right; x++, sx_48+=stepx) {
						temp = (uint)(sx_48 >> 32);
						temp = (temp > 0x8000)? temp - 0x8000 : 0;
						int sx1 = temp >> 16;					// src x #1
						int sx2 = Min(sx1+1,src.getWidth()-1);// src x #2
...
#undef UNPACK
	...
	struct LinearResampler_Float32 {
		static void scale(const PixelBox& src, const PixelBox& dst) {
			...
			uint64 sz_48 = (stepz >> 1) - 1;
			for (int z = dst.m_Front; z < dst.m_Back; z++, sz_48+=stepz) {
				temp = (unsigned int)(sz_48 >> 32);
				temp = (temp > 0x8000)? temp - 0x8000 : 0;
				int sz1 = temp >> 16;				 // src z, sample #1
				int sz2 = Min(sz1+1,src.getDepth()-1);// src z, sample #2
				float szf = (temp & 0xFFFF) / 65536.f; // weight of sample #2

				uint64 sy_48 = (stepy >> 1) - 1;
				for (int y = dst.m_Top; y < dst.m_Bottom; y++, sy_48+=stepy) {
					temp = (unsigned int)(sy_48 >> 32);
					temp = (temp > 0x8000)? temp - 0x8000 : 0;
					int sy1 = temp >> 16;					// src y #1
					int sy2 = Min(sy1+1,src.getHeight()-1);// src y #2
					float syf = (temp & 0xFFFF) / 65536.f; // weight of #2
...
#define ACCUM3(x,y,z,factor) \
						{ float f = factor; \
						int off =(int) ((x+y*src.rowPitch+z*src.slicePitch)*srcchannels); \
...
#undef ACCUM4
	...
	template<unsigned int channels> struct LinearResampler_Byte {
		static void scale(const PixelBox& src, const PixelBox& dst) {
			...
			uint64 sy_48 = (stepy >> 1) - 1;
			for (int y = dst.m_Top; y < dst.m_Bottom; y++, sy_48+=stepy) {
				temp = (unsigned int)(sy_48 >> 36);
				temp = (temp > 0x800)? temp - 0x800: 0;
				unsigned int syf = temp & 0xFFF;
				int sy1 = temp >> 12;
				int sy2 = Min(sy1+1, src.m_Bottom-src.m_Top-1);
				int syoff1 = sy1 * (int)src.rowPitch;
				int syoff2 = sy2 * (int)src.rowPitch;
				...
				for (int x = dst.m_Left; x < dst.m_Right; x++, sx_48+=stepx) {
					...
					int sx1 = temp >> 12;
					int sx2 = Min(sx1+1, src.m_Right-src.m_Left-1);
