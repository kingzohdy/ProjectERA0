	...
	*/
	...
	public:
		...
		*/
		...
		friend Archive &operator<<(Archive &ar, ColourValue &color)
		{
			ar.serializeRawType(color);
			return ar;
		}
	};

	inline ColourValue Lerp( const ColourValue &v1, const ColourValue &v2, float t )
	{
		return ColourValue(v1.r+(v2.r-v1.r)*t, v1.g+(v2.g-v1.g)*t,v1.b+(v2.b-v1.b)*t, v1.a+(v2.a-v1.a)*t);
	}

	inline RGBA MakeRGBA(uint r, uint g, uint b, uint a=255)
	{
	...
	public:
		...
		ColorQuad(){}
		ColorQuad(RGBA rgba) :c(rgba){}
		ColorQuad(uchar r1, uchar g1, uchar b1, uchar a1=255)
			:r(r1), g(g1), b(b1), a(a1)
		{
		}
		ColorQuad(const ColourValue &rhs)
		{
			r = (unsigned char)Clamp(int(rhs.r*255), 0, 255);
			g = (unsigned char)Clamp(int(rhs.g*255), 0, 255);
			b = (unsigned char)Clamp(int(rhs.b*255), 0, 255);
			a = (unsigned char)Clamp(int(rhs.a*255), 0, 255);
		}

		void set(uchar r1, uchar g1, uchar b1, uchar a1=255)
		{
			r = r1, g = g1, b = b1, a = a1;
		}

		void set(uint color){ c = color; }
	};

	inline ColorQuad Lerp( const ColorQuad color1, const ColorQuad color2, float t )
	{
		return ColorQuad(   (unsigned char)(color1.r+(color2.r-color1.r)*t), 
			(unsigned char)(color1.g+(color2.g-color1.g)*t), 
			(unsigned char)(color1.b+(color2.b-color1.b)*t), 
			(unsigned char)(color1.a+(color2.a-color1.a)*t) 
			);
	}

	inline bool operator==(ColorQuad c1, ColorQuad c2)
	{
		return c1.c == c2.c;
	}
} // namespace
