...
mkdir -p ${bin_dir}/bin_deploy/{apexsvr,sql,tools/script_doc,informer_tool,resdb,db_unblob_interface/include,tcm/{bin,cfg,lib,log},dir/cfg,db/db_meta,zone/{bin,cfg,script,cfg/{db_meta,res,fonts,dirty}},logsvr/{cfg,res_bin},version,bill,auth,informer,aas,service,region,accdb,mud,battle,recv/}
...
Generate_Sql()
{
	...
	cp -f ~/guoji/resdb/resdb_data.sql $bin_dir/bin_deploy/resdb/
...
cp -f ~/guoji/script/* $bin_dir/bin_deploy/zone/script/
...
cp -f ~/guoji/shopping/zone_shopping.txt $bin_dir/bin_deploy/zone/cfg/zone_shopping.xml
cp -f ~/myworld/cfg/resdb_meta.tdr $bin_dir/bin_deploy/zone/cfg/
cp -f ~/myworld/cfg/black_white.cfg $bin_dir/bin_deploy/zone/cfg/
cp -f ~/guoji/res/* $bin_dir/bin_deploy/zone/cfg/res/
cp -f ~/guoji/msk/* $bin_dir/bin_deploy/
