...
bool DBFetcher::Query(const std::string &meta_name)
{
	...
	std::string sql;
	sql.resize(1024, 0);
	int len = _snprintf(&sql[0], sql.length(), "SELECT * FROM %s WHERE 1",
	                    meta_name.c_str());
	sql.resize(len);
...
bool DBFetcher::Bin2Xml(std::string *xml_string)
{
	...
	int rv = tdr_output(meta_, &out, &host, version_, TDR_XML_DATA_FORMAT_LIST_ENTRY_NAME);
	if(rv < 0)
	{
