...
int init_dbhandle(LPTDRMETALIB* pstResLib,TDRDBHANDLE *hResDBHandle, DB_TO_BIN_CONF *pstConf)
{
	...
	if (pstConf->szResDBName[0])
	{
		STRNCPY(stConf.szResDBName, pstConf->szResDBName, sizeof(stConf.szResDBName));
	}
...
int main(int argc, char* argv[])
{
	...
	if (argc == 2)
	{
		STRNCPY(stConf.szResDBName, argv[1], sizeof(stConf.szResDBName));
	}
	else
	{
		stConf.szResDBName[0] = 0;
	}
	...
	return 0;
	UNUSED(argc);	//add by paraunused.pl
	UNUSED(argv);	//add by paraunused.pl
}
