...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"OpTradeInfo":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="10%" >'+value['LoginName']+'</td><td width="10%">'+value['RoleName']+'</td><td width="15%" >'+value['TradeID']+'</td><td width="10%" >'+value['OperType']+'</td><td width="10%" >'+value['Gold']+'</td><td width="10%" >'+value['Money']+'</td><td width="10%" >'+value['LogTime']+'</td><td width="25%" >'+value['OperDesc']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">商城--金银币交易</span>
