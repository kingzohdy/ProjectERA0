...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"mailOper":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="7%" >'+value['Uin']+'</td><td width="7%" >'+value['LoginName']+'</td><td width="7%" >'+value['RoleName']+'</td><td width="6%" >'+value['Operation']+'</td><td width="13%">'+value['mid']+'</td><td width="5%" >'+value['Flag']+'</td><td width="7%">'+value['Send']+'</td><td width="7%" >'+value['to']+'</td><td width="7%" >'+value['Title']+'</td><td width="25%" >'+value['Items']+'</td><td width="9%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">邮件操作</span>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
                ...
                <td width="7%">收件人</td>
                <td width="7%">标题</td>
                <td width="25%">物品：唯一ID：邮寄物品数量：有效时间</td>
                <td width="9%">时间</td>
		</tr>
</table>
