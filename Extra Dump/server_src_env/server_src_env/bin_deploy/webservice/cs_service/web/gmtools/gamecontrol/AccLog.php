...
function sear()
{
	...
	var fu=$("#fw").val();
		...
		$.ajax({
 			...
 			url: 'select_log.php',
			dataType: 'json',
  			data: {"acclog":"","ldb": f,"time":tb, "time1":tb1, "acc":acc}, 
  			success: function(msg){
				if( null ==msg || ""== msg)
				{
					$("#tt1").html("未检索到数据......");
				...
				else
				{
					...
					$("#ttt").hide();
					$("#ajaxlist").empty();
					var ss='<tr class=thead2 align=center><td >账号</td><td >登录(登出)时间</td><td >Ip地址</td><td >客户端类型</td></tr>';
					$("#ajaxlist").append(ss);
					//处理检索到的数据......
					var row1 = msg.row1;
					var row2 = msg.row2;
					...
					for(var x=0;x<nums;x++)
					{
						if(ii<nums1)
						{
							...
							var s = '<tr class=mgr_tr align=center ><td height=32 >'+denglv.AccountLoginName+'</td><td >'+denglv.LogTime+'</td><td >'+denglv.IP+'</td><td >'+type+'</td></tr>';
							$("#ajaxlist").append(s);
							ii++;
						}
						if(jj<nums2)
						{
							dengchu = row2[jj];
							var cc = '<tr class=mgr_tr align=center style=color:#F00;><td height=32 >'+dengchu.AccountLoginName+'</td><td >(登出)：'+dengchu.LogTime+'</td><td>未记录</td><td>未记录</td></tr>';
							$("#ajaxlist").append(cc);
							jj++;
						}
						
					}
					var dd = '<tr><td colspan=5 class=mgr_divt></td></tr>';
					$("#ajaxlist").append(dd);
				}
			}
		});
...
<body>
<div class="mgr_header"> <span class="title">帐号--登录登出记录</span>
...
<table width="100%" height="100%">
	<tr>
        ...
        <?php } ?> d
			...
			帐号：<input type="text" id="us" name="us"  onblur="jc()" />
