	...
	if(jintime >= 0)
	{
		...
		*/
		...
		({
			...
			url: 'conduct.php',
			dataType: 'json',
			data: {"dongjie":"","cause":cause,"jintime": jintime,"daqu":daqu,"Account":Account}, 
			beforeSend:function(){
	...
	if(jintime >= 0)
	{
		...
		*/
		...
		({
			...
			url: 'conduct.php',
			dataType: 'json',
			data: {"dongjieRole":"","jintime": jintime,"daqu":daqu,"fu":fu,"RoleName":RoleName}, 
			beforeSend:function(){
				$("#BgDiv").css({ display: "block", height: $(document).height() });
				$('#DialogDiv').show();
			 },
...
<div id="DialogDiv" style="display:none">
 ...
 <?php if($f != -1) { echo "<!--";} ?><p>请输入冻结原因：<br/></p>
<p><textarea id="cause" name="cause" style="width:280; height:120"></textarea></p>
<?php if($f != -1) { echo "-->";} ?>
<p>请输入冻结时间：(0表示解禁)<br/><span style="color:#F00; font-size:12px; font-weight:bold" id="ts"></span></p>
<p>
  <input type="text" id="jt" name="jt" />秒
  <input type="button"  <?php if($f != -1){ echo "onclick='tj1()'"; } if($f== -1) {echo "onclick='tj()'";} ?> value="确认" />
  <input type="hidden" value="<?php echo $q; ?>" id="daqu" />
  <input type="hidden" value="<?php if($f == -1){echo $AccName;} if($f != -1){ echo $RoleName;} ?>" id="Account" />
  <input type="hidden" value="<?php if($f) {echo $f;} ?>" id="fu" />
</p>
</body>
</html>
