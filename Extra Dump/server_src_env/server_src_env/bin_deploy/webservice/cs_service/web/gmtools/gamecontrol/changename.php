...
$clan = $_POST['clan'];
	...
	//日志记录
	$log1 = "<b>修改角色名</b><hr class=dotline>大区:&nbsp;".$qu.",&nbsp;&nbsp;服：&nbsp;".$fu."<hr class=dotline>操作者：".$user['username']."&nbsp;&nbsp;（角色名：".$clan."）<hr class=dotline>操作时间为：".$dqtime."<hr class=dotline>角色名改为：".$roleName;
	
 $result="";
$result = ChgRoleName($qu,$fu,$clan,$roleName);
	if($result->ErrorCode==0 )
	{
		$log2 = "，操作成功";
		$gmlog = $log1.$log2;
		$sql = "INSERT INTO `gmlog` VALUES (null, '$user[id]', '$zhanghao', '', '', '', now(), now(),'', '17','0','$gmlog')";
	...
	if($result->ErrorCode!=0)
	{
		...
		$gmlog = $log1.$log2;
		$sql = "INSERT INTO `gmlog` VALUES (null, '$user[id]', '$zhanghao', '', '', '', now(), now(),'', '17','1','$gmlog');";
		//print_r($arr);
		exit(-1);
	}
}
...
if($err == 1)
{
 ...
 ?> 
<div class="gray_header"> <span class="title">修改角色名</span> <span class="reload"><a href="javascript:location.reload();">刷新</a></span> </div>
<form name="form" id="form" method="post" action="changename.php" >
    ...
    <tr>
        ...
        <tr height="42">
        	<td width="20%" height="35" align="right">原角色名：</td>
             <td width="80%" style="font-size:14px; margin-left:0px;"><input type="text" name="clan"  /></td>
        </tr>
        <tr height="42">
        	<td width="20%" height="35" align="right">新角色名：</td>
