...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"RideChg":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="15%" >'+value['Uin']+'</td><td width="15%" >'+value['LoginName']+'</td><td width="15%" >'+value['RoleName']+'</td><td width="15%">'+value['RideID']+'</td><td width="15%" >'+value['Level']+'</td><td width="10%">'+value['OpType']+'</td><td width="15%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">坐骑操作</span>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
				...
				<td width="15%">账号</td>
				<td width="15%">角色名</td>
				<td width="15%">骑乘的ID</td>
                <td width="15%">骑乘的等级</td>
                <td width="10%">操作类型</td>
                <td width="15%">时间</td>
		</tr>
</table>
