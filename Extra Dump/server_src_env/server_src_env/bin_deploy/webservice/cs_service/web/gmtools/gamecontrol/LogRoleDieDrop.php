...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"LogRoleDieDrop":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="12%" >'+value['wid']+'</td><td width="10%">'+value['Uin']+'</td><td width="9%" >'+value['LoginName']+'</td><td width="9%">'+value['RoleName']+'</td><td width="8%" >'+value['DropNum']+'</td><td width="40%">'+value['Item']+'</td><td width="12%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">死亡掉落</span>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
        		<td width="12%">事件流水号</td>
				<td width="10%">Uin</td>
				<td width="9%">帐号</td>
				<td width="9%">角色名</td>
                <td width="8%">掉落数量</td>
                <td width="40%">掉落（物品名：品质：[星级：强化百分比]：物品全局唯一ID：有效时间）</td>
                <td width="12%">时间</td>
		</tr>
</table>
