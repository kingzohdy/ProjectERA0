...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"LogRoleItemDrop":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="8%" >'+value['Uin']+'</td><td width="10%">'+value['LoginName']+'</td><td width="9%" >'+value['RoleName']+'</td><td width="6%">'+value['OpType']+'</td><td width="8%" >'+value['Item']+'</td><td width="4%" >'+value['pz']+'</td><td width="15%" >'+value['qhlv']+'</td><td width="5%">'+value['ItemNum']+'</td><td width="15%" >'+value['ItemID']+'</td><td width="10%" >'+value['ValidEnd']+'</td><td width="10%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">道具丢弃</span>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
                ...
                <td width="8%">物品名</td>
                <td width="4%">品质</td>
                <td width="15%">强化(星级：强化百分比)</td>
                <td width="5%">丢弃数量</td>
                <td width="15%">物品全局唯一ID</td>
                <td width="10%">物品有效时间</td>
                <td width="10%">时间</td>
		</tr>
</table>
