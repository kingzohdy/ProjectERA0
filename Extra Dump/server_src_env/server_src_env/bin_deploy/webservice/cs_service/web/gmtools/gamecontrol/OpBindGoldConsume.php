...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"OpBindGoldConsume":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="15%" >'+value['Uin']+'</td><td width="15%">'+value['RoleID']+'</td><td width="10%" >'+value['RoleLevel']+'</td><td width="15%" >'+value['OpType']+'</td><td width="10%" >'+value['OpSubType']+'</td><td width="10%" >'+value['BindGoldNum']+'</td><td width="10%" >'+value['OpDesc']+'</td><td width="15%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">金卷消耗记录</span>
