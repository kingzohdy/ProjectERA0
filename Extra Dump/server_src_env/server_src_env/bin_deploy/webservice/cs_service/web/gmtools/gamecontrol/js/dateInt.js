...
function numberToIp(number) {
	//var number = swap32(number);
	a = (Math.floor(number/(256*256*256)));
	b = (Math.floor(number%(256*256*256)/(256*256)));
	c = (Math.floor(number%(256*256)/256));
	d = (Math.floor(number%256));
	return d  + "." + c
	 + "." + b
	 + "." + a;
...
function ntoh ( proper_address ) {
    var output = false;
    if ( !isNaN ( proper_address )) {
    output = (proper_address & 0xff ) + '.' +( (proper_address>>8) & 0xff ) + '.' +( (proper_address>>16) & 0xff ) + '.' +( (proper_address>>24) & 0xff );
    }
    return output;
}
function swap32(val) {
