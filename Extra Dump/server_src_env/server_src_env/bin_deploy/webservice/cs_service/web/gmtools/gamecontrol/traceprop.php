...
function sear()
{
	...
	$.ajax({
		type: 'POST',
		url: 'traceprop_select.php',
		dataType: 'json',
		data: {"traceprop":"","xzdb": f,"time":tb, "us":jsm,"fag":fag,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="16%" >'+value['WID']+'</td><td width="20%">'+value['Items']+'</td><td width="14%" >'+value['Uin']+'</td><td width="14%" >'+value['LoginName']+'</td><td width="14%">'+value['RoleName']+'</td><td width="10%">'+value['oper']+'</td><td width="15%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">道具追踪</span>
...
<table width="100%" height="100%">
	<tr>
        ...
        <?php } ?> 
            ...
            <span style="color:#F00"></span>&nbsp;&nbsp;&nbsp;输入
            <select id="lx">
            	<option value="1" selected="selected">装备/不可叠加类道具ID</option>
                <option value="2" >一次性消耗类道具WID</option>
            </select>
...
<table width="100%" height="100%">
	<tr class="tcss">
		<td>&nbsp;&nbsp;&nbsp;
          查询日期：<input name="time" type="text" id="posttime" class="input_short" onblur="stime()" />
				<script type="text/javascript">
				date = new Date();
				Calendar.setup({
					inputField     :    "posttime",
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
                <td width="16%">道具唯一ID</td>
				<td width="20%">详细</td>
                <td width="14%">操作Uin</td>
				<td width="14%">操作帐号</td>               
				<td width="14%">操作角色名</td>
                <td width="10%">操作类型</td>
                <td width="15%">操作时间</td>
		</tr>
</table>
