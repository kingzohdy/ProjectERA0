...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"ArmMachin":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="8%" >'+value['Uin']+'</td><td width="8%">'+value['LoginName']+'</td><td width="8%" >'+value['RoleName']+'</td><td width="10%" >'+value['oldwid']+'</td><td width="8%" >'+value['Item']+'</td><td width="8%">'+value['oldlv']+'</td><td width="8%">'+value['type']+'</td><td width="8%" >'+value['newwid']+'</td><td width="8%">'+value['Item2']+'</td><td width="8%">'+value['newlv']+'</td><td width="8%" >'+value['Money']+'</td><td width="8%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">装备加工日志</span>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
				...
				<td width="8%">帐号</td>
				<td width="8%">角色名</td>
                <td width="10%">操作前唯一ID</td>
				<td width="8%">操作前名称:品质</td>
				<td width="8%">操作前星级：强化百分比</td>
               	<td width="8%">操作类型</td>
                <td width="8%">操作后唯一ID(0表示没变)</td>
               	<td width="8%">操作后名称:品质</td>              	
               	<td width="8%">操作后星级：强化百分比</td>
                <td width="8%">消耗(银：金币：金卷)</td>
                <td width="8%">时间</td>
		</tr>
</table>
