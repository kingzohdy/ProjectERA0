...
if ($cfgRelation['commwork']) {
    ...
    <p> <?php echo $strDBComment; ?>
        <i><?php echo latin_htmlspecialchars($comment); ?></i></p>
        <?php
    } // end if
}
...
while ($row = PMA_DBI_fetch_assoc($rowset)) {
    $myfieldname = 'Tables_in_' . latin_htmlspecialchars($db);
    ...
    echo '<h2>' . latin_htmlspecialchars($table) . '</h2>' . "\n";
    ...
    if (!empty($show_comment)) {
        echo $strTableComments . ': ' . latin_htmlspecialchars($show_comment) . '<br /><br />';
    }
...
<?php /*    <td<?php echo $type_nowrap; ?>><?php echo $row['Extra']; ?></td>*/ ?>
        ...
        if ($have_rel) {
            ...
            if (isset($res_rel[$field_name])) {
                echo latin_htmlspecialchars($res_rel[$field_name]['foreign_table'] . ' -> ' . $res_rel[$field_name]['foreign_field']);
            }
            echo '</td>' . "\n";
        }
        echo '    <td>';
        if (isset($comments[$field_name])) {
            echo latin_htmlspecialchars($comments[$field_name]);
        }
        echo '</td>' . "\n";
        if ($cfgRelation['mimework']) {
            ...
            if (isset($mime_map[$field_name])) {
                echo latin_htmlspecialchars(str_replace('_', '/', $mime_map[$field_name]['mimetype']));
            }
            echo '</td>' . "\n";
        }
        ?>
