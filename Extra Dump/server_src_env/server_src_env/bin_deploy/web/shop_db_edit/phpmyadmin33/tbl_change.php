...
<?php
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        if (! isset($table_fields[$i]['processed'])) {
            $table_fields[$i]['Field_html'] = latin_htmlspecialchars($table_fields[$i]['Field']);
            ...
            if (isset($comments_map[$table_fields[$i]['Field']])) {
                $table_fields[$i]['Field_title'] = '<span style="border-bottom: 1px dashed black;" title="'
                    . latin_htmlspecialchars($comments_map[$table_fields[$i]['Field']]) . '">'
        ...
        if (isset($vrow)) {
            if (is_null($vrow[$field['Field']])) {
                ...
                //$special_chars   = latin_htmlspecialchars($vrow[$field['Field']]);
                $special_chars   = latin_htmlspecialchars($vrow[$field['Field']],ENT_COMPAT | ENT_HTML401,'ISO-8859-1');
		...
		//We need to duplicate the first \n or otherwise we will lose the first newline entered in a VARCHAR or TEXT column
            ...
            $backup_field  = '<input type="hidden" name="fields_prev'
                ...
                . latin_htmlspecialchars($vrow[$field['Field']]) . '" />';
        } else {
            // (we are inserting)
            ...
            if ($field['True_Type'] == 'bit') {
                ...
                $special_chars = latin_htmlspecialchars($field['Default']);
            }
            $backup_field  = '';
            $special_chars_encoded = PMA_duplicateFirstNewline($special_chars);
        ...
        if ($foreignData['foreign_link'] == true) {
            ...
            <input type="text" name="field_<?php echo $field_name_appendix_md5; ?>"
                ...
                value="<?php echo latin_htmlspecialchars($data); ?>" />
            <script type="text/javascript">
            //<![CDATA[
          ...
          <?php
            ...
            if (! isset($table_fields[$i]['values'])) {
                ...
                foreach ($extracted_fieldspec['enum_set_values'] as $val) {
                    ...
                    $table_fields[$i]['values'][] = array(
                        'plain' => $val,
                        'html'  => latin_htmlspecialchars($val),
                    );
                }
            }
...
<form method="post" action="tbl_replace.php" name="restartForm" >
    ...
    <input type="hidden" name="goto" value="<?php echo latin_htmlspecialchars($GLOBALS['goto']); ?>" />
    <input type="hidden" name="err_url" value="<?php echo latin_htmlspecialchars($err_url); ?>" />
    <input type="hidden" name="sql_query" value="<?php echo latin_htmlspecialchars($sql_query); ?>" />
<?php
    if (isset($where_clauses)) {
        foreach ($where_clause_array as $key_id => $where_clause) {
            echo '<input type="hidden" name="where_clause[' . $key_id . ']" value="' . latin_htmlspecialchars(trim($where_clause)) . '" />'. "\n";
        }
    }
