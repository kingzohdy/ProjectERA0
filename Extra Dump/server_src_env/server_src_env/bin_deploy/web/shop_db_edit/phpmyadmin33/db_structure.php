...
foreach ($tables as $keyname => $each_table) {
    ...
    $alias = (!empty($tooltip_aliasname) && isset($tooltip_aliasname[$each_table['TABLE_NAME']]))
               ? str_replace(' ', '&nbsp;', latin_htmlspecialchars($tooltip_truename[$each_table['TABLE_NAME']]))
               : str_replace(' ', '&nbsp;', latin_htmlspecialchars($each_table['TABLE_NAME']));
    ...
    $row_count++;
    if ($table_is_view) {
        $hidden_fields[] = '<input type="hidden" name="views[]" value="' .  latin_htmlspecialchars($each_table['TABLE_NAME']) . '" />';
    }
    ...
    if (! $db_is_information_schema) {
        if (! empty($each_table['TABLE_ROWS'])) {
            ...
            $empty_table .= urlencode('TRUNCATE ' . PMA_backquote($each_table['TABLE_NAME']))
                 . '&amp;zero_rows='
                 . urlencode(sprintf($strTableHasBeenEmptied, latin_htmlspecialchars($each_table['TABLE_NAME'])))
...
<tr class="<?php echo $odd_row ? 'odd' : 'even'; $odd_row = ! $odd_row; ?>">
    <td align="center">
        <input type="checkbox" name="selected_tbl[]"
            value="<?php echo latin_htmlspecialchars($each_table['TABLE_NAME']); ?>"
