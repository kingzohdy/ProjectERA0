...
?>
<form method="<?php echo $method ?>" action="<?php echo latin_htmlspecialchars($action) ?>">
<?php
...
function display_fieldset_top($title = '', $description = '', $errors = null, $attributes = array())
{
    ...
    foreach ($attributes as $k => &$attr) {
        $attr = $k . '="' . latin_htmlspecialchars($attr) . '"';
    }
...
<tr>
    <th>
        <label for="<?php echo latin_htmlspecialchars($path) ?>"><?php echo $name ?></label>
    ...
    switch ($type) {
        case 'text':
            echo '<input type="text" size="50" ' . $name_id . $field_class
                . ' value="' . latin_htmlspecialchars($value) . '" />';
          break;
        case 'checkbox':
        ...
        case 'select':
            ...
            foreach ($opts['values'] as $opt_value => $opt_name) {
                ...
                // escape if necessary
                if ($escape) {
                    $display = latin_htmlspecialchars($opt_name);
                    $display_value = latin_htmlspecialchars($display_value);
                } else {
                    $display = $opt_name;
                }
        ...
        case 'list':
            echo '<textarea cols="40" rows="5" ' . $name_id . $field_class . '>'
                . latin_htmlspecialchars(implode("\n", $value))
                . '</textarea>';
            break;
    }
    if (isset($opts['setvalue']) && $opts['setvalue']) {
        ?>
        <a class="set-value" href="#<?php echo "$path={$opts['setvalue']}" ?>" title="<?php echo sprintf($GLOBALS['strSetupSetValue'], latin_htmlspecialchars($opts['setvalue'])) ?>" style="display:none"><img alt="set-value" src="../<?php echo $GLOBALS['cfg']['ThemePath'] ?>/original/img/b_edit.png" width="16" height="16" /></a>
        <?php
    }
    ...
    if (isset($opts['errors']) && !empty($opts['errors'])) {
        ...
        foreach ($opts['errors'] as $error) {
            echo '<dd>' . latin_htmlspecialchars($error) . '</dd>';
        }
        echo '</dl>';
    }
    ?>
...
function display_errors($name, $error_list) {
    echo '<dl>';
    echo '<dt>' . latin_htmlspecialchars($name) . '</dt>';
