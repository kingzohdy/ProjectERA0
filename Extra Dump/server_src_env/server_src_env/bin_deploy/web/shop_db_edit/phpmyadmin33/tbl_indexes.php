...
<input type="text" name="index[Key_name]" id="input_index_name" size="25"
    value="<?php echo latin_htmlspecialchars($index->getName()); ?>" onfocus="this.select()" />
</div>
    ...
    foreach ($fields as $field_name => $field_type) {
        if ($index->getType() != 'FULLTEXT'
         || preg_match('/(char|text)/i', $field_type)) {
            echo '<option value="' . latin_htmlspecialchars($field_name) . '"'
                 . (($field_name == $column->getName()) ? ' selected="selected"' : '') . '>'
                 . latin_htmlspecialchars($field_name) . ' [' . $field_type . ']'
                 . '</option>' . "\n";
        }
    } // end foreach $fields
    ?>
