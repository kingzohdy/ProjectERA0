...
if (empty($_REQUEST['search_str']) || ! is_string($_REQUEST['search_str'])) {
    ...
    $searched = latin_htmlspecialchars($_REQUEST['search_str']);
    ...
    foreach ($tables_selected as $each_table) {
        ...
        echo '<tr class="' . ($odd_row ? 'odd' : 'even') . '">'
            .'<td>' . sprintf($GLOBALS['strNumSearchResultsInTable'], $res_cnt,
                latin_htmlspecialchars($each_table)) . "</td>\n";
...
foreach ($tables_names_only as $each_table) {
    ...
    echo '                <option value="' . latin_htmlspecialchars($each_table) . '"'
        . $is_selected . '>'
        . str_replace(' ', '&nbsp;', latin_htmlspecialchars($each_table)) . '</option>' . "\n";
} // end while
...
?>
    ...
    <tr><td align="right">
        ...
        <td><input type="text" name="field_str" size="60"
                value="<?php echo ! empty($field_str) ? latin_htmlspecialchars($field_str) : ''; ?>" /></td>
    </tr>
    </table>
</fieldset>
