...
if (isset($pk)) {
    ...
    ?>
<input type="hidden" name="pk" value="<?php echo latin_htmlspecialchars($pk); ?>" />
    <?php
} else {
    $pk_uri = '';
}
...
<head>
    ...
    function formupdate(field, key) {
        if (opener && opener.document && opener.document.insertForm) {
            ...
            <?php if (isset($pk)) { ?>
            var element_name = field + '[multi_edit][<?php echo latin_htmlspecialchars($pk); ?>][]';
            <?php } else { ?>
...
<input type="hidden" name="field" value="<?php echo latin_htmlspecialchars($field); ?>" />
<input type="hidden" name="fieldkey"
    value="<?php echo isset($fieldkey) ? latin_htmlspecialchars($fieldkey) : ''; ?>" />
<?php if (isset($pk)) { ?>
<input type="hidden" name="pk" value="<?php echo latin_htmlspecialchars($pk); ?>" />
<?php } ?>
<span class="formelement">
    ...
    <input type="text" name="foreign_filter" id="input_foreign_filter"
        value="<?php echo isset($foreign_filter) ? latin_htmlspecialchars($foreign_filter) : ''; ?>" />
...
if (is_array($foreignData['disp_row'])) {
    ...
    //for ($i = 0; $i < $count; $i++) {
        ...
        if (PMA_strlen($val_ordered_current_val) <= $cfg['LimitChars']) {
            $val_ordered_current_val = latin_htmlspecialchars($val_ordered_current_val);
            $val_ordered_current_val_title = '';
        } else {
            $val_ordered_current_val_title =
                latin_htmlspecialchars($val_ordered_current_val);
            $val_ordered_current_val =
                latin_htmlspecialchars(PMA_substr($val_ordered_current_val, 0,
                    $cfg['LimitChars']) . '...');
        }
        if (PMA_strlen($key_ordered_current_val) <= $cfg['LimitChars']) {
            $key_ordered_current_val = latin_htmlspecialchars($key_ordered_current_val);
            $key_ordered_current_val_title = '';
        } else {
            $key_ordered_current_val_title =
                latin_htmlspecialchars($key_ordered_current_val);
            $key_ordered_current_val =
                latin_htmlspecialchars(PMA_substr($key_ordered_current_val, 0,
                    $cfg['LimitChars']) . '...');
        }
    ...
    <tr class="<?php echo $odd_row ? 'odd' : 'even'; $odd_row = ! $odd_row; ?>">
        ...
        echo ($key_ordered_current_equals_data ? '<strong>' : '')
            ...
            .latin_htmlspecialchars($key_ordered_current_key) . '</a>' . ($key_ordered_current_equals_data ? '</strong>' : '');
        ?></td>
        <td>
        <?php
        echo ($key_ordered_current_equals_data ? '<strong>' : '')
        ...
        . '" onclick="formupdate(\'' . md5($field) . '\', \''
        . PMA_jsFormat($val_ordered_current_key, false) . '\'); return false;">' . latin_htmlspecialchars($val_ordered_current_key)
