...
function PMA_displayTableNavigation($pos_next, $pos_prev, $sql_query, $id_for_direction_dropdown)
{
    ...
    $html_sql_query = latin_htmlspecialchars($sql_query);
        ...
        if (isset($analyzed_sql) && isset($analyzed_sql[0]) &&
            ...
            if ($indexes) {
                ...
                foreach ($indexes as $index) {
                    ...
                    echo '<option value="'
                        . latin_htmlspecialchars($unsorted_sql_query  . ' ORDER BY ' . $asc_sort)
                        . '"' . ($local_order == $asc_sort ? ' selected="selected"' : '')
                        . '>' . latin_htmlspecialchars($index->getName()) . ' ('
                        . $GLOBALS['strAscending'] . ')</option>';
                    echo '<option value="'
                        . latin_htmlspecialchars($unsorted_sql_query . ' ORDER BY ' . $desc_sort)
                        . '"' . ($local_order == $desc_sort ? ' selected="selected"' : '')
                        . '>' . latin_htmlspecialchars($index->getName()) . ' ('
                        . $GLOBALS['strDescending'] . ')</option>';
                }
                echo '<option value="' . latin_htmlspecialchars($unsorted_sql_query) . '"' . ($used_index ? '' : ' selected="selected"') . '>' . $GLOBALS['strNone'] . '</option>';
                echo '</select>' . "\n";
...
<tr>
    ...
    for ($i = 0; $i < $fields_cnt; $i++) {
        ...
        if (isset($comments_map) &&
            ...
            $comments = '<span class="tblcomment">' . latin_htmlspecialchars($comments_map[$fields_meta[$i]->table][$fields_meta[$i]->name]) . '</span>';
        } else {
            $comments = '';
        }
        ...
        if ($is_display['sort_lnk'] == '1') {
            ...
            $order_link_content = ($_SESSION['tmp_user_values']['disp_direction'] == 'horizontalflipped' && $GLOBALS['cfg']['HeaderFlipType'] == 'fake' ? PMA_flipstring(latin_htmlspecialchars($fields_meta[$i]->name), "<br />\n") : latin_htmlspecialchars($fields_meta[$i]->name));
             ...
             || $_SESSION['tmp_user_values']['disp_direction'] == 'horizontalflipped') {
                ...
                if ($_SESSION['tmp_user_values']['disp_direction'] == 'horizontalflipped'
                 && $GLOBALS['cfg']['HeaderFlipType'] == 'fake') {
                    echo PMA_flipstring(latin_htmlspecialchars($fields_meta[$i]->name), '<br />');
                } else {
                    echo latin_htmlspecialchars($fields_meta[$i]->name);
                }
                echo "\n" . $comments . '</th>';
            }
            $vertical_display['desc'][] = '    <th '
	...
	//exit;
                    ...
                    if (isset($meta->_type) && $meta->_type === MYSQLI_TYPE_BIT) {
                        ...
                        if ($_SESSION['tmp_user_values']['display_binary']) {
                            ...
                            if ($_SESSION['tmp_user_values']['display_binary_as_hex'] && PMA_contains_nonprintable_ascii($row[$i])) {
                            	...
                            	$row[$i] = latin_htmlspecialchars(PMA_replace_binary_contents($row[$i]));
							}
                        } else {
        ...
        if (!empty($del_url) && $is_display['del_lnk'] != 'kp') {
            $vertical_display['row_delete'][$row_no] .= '    <td align="center" class="' . $class . '" ' . $column_style_vertical . '>' . "\n"
                                                     ...
                                                     .  ' value="' . latin_htmlspecialchars($del_query) . '" ' . (isset($GLOBALS['checkall']) ? 'checked="checked"' : '') . ' />' . "\n"
                                                     .  '    </td>' . "\n";
        } else {
...
function PMA_displayTable(&$dt_result, &$the_disp_mode, $analyzed_sql)
{
    ...
    if (! empty($sort_expression_nodirection)) {
        ...
        if ($sorted_column_index !== false) {
            ...
            $sorted_column_message = ' [' . latin_htmlspecialchars($sort_column) . ': <strong>' . latin_htmlspecialchars($column_for_first_row) . ' - ' . latin_htmlspecialchars($column_for_last_row) . '</strong>]';
    ...
    if ($is_display['del_lnk'] == 'dr' && $is_display['del_lnk'] != 'kp') {
        ...
        echo '<input type="hidden" name="sql_query"'
            .' value="' . latin_htmlspecialchars($sql_query) . '" />' . "\n";
...
function default_function($buffer) {
    ...
    $buffer = latin_htmlspecialchars($buffer,ENT_COMPAT | ENT_HTML401,'ISO-8859-1');
		//var_dump($buffer);
    ...
    if ($size > 0) {
        if ($default_function != $transform_function) {
            ...
            if (stristr($meta->type, 'BLOB') && $_SESSION['tmp_user_values']['display_blob']) {
                ...
                $result = latin_htmlspecialchars(PMA_replace_binary_contents($content));
            }
        }
    }
    return($result);
    ...
    if (isset($map[$meta->name])) {
        ...
        if (isset($GLOBALS['printview']) && $GLOBALS['printview'] == '1') {
            ...
            if ('K' == $_SESSION['tmp_user_values']['relational_display']) {
                ...
                $title = (! empty($dispval))? ' title="' . latin_htmlspecialchars($dispval) . '"' : '';
            } else {
                $title = ' title="' . latin_htmlspecialchars($data) . '"';
            }
		...
		//var_dump($data);
		//var_dump($transform_function);
		//var_dump($default_function);
