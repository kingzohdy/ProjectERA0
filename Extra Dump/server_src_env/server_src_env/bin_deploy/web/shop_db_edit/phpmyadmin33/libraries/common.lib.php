...
function PMA_getIcon($icon, $alternate = '', $container = false, $force_text = false)
{
    ...
    $include_text = false;
    $include_box  = false;
    $alternate    = latin_htmlspecialchars($alternate);
    $button       = '';
...
function PMA_formatSql($parsed_sql, $unparsed_sql = '')
{
    ...
    if (PMA_SQP_isError()) {
        return latin_htmlspecialchars($parsed_sql['raw']); 
    }
                        ...
                        $is_modify_link = true, $back_url = '', $exit = true)
{
    ...
    if (!function_exists('PMA_SQP_isError') || PMA_SQP_isError()) {
        $formatted_sql = latin_htmlspecialchars($the_query);
    } elseif (empty($the_query) || trim($the_query) == '') {
        $formatted_sql = '';
    } else {
        if (strlen($the_query) > $GLOBALS['cfg']['MaxCharactersInDisplayedSQL']) {
            $formatted_sql = latin_htmlspecialchars(substr($the_query, 0, $GLOBALS['cfg']['MaxCharactersInDisplayedSQL'])) . '[...]';
        } else {
    ...
    if (!empty($error_message)) {
        ...
        $error_message = latin_htmlspecialchars($error_message);
...
function PMA_showMessage($message, $sql_query = null, $type = 'notice')
{
    ...
    if ($cfg['ShowSQL'] == true && ! empty($sql_query)) {
        ...
        if (! empty($GLOBALS['show_as_php'])) {
            $new_line = '\\n"<br />' . "\n"
                . '&nbsp;&nbsp;&nbsp;&nbsp;. "';
            $query_base = latin_htmlspecialchars(addslashes($sql_query));
        ...
        if (strlen($query_base) > $cfg['MaxCharactersInDisplayedSQL']) {
            ...
            $query_too_big = true;
            $shortened_query_base = nl2br(latin_htmlspecialchars(substr($sql_query, 0, $cfg['MaxCharactersInDisplayedSQL']) . '[...]'));
...
function PMA_profilingCheckbox($sql_query)
{
    if (PMA_profilingSupported()) {
        ...
        echo '<input type="hidden" name="sql_query" value="' . latin_htmlspecialchars($sql_query) . '" />' . "\n";
...
function PMA_generate_html_tab($tab, $url_params = array())
{
    ...
    if (!empty($tab['warning'])) {
        $tab['class'] .= ' warning';
        $tab['attr'] .= ' title="' . latin_htmlspecialchars($tab['warning']) . '"';
    }
    ...
    $new_form = true, $strip_img = false, $target = '')
{
    ...
    foreach ($tag_params as $par_name => $par_value) {
        // latin_htmlspecialchars() only on non javascript
        $par_value = substr($par_name, 0, 2) == 'on'
            ? $par_value
            : latin_htmlspecialchars($par_value);
    ...
    if ($url_length <= $GLOBALS['cfg']['LinkLengthLimit']) {
        ...
        foreach ($query_parts as $query_pair) {
            ...
            $ret .= '<input type="hidden" name="' . $subname_open . $eachvar
                . $subname_close . '" value="'
                . latin_htmlspecialchars(urldecode($eachval)) . '" />';
        } // end while
        ...
        if (stristr($message, '<img')) {
            if ($strip_img) {
                ...
                $ret .= '<input type="submit"' . $submit_name . ' '
                    . implode(' ', $tag_params_strings)
                    . ' value="' . latin_htmlspecialchars($message) . '" />';
            } else {
                $displayed_message = latin_htmlspecialchars(
    ...
    $image)
{
    if (false === $GLOBALS['cfg']['PropertiesIconic']) {
        echo ' <input type="submit" name="' . $button_name . '"'
                .' value="' . latin_htmlspecialchars($text) . '"'
                .' title="' . latin_htmlspecialchars($text) . '" />' . "\n";
        return;
    }
    ...
    if (PMA_USR_BROWSER_AGENT != 'IE') {
        echo '<button class="' . $button_class . '" type="submit"'
            .' name="' . $button_name . '" value="' . latin_htmlspecialchars($text) . '"'
            .' title="' . latin_htmlspecialchars($text) . '">' . "\n"
            . PMA_getIcon($image, $text)
            .'</button>' . "\n";
    } else {
        echo '<input type="image" name="' . $image_name . '" value="'
            . latin_htmlspecialchars($text) . '" title="' . latin_htmlspecialchars($text) . '" src="' . $GLOBALS['pmaThemeImage']
            . $image . '" />'
            . ($GLOBALS['cfg']['PropertiesIconic'] === 'both' ? '&nbsp;' . latin_htmlspecialchars($text) : '') . "\n";
    }
} // end function
...
function PMA_getDbLink($database = null)
{
    ...
    return '<a href="' . $GLOBALS['cfg']['DefaultTabDatabase'] . '?' . PMA_generate_common_url($database) . '"'
        .' title="' . sprintf($GLOBALS['strJumpToDB'], latin_htmlspecialchars($database)) . '">'
        .latin_htmlspecialchars($database) . '</a>';
}
 ...
 * @param   boolean $escape_label whether to use latin_htmlspecialchars() on label
...
function PMA_display_html_radio($html_field_name, $choices, $checked_choice = '', $line_break = true, $escape_label = true, $class='') {
    foreach ($choices as $choice_value => $choice_label) {
        ...
        echo '<input type="radio" name="' . $html_field_name . '" id="' . $html_field_id . '" value="' . latin_htmlspecialchars($choice_value) . '"';
        if ($choice_value == $checked_choice) {
            echo ' checked="checked"';
        }
        echo ' />' . "\n";
        echo '<label for="' . $html_field_id . '">' . ($escape_label ? latin_htmlspecialchars($choice_label)  : $choice_label) . '</label>';
        if ($line_break) {
            echo '<br />';
        }
        if (! empty($class)) {
...
{
    $result = '<select name="' . latin_htmlspecialchars($select_name) . '" id="' . latin_htmlspecialchars($id) . '">';
    foreach ($choices as $one_choice_value => $one_choice_label) {
        $result .= '<option value="' . latin_htmlspecialchars($one_choice_value) . '"';
        if ($one_choice_value == $active_choice) {
            $result .= ' selected="selected"';
        }
        $result .= '>' . latin_htmlspecialchars($one_choice_label) . '</option>';
    }
    $result .= '</select>';
    return $result;
}
