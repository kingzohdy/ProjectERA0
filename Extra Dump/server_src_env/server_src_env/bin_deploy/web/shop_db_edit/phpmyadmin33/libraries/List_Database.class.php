...
/*public*/ class PMA_List_Database extends PMA_List
{
    ...
    public function getHtmlListGrouped($selected = '', $offset, $count)
    {
        ...
        foreach ($this->getGroupedDetails($offset, $count) as $group => $dbs) {
            if (count($dbs) > 1) {
                $return .= '<li>' . latin_htmlspecialchars($group) . '<ul>' . "\n";
            ...
            foreach ($dbs as $db) {
                ...
                $return .= '><a';
                if (! empty($db['comment'])) {
                    $return .= ' title="' . latin_htmlspecialchars($db['comment']) . '"';
                }
                $return .= ' href="index.php?' . PMA_generate_common_url($db['name'])
                    . '" target="_parent">';
                if ($cut) {
                    $return .= latin_htmlspecialchars($db['disp_name_cut']);
                } else {
                    $return .= latin_htmlspecialchars($db['disp_name']);
                }
    ...
    public function getHtmlSelectGrouped($selected = '', $offset, $count)
    {
        ...
        foreach ($this->getGroupedDetails($offset, $count) as $group => $dbs) {
            if (count($dbs) > 1) {
                $return .= '<optgroup label="' . latin_htmlspecialchars($group)
                    . '">' . "\n";
            ...
            foreach ($dbs as $db) {
				$dbInfo = soap_getDBInfo($db['name']);
                $return .= '<option value="' . latin_htmlspecialchars($db['name']) . '"'
                    .' title="' . latin_htmlspecialchars($db['comment']) . '"';
                if ($db['name'] == $selected) {
                    $return .= ' selected="selected"';
                }
                $return .= '>' . latin_htmlspecialchars($cut ? $db['disp_name_cut'] : $db['disp_name']);
                if (! empty($db['num_tables'])) {
                    $return .= ' (' . $db['num_tables'] . ')';
                }
				$return .= $dbInfo;
