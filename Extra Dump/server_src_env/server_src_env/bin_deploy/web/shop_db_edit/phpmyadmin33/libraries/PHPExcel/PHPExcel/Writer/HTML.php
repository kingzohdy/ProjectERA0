...
class PHPExcel_Writer_HTML implements PHPExcel_Writer_IWriter {
	...
	public function generateHTMLHeader($pIncludeStyles = false) {
		...
		$html .= '    <title>' . latin_htmlspecialchars($this->_phpExcel->getProperties()->getTitle()) . '</title>' . "\r\n";
		if ($pIncludeStyles) {
		...
		foreach ($pSheet->getDrawingCollection() as $drawing) {
			if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
				if ($drawing->getCoordinates() == $coordinates) {
					...
					$filename = latin_htmlspecialchars($filename);
		...
		if (is_array($pValues)) {
			...
			foreach ($pValues as $cell) {
				...
				if ($cell instanceof PHPExcel_Cell) {
					...
					if ($cell->getValue() instanceof PHPExcel_RichText) {
						...
						foreach ($elements as $element) {
							...
							$cellData .= latin_htmlspecialchars($cellText);
				...
				if ($pSheet->hyperlinkExists($coordinate) && !$pSheet->getHyperlink($coordinate)->isInternal()) {
					$cellData = '<a href="' . latin_htmlspecialchars($pSheet->getHyperlink($coordinate)->getUrl()) . '" title="' . latin_htmlspecialchars($pSheet->getHyperlink($coordinate)->getTooltip()) . '">' . $cellData . '</a>';
				}
	...
	public function formatColor($pValue, $pFormat)
	{
		...
		// convert to PCDATA
		$value = latin_htmlspecialchars($pValue);
