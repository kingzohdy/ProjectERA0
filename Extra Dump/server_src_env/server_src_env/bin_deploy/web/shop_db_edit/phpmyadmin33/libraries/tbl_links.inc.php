...
$tab_reload['text']     = '·þÎñÆ÷¼ÓÔØ';//'ReloadServer';//$GLOBALS['strSearch'];
$tab_reload['icon']     = 's_sync.png';
$tab_reload['link']     = 'soap_reload.php';
$tabs[] =& $tab_reload;
...
if (! $tbl_is_view && ! (isset($db_is_information_schema) && $db_is_information_schema)) {
    ...
    $tabs['empty']['args']['zero_rows'] = sprintf($strTableHasBeenEmptied, latin_htmlspecialchars($table));
...
if (! (isset($db_is_information_schema) && $db_is_information_schema)) {
    ...
    $tabs['drop']['args']['zero_rows']  = sprintf(($tbl_is_view ? $strViewHasBeenDropped : $strTableHasBeenDropped), latin_htmlspecialchars($table));
...
{
    $msg = PMA_Message::notice('<a href="tbl_tracking.php?'.$url_query.'">'.sprintf($strTrackingActivated, latin_htmlspecialchars($GLOBALS["db"]), latin_htmlspecialchars($GLOBALS["table"])).'</a>');
    $msg->display();
}
