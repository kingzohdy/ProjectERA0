...
if (! defined('PMA_MINIMUM_COMMON')) {
    ...
    function PMA_SQP_throwError($message, $sql)
    {
        ...
        $SQP_errorString = '<p>'.$GLOBALS['strSQLParserUserError'] . '</p>' . "\n"
            . '<pre>' . "\n"
            . 'ERROR: ' . $message . "\n"
            . 'SQL: ' . latin_htmlspecialchars($sql) .  "\n"
            . '</pre>' . "\n";
    ...
    function PMA_SQP_bug($message, $sql)
    {
        ...
        $debugstr .= 'SQL: ' . latin_htmlspecialchars($sql);
                ...
                do {
                    ...
                    if ($pos < 0) {
                        $debugstr = $GLOBALS['strSQPBugUnclosedQuote'] . ' @ ' . $startquotepos. "\n"
                                  . 'STR: ' . latin_htmlspecialchars($quotetype);
                    ...
                    if ($is_digit && (!$is_hex_digit) && ($c2 == '.')) {
                        ...
                        if (!$is_float_digit) {
                            ...
                            $debugstr = $GLOBALS['strSQPBugInvalidIdentifer'] . ' @ ' . ($count1+1) . "\n"
                                      . 'STR: ' . latin_htmlspecialchars(PMA_substr($sql, $count1, $count2 - $count1));
                            PMA_SQP_throwError($debugstr, $sql);
                            return $sql_array;
                        }
                    }
                ...
                if ($l == 1) {
                     ...
                     */
                        ...
                        $debugstr =  $GLOBALS['strSQPBugUnknownPunctuation'] . ' @ ' . ($count1+1) . "\n"
                                  . 'STR: ' . latin_htmlspecialchars($punct_data);
                        PMA_SQP_throwError($debugstr, $sql);
                        return $sql_array;
                    }
                    PMA_SQP_arrayAdd($sql_array, 'punct', $punct_data, $arraysize);
...
//DEBUG echo "Loop2 <strong>"  . $arr[$i]['data'] . "</strong> (" . $arr[$i]['type'] . ")<br />";
    ...
    function PMA_SQP_formatHtml_colorize($arr)
    {
        ...
        return '<span class="' . $class . '">' . latin_htmlspecialchars($arr['data']) . '</span>';
        ...
        $number_of_tokens=-1)
    {
        ...
        if (!is_array($arr)) {
            return latin_htmlspecialchars($arr);
        }
        // first check for the SQL parser having hit an error
        if (PMA_SQP_isError()) {
            return latin_htmlspecialchars($arr['raw']);
        }
        // else do it properly
        switch ($mode) {
            case 'color':
...
*/
            ...
            if ($mode=='color') {
                ...
                $str .= latin_htmlspecialchars($arr[$i]['data']);
            } else {
                $str .= $arr[$i]['data'];
            }
            $str .= $after;
...
if (! defined('PMA_MINIMUM_COMMON')) {
    ...
    {
        $formatted_sql = latin_htmlspecialchars($arr['raw']);
