...
class PMA_Index
{
    ...
    static public function getView($table, $schema, $print_mode = false)
    {
        ...
        foreach ($indexes as $index) {
            ...
            $r .= '<th ' . $row_span . '>' . latin_htmlspecialchars($index->getName()) . '</th>';
            $r .= '<td ' . $row_span . '>' . latin_htmlspecialchars($index->getType()) . '</td>';
            ...
            foreach ($index->getColumns() as $column) {
                ...
                $r .= '<td>' . latin_htmlspecialchars($column->getName());
                if ($column->getSubPart()) {
                    $r .= ' (' . $column->getSubPart() . ')';
                }
                $r .= '</td>';
                $r .= '<td>' . latin_htmlspecialchars($column->getCardinality()) . '</td>';
                $r .= '<td>' . latin_htmlspecialchars($column->getCollation()) . '</td>';
                $r .= '<td>' . latin_htmlspecialchars($column->getNull()) . '</td>';

                if ($column->getSeqInIndex() == 1) {
                    $r .= '<td ' . $row_span . '>'
                        . latin_htmlspecialchars($index->getComments()) . '</td>';
                }
                $r .= '</tr>';
