...
**  $Id: cs_net.c,v 1.2 2011/01/06 06:16:18 kent Exp $
**  @author $Author: kent $
**  @date $Date: 2011/01/06 06:16:18 $
...
#pragma comment(lib, "ws2_32.lib")
	...
	#pragma comment(lib, "odr_d.lib")
	#else
	#pragma comment(lib, "onet.lib")
	#pragma comment(lib, "odr.lib")
	#endif
#endif
...
struct tagCSMidAttrList
{
	...
	unsigned short unVigorMax;	// 最大精力
...
int net_clt_han_create(char *pszMetaPath, char *pszConndIP,  unsigned short unPort, LPNETCLTHANDLE *ppNetCltHandle)
{
	...
	if (0 > tcltapi_open(szUri, 3000, &pstNetCltHandle->hApi))
	{
		free(pstNetCltHandle);
		return -1;
	}
...
#ifdef WIN32
	{
		int iSock;
		BOOL bNoDelay = TRUE;
		iSock = tcltapi_get_sock(pstNetCltHandle->hApi);
		setsockopt(iSock, IPPROTO_TCP, TCP_NODELAY, (const char*)&bNoDelay, sizeof(bNoDelay) );
	}
#endif	
...
int net_clt_acc_login(char *pszZoneConnIP, unsigned short unZoneConnPort, char *pszAccountLoginName, 
						NETCLTAUTHHANDLE *pstNetCltAuthHandle, int iSigLen, char *szSigInfo)
{
	char szUri[128];
	int iRet;
	CSPKG stPkg;
	CSACCLOGINREQ *pstAccLogin = &stPkg.Body.AccLoginReq;
	PDUHEAD stPDUHead;
	...
	if (0 > auth_send_msg(pstNetCltAuthHandle, &stPDUHead, (char *)&stPkg, sizeof(stPkg), 200))
	{
...
int net_clt_direct_conn_c(char *pstLibC, NetCltAuthInfo *pstNetCltAuthInfo, char *pszZoneConnIP, unsigned short unZoneConnPort, LPNETCLTAUTHHANDLE *ppNetCltAuthHandle)
{
	...
	iRet = net_clt_acc_login(pszZoneConnIP, unZoneConnPort, pstNetCltAuthInfo->AccountLoginName, 
							pstNetCltAuthHandle, pstNetCltAuthInfo->AuthJC.SigLen, pstNetCltAuthInfo->AuthJC.Sig);
	if (0 > iRet)
	{
		goto _error;
	}
...
int net_clt_direct_conn(char *pszMetaPath, NetCltAuthInfo *pstNetCltAuthInfo, char *pszZoneConnIP, unsigned short unZoneConnPort, LPNETCLTAUTHHANDLE *ppNetCltAuthHandle)
{
	...
	iRet = net_clt_direct_conn_c((char * )pstCSProtoLib, pstNetCltAuthInfo, pszZoneConnIP, unZoneConnPort, ppNetCltAuthHandle);
	if (0 > iRet)
	{
		free(pstCSProtoLib);
		return iRet;
								...
								LPNETCLTAUTHHANDLE *ppNetCltAuthHandle)
{
	...
	g_stNetCltAuthInfo.stPkg = stPkg;//add ouyang v20101231
	...
	iSocket	=	tnet_connect(szUri, 3000);
	if (-1 == iSocket)
	{
		iRet = AUTH_ERR_NET;
		goto _error;
	...
	iLen	=	tnet_recv(iSocket, pstNetCltAuthHandle->pszRecvBuff, pstNetCltAuthHandle->iRecvBuff, 3000);
	if (iLen < 0)
	{
		iRet = AUTH_ERR_NET;
		goto _error;
	...
	pszEnc = stPkg.Body.AuthSvr.Auth;
	iEnc = stPkg.Body.AuthSvr.AuthLen;
	...
	memcpy(&pstNetCltAuthHandle->iUin, szDecrypt+2+KEY_LEN, sizeof(int));
	pstNetCltAuthHandle->iUin = NTOHL(pstNetCltAuthHandle->iUin);

	iRet = net_clt_acc_login(pszZoneConnIP, unZoneConnPort, pszAccountLoginName, 
							pstNetCltAuthHandle, iLen - (2+KEY_LEN+sizeof(int)), szDecrypt+(2+KEY_LEN+sizeof(int)));
	if (0 > iRet)
	{
		goto _error;
	}
								...
								LPNETCLTAUTHHANDLE *ppNetCltAuthHandle)
{
	...
	iRet = net_clt_auth_han_create_c((char *)pstCSProtoLib, pszAuthConndIP, unAuthPort, pszZoneConnIP,
									unZoneConnPort, pszAccountLoginName, pszPasswd, ppNetCltAuthHandle);
	if (0 != iRet)
	{
		free(pstCSProtoLib);
		return iRet;
								...
								LPNETCLTAUTHHANDLE *ppNetCltAuthHandle)
{
	...
	memcpy(szMd5,g_stNetCltAuthInfo.szMd5,sizeof(szMd5));
       memcpy(RandK1,g_stNetCltAuthInfo.stPkg.Body.AuthClt.RandK1,sizeof(RandK1));
	stPkg =  g_stNetCltAuthInfo.stPkg;
       pstAuthClt->ActiveFlag = 1;
	...
	iRet = net_clt_acc_login(pszZoneConnIP, unZoneConnPort, g_stNetCltAuthInfo.AccountLoginName, 
							pstNetCltAuthHandle, iLen - (2+KEY_LEN+sizeof(int)), szDecrypt+(2+KEY_LEN+sizeof(int)));
	if (0 > iRet)
	{
		goto _error;
	...
	*ppNetCltAuthHandle = pstNetCltAuthHandle;
	...
	memset(&g_stNetCltAuthInfo.stPkg,0,sizeof(g_stNetCltAuthInfo.stPkg));
...
int net_clt_auth_sprintf(NETCLTAUTHHANDLE *pNetCltAuthHandle, CSPKG *pstPkg, char *pBuff, int iBuff)
{
	...
	return tdr_sprintf(pNetCltAuthHandle->pstRecvMeta, &stOut, &stHost, pNetCltAuthHandle->iPeerVersion);
}
...
int net_clt_sprintf(NETCLTHANDLE *pNetCltHandle, CSPKG *pstPkg, char *pBuff, int iBuff)
{
	...
	return tdr_sprintf(pNetCltHandle->pstNetHandle->pstCSProtoPkgMeta, &stOut, &stHost, pNetCltHandle->hApi->iPeerVersion);
}
