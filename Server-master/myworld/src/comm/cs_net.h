...
struct tagNetCltAuthInfo
{
	...
	int iCltVer;
	PASSWDCARDAUTHSVR stPasswdCardAuthSvr; //密保卡验证信息
	int iSocket;
...
*@param[in]   CertID    注册时玩家的身份证号码,其他时候请填NULL
*@param[in]   RealName  注册时玩家的真实姓名,其他时候请填NULL   
*@param[in]   eMail     注册时玩家的油箱地址,其他时候请填NULL 
*@param[in]   Mobile    注册时玩家的电话号码,其他时候请填NULL
*@param[in]	  iCltType	客户端类型CLT_TYPE
*@param[in]	  iAuthFlag	标志 AUTH_FLAG_QUICK

*@param[out]   pstPkg      创建的协议包
...
/**通过网页签名登录协议创建
*@param[in]   pszMetaPath     proto_cs.meta的路径
*@param[in]   pszAuthConndIP    auth的ip
*@param[in]   unAuthPort    auth的端口
*@param[in]   pszZoneConnIP    world_svr的ip
*@param[in]   unZoneConnPort  world_svr 的端口   
*@param[in]   pszWebSign     web端获得的签名字符串,保存在NetCltAuthInfo.szWebSign
*@param[in]   ppNetCltAuthHandle    NetCltAuth句柄指针
*@param[in]	  iCltType	客户端类型CLT_TYPE
*@param[in]	  iAuthFlag	标志 AUTH_FLAG_QUICK
...
/**登录认证并创建认证句柄
*@param[in]   pstAuthMeta    AuthInfo元数据
*@param[in]   pstRsa    rsa指针
...
*@param[in]   pszAccountLoginName    帐号登录名
*@param[in]   pszPasswd    帐号登录密码
*@param[in]   iCltVersion    客户端版本号
*@param[in]	MacAddr	玩家机器mac地址
*@param[in]   CertID    注册时玩家的身份证号码,其他时候请填NULL
*@param[in]   RealName  注册时玩家的真实姓名,其他时候请填NULL   
*@param[in]   eMail     注册时玩家的油箱地址,其他时候请填NULL 
*@param[in]   Mobile    注册时玩家的电话号码,其他时候请填NULL
*@param[in]	  iCltType	客户端类型CLT_TYPE
*@param[in]	  iAuthFlag	标志 AUTH_FLAG_QUICK
*@param[in]	  pszBirthday, web获取签名时的生日
*@param[in]	  iWebUin, web获取签名时的web uin
*@param[in]	  pszIP,web 获取签名时,客户端的ip
...
/**获取帐号在大区的角色列表
*@param[in]   pstLibC    
*@param[in]   pszAuthConnIP      大区auth_conn的接入ip
*@param[in]   unAuthPort      大区auth_conn的接入端口
*@param[in]   pszAccountLoginName    帐号登录名

*@param[out]   pstRes 如果返回值为0 则表示大区的角色列表

*@retval 0   succ
*@retval <0  fail  -1 系统错误
                   -2 网络错误
*/
...
/**在密保卡验证过程中,每30秒调用一次,保持与auth_svr的连接
*@param[in]   pszMetaPath    proto_cs.meta的路径
...
/**在web签名生成过程中,每30秒调用一次,保持与auth_svr的连接

*@param[in]   pstNetCltAuthHandle    auth客户端句柄
*@param[in]   pstPkg    CSPKG指针
*@param[in]   iSocket    已连接auth_svr的socket
...
/*转化一个unsigned int类型的版本号到字符串
   该函数返回一个静态BUFF  如果在一个函数中连续调用, 会覆盖前面的值
   如果要连续调用 请用字符串拷贝来保存结果
*/
