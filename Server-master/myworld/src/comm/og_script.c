...
extern void *get_script_location_byind(ScriptIndex *pstIndex, char *pszScriptName);
extern ScriptIndex *get_script_ind(char *sName);

static int g_debug_uin[8]={0};
static ScriptKernel *pkernel = NULL;    /* script interpreter kernel */

static int is_expr_true(int num1, unsigned char oper, int num2);
static int get_script_int(void);
static unsigned int get_script_cmd_code();
static int callstack_push();
static int callstack_pop();
static int fn_dummy_mac_decoder(unsigned int code);
static signed int get_proc_address(ScriptHeader *pheader, int proc_no);
...
int get_total_oldproc(signed int *oldproc_addr)
{
    int     i;
    int     total;
...
int load_script(IN char *fname, IN void *code_ptr, IN unsigned int size)
{
	...
	signed int oldproc_addr[MAX_PROC];
...
int exec_script(IN char *fname, IN ScriptIndex *pstIndex, IN int proc_no, OUT char *pszDebug, IN void *pstTestEnv)
{
    ...
    plocation = NULL;
    if (pstIndex && pstIndex->cScriptType > 0)	
    {
	  plocation = 	(unsigned char*)get_script_location_byind(pstIndex, fname);
    }

    if (NULL == plocation)
    {
          ScriptIndex *pstIndex1; 
		
	    pstIndex1 = get_script_ind(fname);
	    if (!pstIndex1)	
	    {
			if (pstIndex) 	pstIndex->cScriptType = 0;
			return -1;
	    ...
	    plocation = get_script_location_byind(pstIndex1, fname); 	
...
int check_script_proc(IN char *fname, IN ScriptIndex *pstIndex, IN int proc_no)
{
    ...
    if (NULL == plocation)
    {
	    ...
	    plocation = get_script_location_byind(pstIndex, fname); 	
...
char*  get_script_string(void)
{
    ...
    if (pkernel->cmdcode > MAX_SYS_CMD && pkernel->curr_hdr->version >= 0x00060002)
    {
	len = *p + (*(p+1))*0x100;
	p += 2;
    }
...
int set_script_call_var(IN int idx, IN int val)
{
	if (pkernel->callstack_top >= SCRIPT_CALLSTACK_SIZE)
	{
		return -1;
	}
	...
	if(idx >= 0 && idx < SCRIPT_VARIABLE_NUM)
       {
           pkernel->callstack[pkernel->callstack_top].variable[idx] = val;
           return 0;
       }
...
static int get_script_int(void)
{
    ...
    p = pkernel->param_ptr;
    int_value = p[0] + p[1] * 0x100 + p[2] * 0x10000 + p[3] * 0x1000000;
    p += 4;
    pkernel->param_ptr = p;
...
static unsigned int get_script_cmd_code()
{
    ...
    if (cmd_code > MAX_SYS_CMD && pkernel->curr_hdr->version >= 0x00060002)
    {
	 skip = *p + (*(p+1))*0x100;
	 p += 2;
...
int PlayerDebugScript(int lUin, int iType)
{
	int iPos;
