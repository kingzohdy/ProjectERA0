...
char sErrMsg[1024];
char g_szSpell[1024], g_szPinyin[1024];
...
void InitDirtySystem(char *pszPath)
{
	...
	*/
	...
	if( ( ret = tInitializeDirtyCheck( pszPath, 66 ) ) < 0 )
	{
		printf("tInitializeDirtyCheck() error, ret = %d\n", ret);
		//exit(0);
	}
}
...
int CheckDirty(const char* pszText, int iLevel)
{
	...
	*/
	...
	/*
	if( tMakeMapCode( pszText, g_szSpell, g_szPinyin, sizeof(g_szPinyin) ) != 0 )
	{
		return 0;
	}
	...
	if( g_szSpell[0] )
	{
		iRet = tDirtyCheck( 0, g_szSpell, &iLv);
		if( iRet > 0 )
		{
			return iRet;
		}
	...
	iRet = tDirtyCheck( 0, (char *)pszText, &iLv );
	if( iRet > 0 )
	{
		return iRet;
	}
	
	iRet = tDirtyCheckPinyin( 0, (char *)pszText, &iLv );
	if( iRet > 0 )
	{
		return iRet;
	}
