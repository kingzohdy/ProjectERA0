...
**  $Id: tconnapi.c,v 1.2 2012/06/19 07:40:51 kent Exp $
**  @author $Author: kent $
**  @date $Date: 2012/06/19 07:40:51 $
...
int tconnapi_create(int iProcID, unsigned long int* piHandle)
{
...
void tconnapi_free(unsigned long int* piHandle)
{
	if( !piHandle )
	{
		return;
...
int tconnapi_recv(unsigned long int iHandle, int* piSrc, char* pszBuff, int* piBuff, TFRAMEHEAD* pstHead)
{
	TCONNAPIHANDLE* pstHandle;
	size_t iSize;
...
int tconnapi_connect(unsigned long int iHandle, int iDst)
{
	TCONNAPIHANDLE* pstHandle;
...
int tconnapi_send(unsigned long int iHandle, int iDst, char* pszBuff, int iBuff, TFRAMEHEAD* pstHead)
{
	TCONNAPIHANDLE* pstHandle;
	int iRet;
	int iSrc;
