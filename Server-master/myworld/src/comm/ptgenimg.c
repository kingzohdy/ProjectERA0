...
int ptg_gen_char_image(char one_char, char* font, gdImagePtr* pImPtr, int* piw, int* pih, int fstd)
{
    ...
    *pImPtr	= gdImageCreate(*piw, *pih);
	...
	if( !fstd )
	{
		...
		ptg_fill_noise_pixel(*pImPtr, *piw, *pih, 0, black);
		ptg_fill_noise_line(*pImPtr, *piw, *pih, 0, black);	
	}
...
//    FILE* pngout;
	...
	iRand = rand() % 23;
	if (iRand == 13) //13.ttf 字体太难认了
	{
		iRand = 14;
	}

    sprintf(font, "../cfg/fonts/%d.ttf", iRand);

	memset(im_char, 0, sizeof(im_char));

    for (j = 0; j < codelen; j++)
    {
        ptg_gen_char_image(code[j], font, &im_char[j], &w[j], &h[j], 0);
        width += w[j];
    }
...
*/
    ...
    image_data = gdImagePngPtr(im_image, len);    
