...
typedef int (*fnScriptDecoder)(unsigned int code);
...
typedef struct
{
    ...
    signed int proc_addr[1];
	
} ScriptOldFileHeader;

typedef struct
{
    unsigned int code_offset;  /* code offset from start of location   */
    unsigned int code_size;    /* code size in bytes                   */
    unsigned int max_proc;     /* max proc in this script segment      */
    unsigned int version;	
...
typedef struct  
{
    ...
    int variable[SCRIPT_VARIABLE_NUM]; /* script variables */
} ScriptCallStack;
