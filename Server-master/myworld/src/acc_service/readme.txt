...
service端:
	...
	修改 ./cfg/acc_service.xml
		将
		<SpawnNum>5 </SpawnNum> 修改成
		<SpawnNum>8 </SpawnNum>
		在
		<SpawnNum>8 </SpawnNum>
		后面增加下面3行:
    ...
    执行 cd cfg;./stop_acc_service.sh; ./start_acc_service.sh

配置web端nginx,配置文件为nginx.conf
	acc_service 默认开启8个实例,每个实例侦听的端口自增1,前5个实例作为主服务,后3个为杂项服务,
	在http段中增加upstream
	 upstream accmisc {
...
service端:
	覆盖 ./bin/acc_service
	覆盖 ./cfg/start_acc_service.sh
	覆盖 ./cfg/stop_acc_service.sh
...
nginx修改:
	...
	将 location ~ ^/cgi-bin/acc_service$ 段中的
...
1.依赖spawn-fcgi-1.6.3,第一次部署需要安装,源码在 ../dep 目录下面
	第一次部署时需要新建数据库acc_service,sql文件在../dep 目录下面
...
4.配置web端nginx,配置文件为nginx.conf
	...
	其中
	fastcgi_pass   192.168.0.77:8092;
	的ip和端口修改成 acc_service.xml中的ip和端口
...
5.配置acc_service.wsdl 在文件结尾处找到以下内容
	...
	需要修改成
	http://nginx内网ip:nginx端口/cgi-bin/acc_service

	修改完成后将文件acc_service.wsdl复制到php相应目录cfg下面,覆盖同名文件

6.php使用acc_service示例
...
7. 负载均衡,配置nginx.conf
	acc_service 默认开启5个实例,每个实例侦听的端口自增1
	在http段中增加upstream
	 upstream acc {
                server  192.168.0.77:8092 weight=1;
                server  192.168.0.77:8093 weight=1;
