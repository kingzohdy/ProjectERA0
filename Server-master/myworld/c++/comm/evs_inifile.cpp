						...
						const char *szDefault, char *szValue, const int iValueLen)
{
	...
	if (!IsValid())
		return EVS_SYSRET::RET_INIFILE_SECTION;
...
int CIniFile::GetInt(const char *szSection, const char *szItem,
					 const int iDefault, unsigned int *piValue)
{
	int iValue(0);
	GetInt(szSection,szItem,iDefault,&iValue);
	*piValue = iValue;

	return EVS_SYSRET::RET_SUCC;
}
int CIniFile::GetInt(const char *szSection, const char *szItem,
					 const size_t iDefault, size_t *piValue)
{
	char szInt[32] =
	{ 0 };
