...
proc 1
	...
	if Rflag = 0 and Maxlv <= 79
		add_talk #b#b很高兴在这见到你勇者，由于魔物变得异常强大，我与底层的誓卫军已被切断联系。
		add_talk #b#b希望你能继续深入底层，找到那里的誓卫军了解情况。
	endif
	if Rflag = 0 and Maxlv > 79
		add_talk #b#b没想到这股来自底层的力量尽如此强大，这世界只有修斯佩恩拥有这股力量。
		add_talk #b#b如今我们只能祈祷，若是修斯佩恩曾经被封印，那我们一定有办法再次将它封印！
	endif
	add_talk #b#b
	strvar ret LV
	GET_ID_PLAYER_INFO 0 8 LV
	PLOY_GC_GET_AWARD ret 2
	if LV < 70
...
proc 200
	...
	if ret != -1 and ret != -2
		...
		if Con != -1
			...
			ADD_EXP 0 @Val Retm
			if Retm = 0
				PLOY_GC_SET_AWARDE 0 2
				
				add_sys_msg_start
				add_sys_msg $_player$在【勇闯失落之域】活动中成功突破第二层，领取了大量经验！
