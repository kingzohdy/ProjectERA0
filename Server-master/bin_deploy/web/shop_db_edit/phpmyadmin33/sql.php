...
if ($do_confirm) {
    ...
    echo '<fieldset class="confirmation">' . "\n"
        ...
        .'    <tt>' . latin_htmlspecialchars($stripped_sql_query) . '</tt>' . "\n"
        .'</fieldset>' . "\n"
...
if (isset($GLOBALS['show_as_php']) || !empty($GLOBALS['validatequery'])) {
    ...
    if ($error        = PMA_DBI_getError()) {
        if ($is_gotofile) {
            ...
            $active_page = $goto;
            $message = latin_htmlspecialchars(PMA_Message::rawError($error));
            /**
             * Go to target path.
             */
            require './' . PMA_securePath($goto);
...
if (0 == $num_rows || $is_affected) {
    if ($is_delete) {
        ...
        $message = PMA_Message::rawSuccess(latin_htmlspecialchars($zero_rows));
...
<input type="hidden" name="goto" value="<?php echo $goto; ?>" />
<input type="hidden" name="fields[dbase]" value="<?php echo latin_htmlspecialchars($db); ?>" />
