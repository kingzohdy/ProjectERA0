...
if ($GLOBALS['cfg']['LeftFrameLight'] && strlen($GLOBALS['db'])) {
	...
	$dbinfo = soap_getDBInfo($disp_name);
    ...
    <p><a class="item"
        ...
        title="<?php echo latin_htmlspecialchars($db_tooltip); ?>" >
    <?php
    if ($GLOBALS['text_dir'] === 'rtl') {
        echo ' <bdo dir="ltr">(' . $table_count . ')</bdo> ';
    }
    echo '<span class="navi_dbName">' . latin_htmlspecialchars($disp_name) . '</span>';
    if ($GLOBALS['text_dir'] === 'ltr') {
        echo ' <bdo dir="ltr">(' . $table_count .' '. latin_htmlspecialchars($dbinfo) . ')</bdo> ';
    }
    echo '</a></p>';
    if ($table_count) {
    ...
    foreach ($ext_dblist as $group => $db_group) {
        ...
        foreach ($db_group as $db) {
            ...
            if (count($GLOBALS['pma']->databases) > 1) {
                ...
                ?>
                <a class="item"
                    id="<?php echo latin_htmlspecialchars($db['name']); ?>"
                    href="index.php?<?php echo $common_url_query; ?>"
                    target="_parent"
                    title="<?php echo latin_htmlspecialchars($db['comment']); ?>"
                    onclick="
                    ...
                    echo latin_htmlspecialchars($db['disp_name']);
                    if ($GLOBALS['text_dir'] === 'ltr') {
                        echo ' <bdo dir="ltr">(' . $db['num_tables'] . ')</bdo> ';
                    }
                    ?>
    ...
    foreach ($tables as $group => $table) {
        ...
        if (isset($table['is' . $sep . 'group']) && $table['tab' . $sep . 'count'] > 1) {
            ...
            <a href="index.php?<?php echo $common_url_query; ?>"
                ...
                echo latin_htmlspecialchars(substr($group, 0, strlen($group) - strlen($sep)));
                if ($GLOBALS['text_dir'] === 'ltr') {
                    echo ' <bdo dir="ltr">(' . $table['tab' . $sep . 'count'] . ')</bdo> ';
                }
                ?>
            ...
            echo '<li>' . "\n";
            echo '<a title="'
                . latin_htmlspecialchars($link_title)
                . ': ' . latin_htmlspecialchars($table['Comment'])
                .' (' . PMA_formatNumber($table['Rows'], 0) . ' ' . $GLOBALS['strRows'] . ')"'
                .' id="quick_' . latin_htmlspecialchars($table_db . '.' . $table['Name']) . '"'
            ...
            echo ' id="icon_' . latin_htmlspecialchars($table_db . '.' . $table['Name']) . '"'
                .' width="10" height="10" alt="' . latin_htmlspecialchars($link_title) . '" /></a>' . "\n";
            ...
            echo '<a href="' . $href
            . '" title="' . latin_htmlspecialchars(PMA_getTitleForTarget($GLOBALS['cfg']['DefaultTabTable']) . ': ' . $table['Comment']
                .' (' . PMA_formatNumber($table['Rows'], 0) . ' ' . $GLOBALS['strRows']) . ')"'
                .' id="' . latin_htmlspecialchars($table_db . '.' . $table['Name']) . '">'
                // preserve spaces in table name
                . str_replace(' ', '&nbsp;', latin_htmlspecialchars($table['disp_name'])) . '</a>';
            echo '</li>' . "\n";
        }
    }
    echo '</ul>';
