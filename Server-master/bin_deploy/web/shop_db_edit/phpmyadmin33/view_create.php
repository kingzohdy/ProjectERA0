...
<fieldset>
    ...
    <tr>
        ...
        <td><select name="view[algorithm]" id="algorithm">
            ...
            foreach ($view_algorithm_options as $option) {
                echo '<option value="' . latin_htmlspecialchars($option) . '"';
                if ($view['algorithm'] === $option) {
                    echo 'selected="selected"';
                }
                echo '>' . latin_htmlspecialchars($option) . '</option>';
            }
            ?>
            </select>
        </td>
    ...
    <tr><td><?php echo $strViewName; ?></td>
        <td><input type="text" size="20" name="view[name]" onfocus="this.select()"
                value="<?php echo latin_htmlspecialchars($view['name']); ?>" />
        </td>
    </tr>
    ...
    <tr><td><?php echo $strColumnNames; ?></td>
        <td><input type="text" maxlength="100" size="50" name="view[column_names]"
                onfocus="this.select()"
                value="<?php echo latin_htmlspecialchars($view['column_names']); ?>" />
        </td>
    </tr>
    ...
    <tr><td>AS</td>
        <td>
            <textarea name="view[as]" rows="<?php echo $cfg['TextareaRows']; ?>"
                ...
                ><?php echo latin_htmlspecialchars($view['as']); ?></textarea>
        </td>
    </tr>
    <tr><td>WITH</td>
        <td>
            ...
            foreach ($view_with_options as $option) {
                ...
                echo ' id="view_with_' . str_replace(' ', '_', latin_htmlspecialchars($option)) . '"';
                echo ' value="' . latin_htmlspecialchars($option) . '" />';
                echo '<label for="view_with_' . str_replace(' ', '_', latin_htmlspecialchars($option)) . '">';
                echo latin_htmlspecialchars($option) . '</label>&nbsp;';
            }
            ?>
        </td>
    </tr>
