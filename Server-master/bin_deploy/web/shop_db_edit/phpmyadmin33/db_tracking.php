...
?>
    ...
    while ($one_result = PMA_DBI_fetch_array($all_tables_result)) {
        ...
        $tmp_link = 'tbl_tracking.php?' . $url_query . '&amp;table=' . latin_htmlspecialchars($version_data['table_name']);
        $delete_link = 'db_tracking.php?' . $url_query . '&amp;table=' . latin_htmlspecialchars($version_data['table_name']) . '&amp;delete_tracking=true&amp';
        ?>
        <tr class="<?php echo $style;?>">
            <td><?php echo latin_htmlspecialchars($version_data['db_name']);?></td>
            <td><?php echo latin_htmlspecialchars($version_data['table_name']);?></td>
...
<?php
    ...
    foreach ($my_tables as $key => $tablename) {
        if (PMA_Tracker::getVersion($GLOBALS['db'], $tablename) == -1) {
            $my_link = '<a href="tbl_tracking.php?' . $url_query . '&amp;table=' . latin_htmlspecialchars($tablename) .'">';
        ...
        ?>
            <tr class="<?php echo $style;?>">
            <td><?php echo latin_htmlspecialchars($tablename);?></td>
