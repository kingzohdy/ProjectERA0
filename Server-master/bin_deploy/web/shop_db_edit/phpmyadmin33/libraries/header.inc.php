...
<body>
    ...
    if (PMA_DISPLAY_HEADING && $GLOBALS['server'] > 0) {
        ...
        printf($item,
                ...
                latin_htmlspecialchars($server_info),
                $GLOBALS['strServer'],
                's_host.png');

        if (strlen($GLOBALS['db'])) {
			$dbInfo = soap_getDBInfo($GLOBALS['db']);

            echo $separator;
            printf($item,
                    $GLOBALS['cfg']['DefaultTabDatabase'],
                    PMA_generate_common_url($GLOBALS['db']),
                    latin_htmlspecialchars($GLOBALS['db']) . $dbInfo,
                    $GLOBALS['strDatabase'],
                    's_db.png');
            ...
            if (strlen($GLOBALS['table']) && ! (isset($_REQUEST['purge']))) {
                ...
                printf($item,
                        ...
                        str_replace(' ', '&nbsp;', latin_htmlspecialchars($GLOBALS['table'])),
                ...
                if (!empty($show_comment) && !isset($GLOBALS['avoid_show_comment'])) {
                    ...
                    echo '<span class="table_comment" id="span_table_comment">'
                        .'&quot;' . latin_htmlspecialchars($show_comment)
                        .'&quot;</span>' . "\n";
                } // end if
            } else {
                ...
                if ($cfgRelation['commwork']) {
                    ...
                    if (! empty($comment)) {
                        echo '<span class="table_comment"'
                           ...
                           . latin_htmlspecialchars($comment)
                           . '&quot;</span>' . "\n";
                    } // end if
                }
            }
