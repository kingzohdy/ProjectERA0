...
class PMA_StorageEngine_innodb extends PMA_StorageEngine
{
        ...
        $output = '<table class="data" id="table_innodb_bufferpool_usage">' . "\n"
            ...
            $output .= '    </tbody>' . "\n"
                ...
                . ($status['Innodb_buffer_pool_read_requests'] == 0
                    ? '---'
                    : latin_htmlspecialchars(PMA_formatNumber($status['Innodb_buffer_pool_reads'] * 100 / $status['Innodb_buffer_pool_read_requests'], 3, 2)) . ' %') . "\n"
                . '</td>' . "\n"
                . '        </tr>' . "\n"
                . '        <tr class="even">' . "\n"
                ...
                . ($status['Innodb_buffer_pool_write_requests'] == 0
                    ? '---'
                    : latin_htmlspecialchars(PMA_formatNumber($status['Innodb_buffer_pool_wait_free'] * 100 / $status['Innodb_buffer_pool_write_requests'], 3, 2)) . ' %') . "\n"
                . '</td>' . "\n"
                . '        </tr>' . "\n"
                . '    </tbody>' . "\n"
                . '</table>' . "\n";
    ...
    function getPageStatus()
    {
        return '<pre id="pre_innodb_status">' . "\n"
            . latin_htmlspecialchars(PMA_DBI_fetch_value('SHOW INNODB STATUS;', 0, 'Status')) . "\n"
            . '</pre>' . "\n";
    }
