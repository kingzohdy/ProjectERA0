...
if (!empty($submit_mult) && !empty($what)) {
    ...
    foreach ($selected AS $idx => $sval) {
        switch ($what) {
            case 'row_delete':
                $full_query .= latin_htmlspecialchars($sval)
                    . ';<br />';
                break;
            case 'drop_db':
                $full_query .= 'DROP DATABASE '
                    . PMA_backquote(latin_htmlspecialchars($sval))
                    . ';<br />';
                $reload = 1;
                break;
            ...
            case 'drop_tbl':
                ...
                if (!empty($views) && in_array($current, $views)) {
                    $full_query_views .= (empty($full_query_views) ? 'DROP VIEW ' : ', ')
                        . PMA_backquote(latin_htmlspecialchars($current));
                } else {
            ...
            case 'empty_tbl':
                $full_query .= 'TRUNCATE ';
                $full_query .= PMA_backquote(latin_htmlspecialchars($sval))
                            . ';<br />';
                break;
            ...
            case 'primary_fld':
                if ($full_query == '') {
                    $full_query .= 'ALTER TABLE '
                        . PMA_backquote(latin_htmlspecialchars($table))
                        . '<br />&nbsp;&nbsp;DROP PRIMARY KEY,'
                        . '<br />&nbsp;&nbsp; ADD PRIMARY KEY('
                        . '<br />&nbsp;&nbsp;&nbsp;&nbsp; '
                        . PMA_backquote(latin_htmlspecialchars($sval))
...
elseif ($mult_btn == $strYes) {
    ...
    for ($i = 0; $i < $selected_cnt; $i++) {
        switch ($query_type) {
            ...
            case 'empty_tbl':
                $a_query = 'TRUNCATE ';
                $a_query .= PMA_backquote(latin_htmlspecialchars($selected[$i]));
                $run_parts = TRUE;
                break;
