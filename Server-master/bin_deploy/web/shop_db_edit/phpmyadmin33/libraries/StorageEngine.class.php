...
class PMA_StorageEngine
{
      ...
      $selected = null, $offerUnavailableEngines = false)
    {
        ...
        foreach (PMA_StorageEngine::getStorageEngines() as $key => $details) {
            ...
            $output .= '    <option value="' . latin_htmlspecialchars($key). '"'
                . (empty($details['Comment'])
                    ? '' : ' title="' . latin_htmlspecialchars($details['Comment']) . '"')
                . (strtolower($key) == $selected || (empty($selected) && $details['Support'] == 'DEFAULT')
                    ? ' selected="selected"' : '') . '>' . "\n"
                . '        ' . latin_htmlspecialchars($details['Engine']) . "\n"
                . '    </option>' . "\n";
        }
        $output .= '</select>' . "\n";
        return $output;
    ...
    function getHtmlVariables()
    {
        ...
        foreach ($this->getVariablesStatus() as $details) {
            ...
            $ret .= '    </td>' . "\n"
                  . '    <th>' . latin_htmlspecialchars($details['title']) . '</th>' . "\n"
                  . '    <td class="value">';
            switch ($details['type']) {
                ...
                break;
                default:
                    $ret .= latin_htmlspecialchars($details['value']) . '   ';
            }
            $ret .= '</td>' . "\n"
                  . '</tr>' . "\n";
            $odd_row = !$odd_row;
    ...
    function getSupportInformationMessage()
    {
        ...
        return sprintf($message, latin_htmlspecialchars($this->title));
    }
