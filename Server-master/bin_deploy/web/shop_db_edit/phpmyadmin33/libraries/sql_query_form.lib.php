...
function PMA_sqlQueryForm($query = true, $display_tab = false, $delimiter = ';')
{
    ...
    echo '<input type="hidden" name="is_js_confirmed" value="0" />' . "\n"
        ...
        .latin_htmlspecialchars($goto) . '" />' . "\n"
        .'<input type="hidden" name="zero_rows" value="'
        . latin_htmlspecialchars($GLOBALS['strSuccess']) . '" />' . "\n"
        .'<input type="hidden" name="prev_sql_query" value="'
        . latin_htmlspecialchars($query) . '" />' . "\n";
...
function PMA_soapReloadForm($query = true, $display_tab = false, $delimiter = ';',$soap_ret=0)
{
    // check tab to display if inside querywindow
    if (! $display_tab) {
        $display_tab = 'full';
    ...
    if ($is_querywindow) {
        ...
        echo '<form method="post" action="soap_reload.php" ' . $enctype . ' id="sqlqueryform"'
            .' onsubmit="return checkSqlQuery(this)" name="sqlform">' . "\n";
    }
    ...
    // display querybox
    if ($display_tab === 'full' || $display_tab === 'sql') {
        PMA_soapForm($query, $is_querywindow, $delimiter,$soap_ret);
    }
...
function PMA_sqlQueryFormInsert($query = '', $is_querywindow = false, $delimiter = ';')
{
    ...
    if (! strlen($GLOBALS['db'])) {
        ...
        $strDBLink .= '>'
            . latin_htmlspecialchars($db) . '</a>';
        // else use
        // $strDBLink = latin_htmlspecialchars($db);
    ...
    echo '<div id="' . $sqlquerycontainer_id . '">' . "\n"
        ...
        .$auto_sel . $locking . '>' . latin_htmlspecialchars($query) . '</textarea>' . "\n";
    echo '</div>' . "\n";
    ...
    if (count($fields_list)) {
        ...
        foreach ($fields_list as $field) {
            echo '<option value="'
                .PMA_backquote(latin_htmlspecialchars($field['Field'])) . '"';
            if (isset($field['Field']) && strlen($field['Field']) && isset($field['Comment'])) {
                echo ' title="' . latin_htmlspecialchars($field['Comment']) . '"';
            }
            echo '>' . latin_htmlspecialchars($field['Field']) . '</option>' . "\n";
        }
        echo '</select>' . "\n"
...
function PMA_soapForm($query = '', $is_querywindow = false, $delimiter = ';',$soap_ret=0)
{
    ...
    echo '<a name="querybox"></a>' . "\n"
        ...
        .'<fieldset id="querybox">' . "\n";
    echo '<legend>' . $legend . '</legend>' . "\n";
//    echo '<div id="queryfieldscontainer" type="hidden">' . "\n";
//    echo '<div id="' . $sqlquerycontainer_id . '">' . "\n"
//        .'<textarea name="sql_query" id="sqlquery"'
//        .'  cols="' . $GLOBALS['cfg']['TextareaCols'] . '"'
//        .'  rows="' . $height . '"'
...
//		var_dump($a[1]);
	...
	if ($soap_ret != 0)
	{
		$RegionID = $GLOBALS['RegionID'];
		$WorldID = $GLOBALS['WorldID'];
	}
	else
	{
		$GLOBALS['RegionID'] = $RegionID;
		$GLOBALS['WorldID'] = $WorldID;
	}

//	$RegionID = $db;
    echo '<label for="id_sql_delimiter"> ' . '区ID'
        .'</label>' . "\n";
    echo '<input type="text" name="region_id" size="8" '
        .'value="' . $RegionID . '" '
        .'id="id_sql_delimiter" /> ' . "\n";

	echo '<label for="id_sql_delimiter"> ' . '服ID'
        .'</label>' . "\n";
    echo '<input type="text" name="world_id" size="8" '
        .'value="' . $WorldID . '" '
        .'id="id_sql_delimiter" /> ' . "\n";
	
	
	$dbInfo = soap_getDBInfo();
	echo '<label >' . $dbInfo 
			.'</label>' . "\n";
...
//        .'</label>' . "\n";
	...
	else
	{
		...
		echo '<label >' . '加载失败:' .$soap_ret .' '.$GLOBALS['soap_err_str']
			.'</label>' . "\n";
	}
...
function PMA_sqlQueryFormBookmark()
{
    ...
    foreach ($bookmark_list as $key => $value) {
        echo '<option value="' . latin_htmlspecialchars($key) . '">'
            .latin_htmlspecialchars($value) . '</option>' . "\n";
    }
