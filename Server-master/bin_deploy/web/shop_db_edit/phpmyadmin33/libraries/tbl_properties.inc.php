...
for ($i = 0; $i < $num_fields; $i++) {
    ...
    $content_cells[$i][$ci] = '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '"'
        ...
        . ' value="' . (isset($row['Field']) ? latin_htmlspecialchars($row['Field']) : '') . '"'
        . ' />';
    $ci++;
    ...
    foreach ($cfg['ColumnTypes'] as $col_goup => $column_type) {
        if (is_array($column_type)) {
            $content_cells[$i][$ci] .= '<optgroup label="' . latin_htmlspecialchars($col_goup) . '">';
    ...
    $content_cells[$i][$ci] = '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '"'
        ...
        . ' value="' . latin_htmlspecialchars($length_to_display) . '"'
        . ' class="textfield" />';
    $ci++;
    ...
    $content_cells[$i][$ci] .= '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '"'
        ...
        . ' value="' . (isset($row['DefaultValue']) ? latin_htmlspecialchars($row['DefaultValue']) : '') . '"'
        . ' class="textfield" />';
    $ci++;
    ...
    $content_cells[$i][$ci] = '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '"'
        ...
        . ' value="' . (isset($row['Field']) && is_array($comments_map) && isset($comments_map[$row['Field']]) ?  latin_htmlspecialchars($comments_map[$row['Field']]) : '') . '"'
        . ' class="textfield" />';
    $ci++;
    ...
    if ($cfgRelation['mimework'] && $cfg['BrowseMIME'] && $cfgRelation['commwork']) {
        ...
        if (is_array($available_mime['mimetype'])) {
            foreach ($available_mime['mimetype'] AS $mimekey => $mimetype) {
                ...
                $content_cells[$i][$ci] .= '    <option value="' . str_replace('/', '_', $mimetype) . '" ' . $checked . '>' . latin_htmlspecialchars($mimetype) . '</option>';
            }
        }
        ...
        if (is_array($available_mime['transformation'])) {
            foreach ($available_mime['transformation'] AS $mimekey => $transform) {
                ...
                $content_cells[$i][$ci] .= '<option value="' . $available_mime['transformation_file'][$mimekey] . '" ' . $checked . ' title="' . latin_htmlspecialchars($tooltip) . '">' . latin_htmlspecialchars($transform) . '</option>';
            }
        }
        ...
        $content_cells[$i][$ci] = '<input id="field_' . $i . '_' . ($ci - $ci_offset) . '"'
            ...
            . ' value="' . (isset($row['Field']) && isset($mime_map[$row['Field']]['transformation_options']) ?  latin_htmlspecialchars($mime_map[$row['Field']]['transformation_options']) : '') . '"'
            . ' />';
        //$ci++;
    }
} // end for
...
if ($action == 'tbl_create.php') {
    ...
    <tr><td><input type="text" name="comment" size="40" maxlength="80"
                value="<?php echo (isset($_REQUEST['comment']) ? latin_htmlspecialchars($_REQUEST['comment']) : ''); ?>"
                class="textfield" />
        </td>
        <td width="25">&nbsp;</td>
        <td>
    ...
    <tr>
        <td>
            <textarea name="partition_definition" id="partitiondefinition"
                ...
                dir="<?php echo $GLOBALS['text_dir'];?>"><?php echo (isset($_REQUEST['partition_definition']) ? latin_htmlspecialchars($_REQUEST['partition_definition']) : ''); ?></textarea>
        </td>
    </tr>
        <?php
    }
