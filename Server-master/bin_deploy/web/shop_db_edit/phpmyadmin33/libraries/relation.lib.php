...
function PMA__foreignDropdownBuild($foreign, $data, $mode)
{
    ...
    foreach ($foreign as $key => $value) {
        ...
        if (PMA_strlen($value) <= $GLOBALS['cfg']['LimitChars']) {
            ...
            $vtitle  = latin_htmlspecialchars($value);
            $value  = latin_htmlspecialchars(substr($value, 0, $GLOBALS['cfg']['LimitChars']) . '...');
        }

        $reloption = '                <option value="' . latin_htmlspecialchars($key) . '"';
        if ($vtitle != '') {
        ...
        if ($mode == 'content-id') {
            $reloptions[] = $reloption . '>' . $value . '&nbsp;-&nbsp;' . latin_htmlspecialchars($key) .  '</option>' . "\n";
        } else {
            $reloptions[] = $reloption . '>' . latin_htmlspecialchars($key) .  '&nbsp;-&nbsp;' . $value . '</option>' . "\n";
        }
    } // end foreach
