...
if ($data === TRUE && !$error && !$timeout_passed) {
    ...
    foreach ($lines AS $lkey => $line) {
        ...
        if (!empty($inf[1]) && strlen(trim($inf[1])) > 0) {
            ...
            PMA_importRunQuery($qry, $qry . '-- ' . latin_htmlspecialchars($tab) . '.' . latin_htmlspecialchars($inf[0]), true);
        } // end inf[1] exists
        if (!empty($inf[2]) && strlen(trim($inf[2])) > 0) {
            ...
            PMA_importRunQuery($qry, $qry . '-- ' . latin_htmlspecialchars($tab) . '.' . latin_htmlspecialchars($inf[0]) . '(' . latin_htmlspecialchars($inf[2]) . ')', true);
        } // end inf[2] exists
    } // End lines loop
} // End import
