...
function PMA_getHiddenFields($values, $pre = '')
{
    ...
    foreach ($values as $name => $value) {
        ...
        if (is_array($value)) {
            ...
            $fields .= '<input type="hidden" name="' . latin_htmlspecialchars($name)
 ...
 *
 * @param   string   - if first param is array: 'html' to use latin_htmlspecialchars()
 *                   on the resulting URL (for a normal URL displayed in HTML)
 *                   or something else to avoid using latin_htmlspecialchars() (for
...
function PMA_generate_common_url()
{
    ...
    if ($encode === 'html') {
        $query = latin_htmlspecialchars($query);
    }
