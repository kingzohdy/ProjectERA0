...
if (! empty($_sql_history)
 && ($querydisplay_tab == 'history' || $querydisplay_tab == 'full')) {
    ...
    foreach ($_sql_history as $query) {
        ...
        if (! empty($query['db'])) {
            echo '[';
            echo latin_htmlspecialchars(PMA_backquote($query['db']));
            if (! empty($query['table'])) {
                echo '.' . latin_htmlspecialchars(PMA_backquote($query['table']));
            }
            echo  '] ';
        }
        if (strlen($query['sqlquery']) > 120) {
            echo '<span title="' . latin_htmlspecialchars($query['sqlquery']) . '">';
            echo latin_htmlspecialchars(substr($query['sqlquery'], 0, 50)) . ' [...] ';
            echo latin_htmlspecialchars(substr($query['sqlquery'], -50));
            echo '</span>';
        } else {
            echo latin_htmlspecialchars($query['sqlquery']);
        }
        echo '</a>' . "\n";
        echo '</li>' . "\n";
    }
...
<form action="querywindow.php" method="post" name="hiddenqueryform"
    ...
    <input type="hidden" name="db" value="<?php echo latin_htmlspecialchars($db); ?>" />
    <input type="hidden" name="table" value="<?php echo latin_htmlspecialchars($table); ?>" />
