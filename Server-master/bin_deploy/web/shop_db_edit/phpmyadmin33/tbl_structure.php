...
if (! empty($submit_mult) && isset($_REQUEST['selected_fld'])) {
    ...
    if ($submit_mult == $strBrowse) {
        ...
        // what is this latin_htmlspecialchars() for??
        //$sql_query .= ' FROM ' . PMA_backquote(latin_htmlspecialchars($table));
...
while ($row = PMA_DBI_fetch_assoc($fields_rs)) {
    ...
    if ('set' == $extracted_fieldspec['type'] || 'enum' == $extracted_fieldspec['type']) {
        ...
        $type         = latin_htmlspecialchars($type);
    ...
    if (!isset($row['Default'])) {
        ...
        $row['Default'] = latin_htmlspecialchars($row['Default']);
    }

    $field_encoded = urlencode($row['Field']);
    $field_name    = latin_htmlspecialchars($row['Field']);

    // garvin: underline commented fields and display a hover-title (CSS only)

    $comment_style = '';
    if (isset($comments_map[$row['Field']])) {
        $field_name = '<span style="border-bottom: 1px dashed black;" title="' . latin_htmlspecialchars($comments_map[$row['Field']]) . '">' . $field_name . '</span>';
    }
...
<tr class="<?php echo $odd_row ? 'odd': 'even'; $odd_row = !$odd_row; ?>">
    <td align="center">
        <input type="checkbox" name="selected_fld[]" value="<?php echo latin_htmlspecialchars($row['Field']); ?>" id="checkbox_row_<?php echo $rownum; ?>" <?php echo $checked; ?> />
    </td>
    ...
    <td align="center">
        <a href="sql.php?<?php echo $url_query; ?>&amp;sql_query=<?php echo urlencode('ALTER TABLE ' . PMA_backquote($table) . ' DROP ' . PMA_backquote($row['Field'])); ?>&amp;cpurge=1&amp;purgekey=<?php echo urlencode($row['Field']); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strFieldHasBeenDropped, latin_htmlspecialchars($row['Field']))); ?>"
    ...
    <td align="center">
        ...
        if ($type == 'text' || $type == 'blob' || 'ARCHIVE' == $tbl_type) {
            ...
            echo "\n";
            ?>
        <a href="sql.php?<?php echo $url_query; ?>&amp;sql_query=<?php echo urlencode('ALTER TABLE ' . PMA_backquote($table) . ($primary ? ' DROP PRIMARY KEY,' : '') . ' ADD PRIMARY KEY(' . PMA_backquote($row['Field']) . ')'); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strAPrimaryKey, latin_htmlspecialchars($row['Field']))); ?>"
    ...
    <td align="center">
        ...
        if ($type == 'text' || $type == 'blob' || 'ARCHIVE' == $tbl_type) {
            ...
            echo "\n";
            ?>
        <a href="sql.php?<?php echo $url_query; ?>&amp;sql_query=<?php echo urlencode('ALTER TABLE ' . PMA_backquote($table) . ' ADD UNIQUE(' . PMA_backquote($row['Field']) . ')'); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strAnIndex, latin_htmlspecialchars($row['Field']))); ?>">
    ...
    <td align="center">
        ...
        if ($type == 'text' || $type == 'blob' || 'ARCHIVE' == $tbl_type) {
            ...
            echo "\n";
            ?>
        <a href="sql.php?<?php echo $url_query; ?>&amp;sql_query=<?php echo urlencode('ALTER TABLE ' . PMA_backquote($table) . ' ADD INDEX(' . PMA_backquote($row['Field']) . ')'); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strAnIndex, latin_htmlspecialchars($row['Field']))); ?>">
    ...
    <td align="center" nowrap="nowrap">
        <a href="sql.php?<?php echo $url_query; ?>&amp;sql_query=<?php echo urlencode('ALTER TABLE ' . PMA_backquote($table) . ' ADD FULLTEXT(' . PMA_backquote($row['Field']) . ')'); ?>&amp;zero_rows=<?php echo urlencode(sprintf($strAnIndex, latin_htmlspecialchars($row['Field']))); ?>">
...
<form method="post" action="tbl_addfield.php"
    ...
    foreach ($aryFields as $fieldname) {
        $fieldOptions .= '<option value="' . latin_htmlspecialchars($fieldname) . '">' . latin_htmlspecialchars($fieldname) . '</option>' . "\n";
    }
    unset($aryFields);
    $fieldOptions .= '</select>';
