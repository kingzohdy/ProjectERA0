...
{
    echo latin_htmlspecialchars($dropdown_question) . '&nbsp;&nbsp;';

    echo '<select name="' . latin_htmlspecialchars($select_name) . '">' . "\n";
    echo '<option value=""></option>' . "\n";

    foreach ($choices as $one_value => $one_label) {
        echo '<option value="' . latin_htmlspecialchars($one_value) . '"';
        if ($selected_value == $one_value) {
            echo ' selected="selected" ';
        }
        echo '>' . latin_htmlspecialchars($one_label) . '</option>' . "\n";
    }
    echo '</select>' . "\n";
}
...
if ($col_rs && PMA_DBI_num_rows($col_rs) > 0) {
    ...
    for ($i = 0; $i < $saved_row_cnt; $i++) {
        ...
        $myfield_html = latin_htmlspecialchars($myfield);
        ?>
    <tr class="<?php echo $odd_row ? 'odd' : 'even'; $odd_row = ! $odd_row; ?>">
        ...
        <td><select name="destination[<?php echo $myfield_md5; ?>]">
            ...
            if ($foreign_field && !$seen_key) {
                echo '                '
                    .'<option value="' . latin_htmlspecialchars($foreign_field) . '"'
                    .' selected="selected"'
    ...
    <fieldset>
        ...
        foreach ($save_row AS $row) {
            echo '            <option value="' . latin_htmlspecialchars($row['Field']) . '"';
            if (isset($disp) && $row['Field'] == $disp) {
                echo ' selected="selected"';
            }
            echo '>' . latin_htmlspecialchars($row['Field']) . '</option>'. "\n";
        } // end while
        ?>
        </select>
    </fieldset>
