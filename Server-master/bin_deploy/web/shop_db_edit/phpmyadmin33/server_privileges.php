    ...
    $priv_for_header, $name, $name_for_dfn, $name_for_current)
{
        ...
        foreach ($columns as $current_column => $current_column_privileges) {
            echo '            <option value="' . latin_htmlspecialchars($current_column) . '"';
            if ($row[$name_for_select] == 'Y' || $current_column_privileges[$name_for_current]) {
                echo ' selected="selected"';
            }
            echo '>' . latin_htmlspecialchars($current_column) . '</option>' . "\n";
        }
...
function PMA_displayLoginInformationFields($mode = 'new')
{
    ...
    echo '<fieldset id="fieldset_add_user_login">' . "\n"
       ...
       . '<input type="text" name="username" maxlength="'
        ...
        . (empty($GLOBALS['username'])
            ? ''
            : ' value="' . latin_htmlspecialchars(isset($GLOBALS['new_username'])
    ...
    echo '    onchange="if (this.value == \'any\') { hostname.value = \'%\'; } else if (this.value == \'localhost\') { hostname.value = \'localhost\'; } '
       . (empty($thishost) ? '' : 'else if (this.value == \'thishost\') { hostname.value = \'' . addslashes(latin_htmlspecialchars($thishost)) . '\'; } ')
    ...
    echo '        <option value="hosttable"'
       ...
       . '<input type="text" name="hostname" maxlength="'
        ...
        . latin_htmlspecialchars(isset($GLOBALS['hostname']) ? $GLOBALS['hostname'] : '')
...
if (!empty($update_privs)) {
    ...
    $message->addParam('\'' . latin_htmlspecialchars($username) . '\'@\'' . latin_htmlspecialchars($hostname) . '\'');
}
...
if (empty($_REQUEST['adduser']) && (! isset($checkprivs) || ! strlen($checkprivs))) {
    if (! isset($username)) {
                    ...
                    foreach ($user as $host) {
                        ...
                        echo '        <tr class="' . ($odd_row ? 'odd' : 'even') . '">' . "\n"
                           . '            <td><input type="checkbox" name="selected_usr[]" id="checkbox_sel_users_'
                            ...
                            . latin_htmlspecialchars($host['User'] . '&amp;#27;' . $host['Host'])
                            . '"'
                            . (empty($GLOBALS['checkall']) ?  '' : ' checked="checked"')
                            . ' /></td>' . "\n"
                           . '            <td><label for="checkbox_sel_users_' . $index_checkbox . '">' . (empty($host['User']) ? '<span style="color: #FF0000">' . $GLOBALS['strAny'] . '</span>' : latin_htmlspecialchars($host['User'])) . '</label></td>' . "\n"
                           . '            <td>' . latin_htmlspecialchars($host['Host']) . '</td>' . "\n";
                        echo '            <td>';
        ...
        if (isset($dbname)) {
            echo ' <i><a href="server_privileges.php?'
                . $GLOBALS['url_query'] . '&amp;username=' . latin_htmlspecialchars(urlencode($username))
                . '&amp;hostname=' . latin_htmlspecialchars(urlencode($hostname)) . '&amp;dbname=&amp;tablename=">\''
                . latin_htmlspecialchars($username) . '\'@\'' . latin_htmlspecialchars($hostname)
                . '\'</a></i>' . "\n";
            ...
            if (isset($tablename)) {
                echo ' <i><a href="server_privileges.php?' . $GLOBALS['url_query']
                    . '&amp;username=' . latin_htmlspecialchars(urlencode($username)) . '&amp;hostname=' . latin_htmlspecialchars(urlencode($hostname))
                    . '&amp;dbname=' . latin_htmlspecialchars($url_dbname) . '&amp;tablename=">' . latin_htmlspecialchars($dbname) . '</a></i>';
                echo ' - ' . $GLOBALS['strTable'] . ' <i>' . latin_htmlspecialchars($tablename) . '</i>';
            } else {
                echo ' <i>' . latin_htmlspecialchars($dbname) . '</i>';
            }

        } else {
            echo ' <i>\'' . latin_htmlspecialchars($username) . '\'@\'' . latin_htmlspecialchars($hostname)
                . '\'</i>' . "\n";
        ...
        if (! isset($tablename) && empty($dbname_is_wildcard)) {
            ...
            echo '<form action="server_privileges.php" method="post">' . "\n"
               . PMA_generate_common_hidden_inputs('', '')
               . '<input type="hidden" name="username" value="' . latin_htmlspecialchars($username) . '" />' . "\n"
               . '<input type="hidden" name="hostname" value="' . latin_htmlspecialchars($hostname) . '" />' . "\n"
               . '<fieldset>' . "\n"
            ...
            if (count($db_rights) < 1) {
                ...
                foreach ($db_rights as $row) {
                    ...
                    echo '<tr class="' . ($odd_row ? 'odd' : 'even') . '">' . "\n"
                       . '    <td>' . latin_htmlspecialchars((! isset($dbname)) ? $row['Db'] : $row['Table_name']) . '</td>' . "\n"
                       . '    <td><tt>' . "\n"
                    ...
                    echo '</td>' . "\n"
                       . '    <td>';
                    printf($link_edit, latin_htmlspecialchars(urlencode($username)),
                        urlencode(latin_htmlspecialchars($hostname)),
                        urlencode((! isset($dbname)) ? $row['Db'] : latin_htmlspecialchars($dbname)),
                        urlencode((! isset($dbname)) ? '' : $row['Table_name']));
                    echo '</td>' . "\n"
                       . '    <td>';
                    if (! empty($row['can_delete']) || isset($row['Table_name']) && strlen($row['Table_name'])) {
                        printf($link_revoke, latin_htmlspecialchars(urlencode($username)),
                            urlencode(latin_htmlspecialchars($hostname)),
                            urlencode((! isset($dbname)) ? $row['Db'] : latin_htmlspecialchars($dbname)),
                            urlencode((! isset($dbname)) ? '' : $row['Table_name']));
                    }
            ...
            if (! isset($dbname)) {
                ...
                if (!empty($pred_db_array)) {
                    ...
                    foreach ($pred_db_array as $current_db) {
                        ...
                        if (empty($found_rows) || ! in_array($current_db, $found_rows)) {
                            echo '        <option value="' . latin_htmlspecialchars($current_db) . '">'
                                . latin_htmlspecialchars($current_db) . '</option>' . "\n";
                        }
                    }
                    echo '    </select>' . "\n";
                }
                ...
                echo '    <input type="hidden" name="dbname" value="' . latin_htmlspecialchars($dbname) . '"/>' . "\n"
                ...
                if ($res = @PMA_DBI_try_query('SHOW TABLES FROM ' . PMA_backquote(PMA_unescape_mysql_wildcards($dbname)) . ';', null, PMA_DBI_QUERY_STORE)) {
                    ...
                    if (!empty($pred_tbl_array)) {
                        ...
                        foreach ($pred_tbl_array as $current_table) {
                            echo '        <option value="' . latin_htmlspecialchars($current_table) . '">' . latin_htmlspecialchars($current_table) . '</option>' . "\n";
                        }
                        echo '    </select>' . "\n";
                    }
                } else {
        ...
        if (isset($dbname) && empty($dbname_is_wildcard)) {
            echo '[ ' . $GLOBALS['strDatabase']
                ...
                . latin_htmlspecialchars($dbname) . ': ' . PMA_getTitleForTarget($GLOBALS['cfg']['DefaultTabDatabase']) . "</a> ]\n";

            if (isset($tablename)) {
                echo ' [ ' . $GLOBALS['strTable'] . ' <a href="'
                    . $GLOBALS['cfg']['DefaultTabTable'] . '?' . $GLOBALS['url_query']
                    . '&amp;db=' . $url_dbname . '&amp;table=' . latin_htmlspecialchars(urlencode($tablename))
                    . '&amp;reload=1">' . latin_htmlspecialchars($tablename) . ': '
        ...
        if (! isset($dbname) && ! $user_does_not_exists) {
            ...
            echo '<form action="server_privileges.php" method="post" onsubmit="return checkPassword(this);">' . "\n"
               . PMA_generate_common_hidden_inputs('', '')
               . '<input type="hidden" name="old_username" value="' . latin_htmlspecialchars($username) . '" />' . "\n"
               . '<input type="hidden" name="old_hostname" value="' . latin_htmlspecialchars($hostname) . '" />' . "\n"
    ...
    if ( !empty($dbname) ) {
        $choices['3'] = sprintf($GLOBALS['strCreateUserDatabasePrivileges'], latin_htmlspecialchars($dbname));
        $default_choice = 3;
        echo '<input type="hidden" name="dbname" value="' . latin_htmlspecialchars($dbname) . '" />' . "\n";
    }

    // 4th parameter set to true to add line breaks
    // 5th parameter set to false to avoid latin_htmlspecialchars() escaping in the label
    ...
    echo '<table id="tablespecificuserrights" class="data">' . "\n"
       ...
       . '    ' . sprintf($GLOBALS['strUsersHavingAccessToDb'], '<a href="' . $GLOBALS['cfg']['DefaultTabDatabase'] . '?' . PMA_generate_common_url($checkprivs) . '">' .  latin_htmlspecialchars($checkprivs) . '</a>') . "\n"
       . '</caption>' . "\n"
       . '<thead>' . "\n"
    ...
    if ($found) {
        while (TRUE) {
            ...
            echo '>' . (empty($current_user) ? '<span style="color: #FF0000">' . $GLOBALS['strAny'] . '</span>' : latin_htmlspecialchars($current_user)) . "\n"
               . '        </td>' . "\n"
               . '        <td';
            if (count($current_privileges) > 1) {
                echo ' rowspan="' . count($current_privileges) . '"';
            }
            echo '>' . latin_htmlspecialchars($current_host) . '</td>' . "\n";
            foreach ($current_privileges as $current) {
                ...
                if (!isset($current['Db']) || $current['Db'] == '*') {
                    ...
                    echo $GLOBALS['strWildcard'], ': <tt>' . latin_htmlspecialchars($current['Db']) . '</tt>';
                }
                echo "\n"
                   . '        </td>' . "\n"
                   . '        <td>' . "\n"
    ...
    echo '<fieldset id="fieldset_add_user">' . "\n"
       . '    <a href="server_privileges.php?' . $GLOBALS['url_query'] . '&amp;adduser=1&amp;dbname=' . latin_htmlspecialchars($checkprivs) .'">' . "\n"
       . PMA_getIcon('b_usradd.png')
       . '        ' . $GLOBALS['strAddUser'] . '</a>' . "\n"
       . '</fieldset>' . "\n";
