...
if (isset($_REQUEST['submit_sql']) && ! empty($sql_query)) {
    $goto      = 'db_sql.php';
    $zero_rows = latin_htmlspecialchars($GLOBALS['strSuccess']);
    require './sql.php';
    exit;
} else {
    $sub_part  = '_qbe';
...
function showColumnSelectCell($columns, $column_number, $selected = '')
{
    ...
    foreach ($columns as $column) {
        ...
        echo '<option value="' . latin_htmlspecialchars($column) . '"' . $sel . '>'
            . str_replace(' ', '&nbsp;', latin_htmlspecialchars($column)) . '</option>' . "\n";
    }
    ?>
        </select>
    </td>
...
for ($x = 0; $x < $col; $x++) {
    ...
    if ((empty($prev_criteria) || !isset($prev_criteria[$x]))
        || $prev_criteria[$x] != latin_htmlspecialchars($stripped_Criteria)) {
    ...
    ?>
    <td align="center">
        <input type="hidden" name="prev_criteria[<?php echo $z; ?>]" value="<?php echo latin_htmlspecialchars($curCriteria[$z]); ?>" />
        <input type="text" name="criteria[<?php echo $z; ?>]" value="<?php echo latin_htmlspecialchars($stripped_Criteria); ?>" class="textfield" style="width: <?php echo $realwidth; ?>" size="20" />
    </td>
    <?php
    $z++;
    echo "\n";
...
<tr class="<?php echo $odd_row ? 'odd' : 'even'; ?> noclick">
    ...
    <td align="center">
        ...
        ?>
    <td align="center">
        <textarea cols="20" rows="2" style="width: <?php echo $realwidth; ?>" name="Or<?php echo $w . '[' . $z . ']'; ?>" dir="<?php echo $text_dir; ?>"><?php echo latin_htmlspecialchars($stripped_or); ?></textarea>
    </td>
        <?php
        if (!empty(${$or}) && isset(${$or}[$x])) {
            ${'cur' . $or}[$z] = ${$or}[$x];
...
foreach ($tbl_names as $key => $val) {
    $strTableListOptions .= '                        ';
    $strTableListOptions .= '<option value="' . latin_htmlspecialchars($key) . '"' . $val . '>'
        . str_replace(' ', '&nbsp;', latin_htmlspecialchars($key)) . '</option>' . "\n";
    $numTableListOptions++;
}
?>
...
if (!empty($qry_select)) {
    echo  'SELECT ' . latin_htmlspecialchars($qry_select) . "\n";
}
...
if (!empty($qry_from)) {
    echo 'FROM ' . latin_htmlspecialchars($qry_from) . "\n";
}
...
if (!empty($qry_where) && $qry_where != '()') {
    echo 'WHERE ' . latin_htmlspecialchars($qry_where) . "\n";
} // end if
...
if (!empty($qry_orderby)) {
    echo 'ORDER BY ' . latin_htmlspecialchars($qry_orderby) . "\n";
}
?>
        </textarea>
        </fieldset>
