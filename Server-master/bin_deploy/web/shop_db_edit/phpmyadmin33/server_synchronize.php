        ...
        if (($src_db_selected != 1) || ($trg_db_selected != 1)) {
            ...
            */
            ...
            if ($src_db_selected != 1) {
                echo sprintf($GLOBALS['strDatabaseNotExisting'], latin_htmlspecialchars($src_db));
            }
            if ($trg_db_selected != 1) {
                echo sprintf($GLOBALS['strDatabaseNotExisting'], latin_htmlspecialchars($trg_db));
            }
            echo '</div>';    
            unset($_REQUEST['submit_connect']);
            ...
            */
            for($i = 0; $i < count($matching_tables); $i++) {
                ...
                */
                ...
                echo '<td>' . latin_htmlspecialchars($matching_tables[$i]) . '</td>
                <td align="center">';
                /**
                ...
                */
                if (($num_alter_cols > 0) || ($num_insert_cols > 0) || ($num_remove_cols > 0) || ($num_add_index > 0) || ($num_remove_index > 0)) {
                   ...
                   . ', this ,' . "'" . latin_htmlspecialchars($matching_tables[$i]) . "'" . ')"/>';
                }
                /**
                ...
                if (isset($update_array[$i]) || isset($insert_array[$i])) {
                    if (isset($update_array[$i][0][$matching_tables_keys[$i][0]]) || isset($insert_array[$i][0][$matching_tables_keys[$i][0]])) {
                        ...
                        alt="' . $GLOBALS['strClickToSelect'] . '" onmouseover="change_Image(this);" onmouseout="change_Image(this);"
                         ...
                         "','" . null . "','" . null . "','" . null . "'" . ', this ,' . "'" . latin_htmlspecialchars($matching_tables[$i]) . "'" . ')" />';    
                    }    
                }
                echo '</td>
                </tr>';
            ...
            */
            for ($j = 0; $j < count($source_tables_uncommon); $j++) {
                $odd_row = PMA_syncDisplayBeginTableRow($odd_row);
                echo '<td> + ' . latin_htmlspecialchars($source_tables_uncommon[$j]) . '</td> ';
                ...
                . "'" . latin_htmlspecialchars($source_tables_uncommon[$j]) . "'" . ')"/>';
                ...
                if ($row_count[$j] > 0)
                {
                    ...
                    "','" . null . "','" . null . "'" . ', this ,' . "'" . latin_htmlspecialchars($source_tables_uncommon[$j]) . "'" . ')" />';
                } 
                echo '</td>
                </tr>';
            }
            ...
            */
            ...
            foreach ($source_tables_uncommon as $tbl_nc_name) {
                $odd_row = PMA_syncDisplayBeginTableRow($odd_row);
                echo '<td height="32">' . latin_htmlspecialchars($tbl_nc_name) . ' (' . $GLOBALS['strNotPresent'] . ')</td>
                </tr>';
            }
            foreach ($target_tables_uncommon as $tbl_nc_name) {
                $odd_row = PMA_syncDisplayBeginTableRow($odd_row);
                echo '<td> - ' . latin_htmlspecialchars($tbl_nc_name) . '</td>';
                echo '</tr>';
            }
            echo '</table>';
            echo '</div>';
            ...
            */
            ...
            echo '<input type="button" name="apply_changes" value="' . $GLOBALS['strApplyChanges']
             . '" onclick ="ApplySelectedChanges(' . "'" . latin_htmlspecialchars($_SESSION['token']) . "'" . ')" />';
...
if (isset($_REQUEST['Table_ids'])) {
    ...
    */
    ...
    for($i = 0; $i < count($matching_tables); $i++) {   
        $odd_row = PMA_syncDisplayBeginTableRow($odd_row);
        echo '<td align="center">' . latin_htmlspecialchars($matching_tables[$i]) . '</td>
        <td align="center">';
    ...
    */
    ...
    for ($j = 0; $j < count($source_tables_uncommon); $j++) {
        $odd_row = PMA_syncDisplayBeginTableRow($odd_row);
        echo '<td align="center"> + ' . latin_htmlspecialchars($source_tables_uncommon[$j]) . '</td>
        <td align="center">';
        /**
        ...
        */
        if (!(in_array($j, $uncommon_table_structure_diff))) {
            if (isset($uncommon_tables[$j])) {
                echo '<img class="icon" src="' . $pmaThemeImage  . 'new_struct.jpg" width="29"  height="29" 
                alt="' . $GLOBALS['strClickToSelect'] . '" onmouseover="change_Image(this);" onmouseout="change_Image(this);"
                onclick="showDetails(' . "'US" . $j . "','" . null . "','" . null . "','" . null . "','" . null . "','" . null . "'" . ', this ,' . "'" . latin_htmlspecialchars($source_tables_uncommon[$j]) . "'" . ')"/>' .' ';    
            }            
        } else {
        ...
        */
        if (!(in_array($j, $uncommon_table_data_diff))) {
            if (isset($row_count[$j]) && ($row_count > 0)) {
                ...
                . null . "','" . null . "','" . null . "'" . ', this ,' . "'". latin_htmlspecialchars($source_tables_uncommon[$j]) . "'" . ')" />';
            }    
        } else {
            unset($row_count[$j]);
        }
    ...
    */
    ...
    foreach ($source_tables_uncommon as $tbl_nc_name) {
        ...
        if (in_array($tbl_nc_name, $uncommon_tables)) {
            echo '<td>' . latin_htmlspecialchars($tbl_nc_name) . ' (' .  $GLOBALS['strNotPresent'] . ')</td>';
        } else {
            echo '<td>' . latin_htmlspecialchars($tbl_nc_name) . '</td>';    
        }
        echo '
  ...
  */
	...
	if (count($databases) == 0) {
		...
		foreach ($databases as $db) {
            echo '		<option>' . latin_htmlspecialchars($db['SCHEMA_NAME']) . '</option>';
		}  
        echo '</select>';
	}
	echo '</td> </tr>
