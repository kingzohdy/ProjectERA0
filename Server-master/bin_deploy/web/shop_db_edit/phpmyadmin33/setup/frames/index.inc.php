...
if (!$is_https) {
    ...
    if (!empty($_SERVER['REQUEST_URI']) && !empty($_SERVER['HTTP_HOST'])) {
        $text .= ' ' . PMA_lang('InsecureConnectionMsg2',
            'https://' . latin_htmlspecialchars($_SERVER['HTTP_HOST'] . $_SERVER['REQUEST_URI']));
    }
    messages_set('warning', 'no_https', 'InsecureConnection', $text);
}
?>

<form id="select_lang" method="post" action="<?php echo latin_htmlspecialchars($_SERVER['REQUEST_URI']) ?>">
...
<tr>
    <td><?php echo $id ?></td>
    <td><?php echo latin_htmlspecialchars($cf->getServerName($id)) ?></td>
    <td><?php echo latin_htmlspecialchars($cf->getValue("Servers/$id/auth_type")) ?></td>
    <td><?php echo latin_htmlspecialchars($cf->getServerDSN($id)) ?></td>
