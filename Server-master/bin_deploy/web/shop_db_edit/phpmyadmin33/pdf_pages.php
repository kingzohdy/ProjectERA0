...
<form method="get" action="pdf_pages.php" name="selpage">
    ...
    <select name="chpage" onchange="this.form.submit()">
        ...
        while ($curr_page = PMA_DBI_fetch_assoc($page_rs)) {
            ...
            echo '>' . $curr_page['page_nr'] . ': ' . latin_htmlspecialchars($curr_page['page_descr']) . '</option>';
        } // end while
        echo "\n";
        ?>
    </select>
...
foreach ($array_sh_page AS $key => $temp_sh_page) {
    ...
    if (isset($with_field_names)) {
        while ($row = PMA_DBI_fetch_assoc($fields_rs)) {
            echo '<br />' . latin_htmlspecialchars($row['Field']) . "\n";
        }
    }
    echo '</div>' . "\n";
...
<form method="post" action="pdf_pages.php" name="edcoord">
    <?php echo PMA_generate_common_hidden_inputs($db, $table); ?>
    <input type="hidden" name="chpage" value="<?php echo latin_htmlspecialchars($chpage); ?>" />
    ...
    <tr>
        ...
        foreach ($array_sh_page AS $dummy_sh_page => $sh_page) {
            ...
            foreach ($selectboxall AS $key => $value) {
                echo "\n" . '                <option value="' . latin_htmlspecialchars($value) . '"';
                if ($value == $sh_page['table_name']) {
                    echo ' selected="selected"';
                    $tabExist[$_mtab] = TRUE;
                }
                echo '>' . latin_htmlspecialchars($value) . '</option>';
            } // end while
            echo "\n" . '            </select>'
                 . "\n" . '        </td>';
            echo "\n" . '        <td>'
        ...
        echo "\n" . '        <td>'
             . "\n" . '            <select name="c_table_' . $i . '[name]">';
        foreach ($selectboxall AS $key => $value) {
            echo "\n" . '                <option value="' . latin_htmlspecialchars($value) . '">' . latin_htmlspecialchars($value) . '</option>';
        }
        echo "\n" . '            </select>'
             . "\n" . '        </td>';
        echo "\n" . '        <td>'
    ...
    if (!empty($tabExist) && is_array($tabExist)) {
        foreach ($tabExist AS $key => $value) {
            if (!$value) {
                $_strtrans  .= '<input type="hidden" name="delrow[]" value="' . latin_htmlspecialchars($key) . '" />' . "\n";
                $_strname   .= '<li>' . latin_htmlspecialchars($key) . '</li>' . "\n";
                $shoot       = TRUE;
            }
        }
        if ($shoot) {
            echo '<form action="pdf_pages.php" method="post">' . "\n"
               ...
               . '<input type="hidden" name="chpage" value="' . latin_htmlspecialchars($chpage) . '" />' . "\n"
               . $strDelOld
               . '<ul>' . "\n"
               . $_strname
               . '</ul>' . "\n"
...
<form method="post" action="pdf_schema.php" name="pdfoptions">
    ...
    <input type="hidden" name="pdf_page_number" value="<?php echo latin_htmlspecialchars($chpage); ?>" />
