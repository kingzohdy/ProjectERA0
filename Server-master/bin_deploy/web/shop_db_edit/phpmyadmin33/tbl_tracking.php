...
if (isset($_REQUEST['report_export']) && $_REQUEST['export_type'] == 'sqldumpfile') {
    ...
    $dump = "# " . sprintf($strTrackingReportForTable, latin_htmlspecialchars($_REQUEST['table'])) . "\n" .
            "# " . date('Y-m-d H:i:s') . "\n";
    foreach($entries as $entry) {
        $dump .= $entry['statement'];
    }
    $filename = 'log_' . latin_htmlspecialchars($_REQUEST['table']) . '.sql';
...
if (isset($_REQUEST['submit_create_version'])) {
    ...
    if (PMA_Tracker::createVersion($GLOBALS['db'], $GLOBALS['table'], $_REQUEST['version'], $tracking_set )) {
        $msg = PMA_Message::success(sprintf($strTrackingVersionCreated, $_REQUEST['version'], latin_htmlspecialchars($GLOBALS['db']), latin_htmlspecialchars($GLOBALS['table'])));
        $msg->display();
    }
}
...
if (isset($_REQUEST['submit_deactivate_now'])) {
    if (PMA_Tracker::deactivateTracking($GLOBALS['db'], $GLOBALS['table'], $_REQUEST['version'])) {
        $msg = PMA_Message::success(sprintf($strTrackingVersionDeactivated, latin_htmlspecialchars($GLOBALS['db']), latin_htmlspecialchars($GLOBALS['table']), $_REQUEST['version']));
        $msg->display();
    }
}
...
if (isset($_REQUEST['submit_activate_now'])) {
    if (PMA_Tracker::activateTracking($GLOBALS['db'], $GLOBALS['table'], $_REQUEST['version'])) {
        $msg = PMA_Message::success(sprintf($strTrackingVersionActivated, latin_htmlspecialchars($GLOBALS['db']), latin_htmlspecialchars($GLOBALS['table']), $_REQUEST['version']));
        $msg->display();
    }
}
...
?>
        <tr class="<?php echo $style; ?>">
            <?php
            if ($field['Key'] == 'PRI') {
                echo '<td><b><u>' . latin_htmlspecialchars($field['Field']) . '</u></b></td>' . "\n";
            } else {
                echo '<td><b>' . latin_htmlspecialchars($field['Field']) . '</b></td>' . "\n";
            }
            ?>
            <td><?php echo latin_htmlspecialchars($field['Type']);?></td>
            <td><?php echo latin_htmlspecialchars($field['Collation']);?></td>
            <td><?php echo latin_htmlspecialchars($field['Null']);?></td>
            <td><?php echo latin_htmlspecialchars($field['Default']);?></td>
            <td><?php echo latin_htmlspecialchars($field['Extra']);?></td>
            <td><?php echo latin_htmlspecialchars($field['Comment']);?></td>
        </tr>
<?php
            if ($style == 'even') {
                $style = 'odd'; 
...
?>
            <tr class="<?php echo $style; ?>">
                <td><b><?php echo latin_htmlspecialchars($index['Key_name']);?></b></td>
                <td><?php echo latin_htmlspecialchars($index['Index_type']);?></td>
                <td><?php echo $str_unique;?></td>
                <td><?php echo $str_packed;?></td>
                <td><?php echo latin_htmlspecialchars($index['Column_name']);?></td>
                <td><?php echo latin_htmlspecialchars($index['Cardinality']);?></td>
                <td><?php echo latin_htmlspecialchars($index['Collation']);?></td>
                <td><?php echo latin_htmlspecialchars($index['Null']);?></td>
                <td><?php echo latin_htmlspecialchars($index['Comment']);?></td>
            </tr>
<?php
            if ($style == 'even') {
                $style = 'odd'; 
...
if (isset($_REQUEST['report']) || isset($_REQUEST['report_export'])) {
    ...
    <small><?php echo $strTrackingStatements . ' ' . latin_htmlspecialchars($data['tracking']); ?></small><br/>
    <br/>
    ...
    $str1 = '<select name="logtype">' .
            ...
            '</select>';
    $str2 = '<input type="text" name="date_from" value="' . latin_htmlspecialchars($_REQUEST['date_from']) . '" size="19" />';
    $str3 = '<input type="text" name="date_to" value="' . latin_htmlspecialchars($_REQUEST['date_to']) . '" size="19" />';
    $str4 = '<input type="text" name="users" value="' . latin_htmlspecialchars($_REQUEST['users']) . '" />';
    ...
    ?>
        ...
        ?>
                <tr class="<?php echo $style; ?>">
                    ...
                    <td><small><?php echo latin_htmlspecialchars($entry['date']);?></small></td>
                    <td><small><?php echo latin_htmlspecialchars($entry['username']); ?></small></td>
    ...
    <form method="post" action="tbl_tracking.php<?php echo PMA_generate_common_url($url_params + array('report' => 'true', 'version' => $_REQUEST['version'])); ?>">
    <input type="hidden" name="logtype" value="<?php echo latin_htmlspecialchars($_REQUEST['logtype']);?>" />
    <input type="hidden" name="date_from" value="<?php echo latin_htmlspecialchars($_REQUEST['date_from']);?>" />
    <input type="hidden" name="date_to" value="<?php echo latin_htmlspecialchars($_REQUEST['date_to']);?>" />
    <input type="hidden" name="users" value="<?php echo latin_htmlspecialchars($_REQUEST['users']);?>" />
    <?php
...
?>
    ...
    while ($entries = PMA_DBI_fetch_array($sql_result)) {
        ...
        echo '<option value="' . latin_htmlspecialchars($entries['table_name']) . '"' . $s . '>' . latin_htmlspecialchars($entries['db_name']) . ' . ' . latin_htmlspecialchars($entries['table_name']) . $status . '</option>' . "\n";
    }
    ?>
    </select>
...
?>
    ...
    ?>
        <tr class="<?php echo $style;?>">
            <td><?php echo latin_htmlspecialchars($version['db_name']);?></td>
            <td><?php echo latin_htmlspecialchars($version['table_name']);?></td>
            <td><?php echo latin_htmlspecialchars($version['version']);?></td>
            <td><?php echo latin_htmlspecialchars($version['date_created']);?></td>
            <td><?php echo latin_htmlspecialchars($version['date_updated']);?></td>
    ...
    <?php if ($tracking_active == true) {?>
        ...
        <form method="post" action="tbl_tracking.php?<?php echo $url_query; ?>">
        <fieldset>
            <legend><?php printf($strTrackingDeactivateTrackingFor, latin_htmlspecialchars($GLOBALS['db']), latin_htmlspecialchars($GLOBALS['table'])); ?></legend>
    ...
    <?php if ($tracking_active == false) {?>
        ...
        <form method="post" action="tbl_tracking.php?<?php echo $url_query; ?>">
        <fieldset>
            <legend><?php printf($strTrackingActivateTrackingFor, latin_htmlspecialchars($GLOBALS['db']), latin_htmlspecialchars($GLOBALS['table'])); ?></legend>
            <input type="hidden" name="version" value="<?php echo $last_version; ?>" />
...
<fieldset>
    <legend><?php printf($strTrackingCreateVersionOf, ($last_version + 1), latin_htmlspecialchars($GLOBALS['db']), latin_htmlspecialchars($GLOBALS['table'])); ?></legend>
