...
function UploadFile($upfile, $iswatermark='')
{
	...
	//global $cfg_markswitch, $cfg_markpicurl, $cfg_markwhere, $cfg_marktext, $cfg_marksize, $cfg_markcolor, $cfg_marktype;
	...
	$tempfile_ext  = substr(strrchr($tempfile_name, '.'), 1);
	...
	//检查文件类型,上传文件目录
	if(in_array($tempfile_ext, explode('|', $cfg_upload_img_type)))
	{
		$upload_url = 'image';
		$upload_dir = $cfg_image_dir;
	}
	else if(in_array($tempfile_ext, explode('|', $cfg_upload_soft_type)))
	{
		$upload_url = 'soft';
		$upload_dir = $cfg_soft_dir;
	}
	else if(in_array($tempfile_ext, explode('|', $cfg_upload_media_type)))
	{
		$upload_url = 'media';
	...
	//上传文件路径
	$save_url = 'upload/'.$upload_url.'/'.$filename;
	...
	{
		/*
		if($cfg_markswitch=='Y' and ($iswatermark=='true' or $iswatermark=='checked') and in_array($tempfile_ext, array('jpg','png','gif','bmp')))
		{
			WaterMark($save_dir, PHPMYWIND_ROOT.'/'.$cfg_markpicurl, $cfg_markwhere, $cfg_marktext, PHPMYWIND_DATA.'/fonts/incite.ttf', $cfg_marksize, $cfg_markcolor, $cfg_marktype);
		}
