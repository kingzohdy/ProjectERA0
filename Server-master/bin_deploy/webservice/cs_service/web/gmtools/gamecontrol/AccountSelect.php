...
var daq = -1;//用于保存选择的大区信息
var tp = "";//判断用户类型
var acct = 0; //保存用户的账号信息，用于修改邮箱
$(document).ready(function(){
		...
		{
			if(serverlist.AccountInfos.length==1)
			{
				$("#xdq").options.add(new Option(msg.AccountInfos.RegionName, msg.AccountInfos.RegionID, false, false));
			}
			else
			{
				for(var i = 0; i < serverlist.AccountInfos.length; i++)
...
function mingxi()
{
	...
	({
			...
			url: 'conduct.php',
			dataType: 'json',
			data: {"AccName": rolename,"cfs":cfs,"qu":daq},
			async:false,
			beforeSend:function(){
				$("#BgDiv").css({ display: "block", height: $(document).height() });
				$('#DialogDiv').show();
...
function pnext()
{
	...
	if(page > 0 && page == RegArray.length)
	{
		...
		({
			...
			url: 'conduct.php',
			dataType: 'json',
			data: {"AccName2np":"","page": FindId,"daqu":daq}, 
			beforeSend:function(){
				$("#BgDiv").css({ display: "block", height: $(document).height() });
				$('#DialogDiv').show();
			 },
	...
	if(page > 0 && page < RegArray.length)
	{
		...
		if(msg.ResNum == 1)
				...
				if(msg.ResNum > 1)
				{
					...
					for(var i = 0; i < msg.AccountLiteArray.length; i++){
						if(JSON.stringify(msg.AccountLiteArray[i].FreezeReason) == "{}"){ msg.AccountLiteArray[i].FreezeReason = "未冻结";}
						d +='<tr align=center ><td  height=32 width="10%">'+msg.AccountLiteArray[i].Pass9Account+'</td><td  height=32 width="10%">'+msg.AccountLiteArray[i].Uin+'</td><td  height=32 width="10%">'+date('Y-m-d H:i:s',msg.AccountLiteArray[i].AccCreateTime)+'</td><td  height=32 width="10%">'+msg.AccountLiteArray[i].FreezeReason+'</td><td  height=32 width="10%">'+date('Y-m-d H:i:s',msg.AccountLiteArray[i].FreezeEndTime)+'</td><td width="10%" >'+date('Y-m-d H:i:s',msg.AccountLiteArray[i].FreezeStartTime)+'</td><td width="10%" >'+numberToIp(msg.AccountLiteArray[i].LastLoginIP)+'</td><td width="10%" >'+date('Y-m-d H:i:s',msg.AccountLiteArray[i].LastLoginTime)+'</td>';
						if("高级游戏管理员"==tp || "超级游戏管理员"==tp)
						{
							d += '<td width="10%" >[<a href="#"  onclick = jingyan("'+msg.AccountLiteArray[i].Pass9Account+'","'+msg.ReginID+'") >禁言</a>][<a href="#" onclick = dongjie("'+msg.AccountLiteArray[i].Pass9Account+'","'+msg.ReginID+'") >冻结此帐号</a>][<a href="#" onclick = sfyz("'+msg.AccountLiteArray[i].Pass9Account+'") >修改安全邮箱</a>]';
						}
...
#DialogDiv{position:absolute;width:300px; left:55%; top:20%; margin-left:-200px; width:300px; z-index:100; border:0px #ccc solid; padding:1px;}
...
<div id="DialogDiv" style="display:none">
    ...
    <center style=" margin-top:5px;font-size:16px">详情</center>
    <br />
