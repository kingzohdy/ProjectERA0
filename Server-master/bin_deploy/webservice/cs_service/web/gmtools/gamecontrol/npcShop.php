...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"npcshop":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="12%" >'+value['acc']+'</td><td width="12%" >'+value['role']+'</td><td width="7%" >'+value['ShopAct']+'</td><td width="7%" >'+value['yinbi']+'</td><td width="7%">'+value['bangyin']+'</td><td width="10%" >'+value['daoju']+'</td><td width="7%" >'+value['GoodsData_Num']+'</td><td width="20%">'+value['xiaohao']+'</td><td width="20%" >'+value['time']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">NPC商店操作记录</span>
...
<table width="100%" border="0" cellpadding="0" cellspacing="0" class="mgr_table" id="bd">
		<tr class="thead2" align="center">
                ...
                <td width="7%">背包银币</td>
                <td width="7%">背包绑银</td>
                <td width="10%">购买(贩卖)道具</td>
                 <td width="5%">数量</td>
                <td width="20%">消耗/获得</td>
                <td width="18%">时间</td>
		</tr>
</table>
