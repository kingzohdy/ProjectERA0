...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"logstore":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="12%" >'+value['StoreName']+'</td><td width="12%" >'+value['StoreOwnerName']+'</td><td width="7%" >'+value['Type']+'</td><td width="12%" >'+value['BuyName']+'</td><td width="6%">'+value['Num']+'</td><td width="6%" >'+value['Money']+'</td><td width="6%" >'+value['PreMoney']+'</td><td width="6%">'+value['CurrMoney']+'</td><td width="21%">'+value['daoju']+'</td><td width="12%" >'+value['time']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">玩家挂单</span>
...
<table width="100%" height="100%">
	<tr>
		<td>大区：<select name="dq" id="dq" onchange="xf()" >
        ...
        <?php } ?> 
                     </select><span style="color:#F00" id="qu"></span><select name="fw" id="fw" >
        	 </select>
			<select id="fag" >
            	<option value="1" selected="selected">开店的角色</option>
                <option value="2">购买的角色</option>
            </select>
...
<table width="100%" border="0" cellpadding="0" id="tabr" cellspacing="0" class="mgr_table" id="datalist"></table>
