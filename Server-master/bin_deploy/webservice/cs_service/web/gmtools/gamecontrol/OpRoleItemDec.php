...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"OpRoleItemDec":"","xzdb": f,"time":tb, "time1":tb1, "us":jsm,"fag":fag,"qq":fu},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width="15%" >'+value['Uin']+'</td><td width="15%">'+value['LoginName']+'</td><td width="15%" >'+value['RoleName']+'</td><td width="10%">'+value['OpType']+'</td><td width="10%" >'+value['Item']+'</td><td width="10%">'+value['ItemNum']+'</td><td width="10%" >'+value['ItemBindNum']+'</td><td width="15%" >'+value['LogTime']+'</td></tr>';
					$("#tabr").append(s);
				});
			}
		}
