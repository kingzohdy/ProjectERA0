...
function sear()
{
	if(i ==1 && j == 1 && t==1)
	{
		...
		$.ajax({
			...
			url: 'select_log.php',
			dataType: 'json',
			data: {"rolelog":"","ldb": f,"time":tb, "time1":tb1, "acc":acc,"fag":fag}, 
			success: function(msg){
				if( null ==msg || ""== msg)
				{
					$("#tt1").html("未检索到数据......");
				...
				else
				{
					...
					$("#ttt").hide();
					$("#ajaxlist").empty();
					var ss='<tr class=thead2 align=center><td >账号</td><td >Uin</td><td >角色</td><td >Ip地址</td><td >登录/登出时间</td><td >MacAddr</td></tr>';
					$("#ajaxlist").append(ss);
					//处理检索到的数据......
					var row1 = msg.row1;
					var row2 = msg.row2;
					...
					for(var x=0;x<nums;x++)
					{
						...
						{
							denglv = row1[ii];
							var s = '<tr class=mgr_tr align=center ><td height=32 >'+denglv.AccountLoginName+'</td><td >'+denglv.Uin+'</td><td >'+denglv.RoleName+'</td><td >'+denglv.IP+'</td><td >'+denglv.LogTime+'</td><td >'+denglv.MacAddr+'</td></tr>';
							$("#ajaxlist").append(s);
						}
						if(jj<nums2)
						{
							dengchu = row2[jj];
							var cc = '<tr class=mgr_tr align=center style=color:#F00; ><td height=32 >(登出)：'+dengchu.AccountLoginName+'</td><td >'+dengchu.Uin+'</td><td >'+dengchu.RoleName+'</td><td >'+dengchu.IP+'</td><td>'+dengchu.LogTime+'</td><td></td></tr>';
							$("#ajaxlist").append(cc);
						}
					}
...
<body>
<div class="mgr_header"> <span class="title">角色(角色列表)--登录登出记录</span>
