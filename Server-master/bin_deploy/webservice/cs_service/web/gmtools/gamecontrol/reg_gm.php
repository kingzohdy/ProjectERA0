...
function sear()
{
	...
	$.ajax({
		...
		url: 'select_log.php',
		dataType: 'json',
		data: {"ldb": f,"time":tb, "time1":tb1, "us":jsm, "key":skey, "tab":'LogRoleGmCmd_'},
		success: function(msg){
			if( null ==msg || ""== msg)
			{
				$("#tabr").empty();
			...
			else
			{
				...
				$.each(msg, function (index, value) {
					...
					var s = '<tr class=mgr_tr align=center ><td height=32 width=5%><input type=checkbox name=checkid[] /></td><td width=24%>'+value['RoleName']+'</td><td width=24%>'+value['CmdLine']+'</td><td width=24%>'+value['LogTime']+'</td><td width=24%>LogRoleGmCmd_</td></tr>';
					$("#tabr").append(s);
					//$("#tabr tr").html((false,value.RoleName, value.ToName, value.Content, value.LogTime,value.data_table));
					//tri++;
				});
			}
		}
...
<body>
<div class="mgr_header"> <span class="title">GM日志管理</span>
...
<table width="100%" border="0" cellpadding="0" id="tabr" cellspacing="0" class="mgr_table" id="ajaxlist"></table>
 <div class="page_area">
