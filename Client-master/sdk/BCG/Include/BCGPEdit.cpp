...
//********************************************************************************
UINT CBCGPEdit::OnNcHitTest(CPoint point) 
{
...
void CBCGPEdit::OnBrowse ()
{
	...
	case BrowseMode_Folder:
		if (g_pShellManager != NULL)
		{
			...
			if (g_pShellManager->BrowseForFolder (strFolder, this, strFolder))
			{
	...
	case BrowseMode_File:
		{
			...
			if (GetParent () != NULL)
			{
				GetParent ()->RedrawWindow (NULL, NULL, RDW_FRAME | RDW_INVALIDATE);
			}
		}
		break;
...
void CBCGPEdit::SetIntenalImage ()
{
	...
	m_ImageBrowse.Create (IDB_BCGBARRES_BROWSE, 16, 0, RGB (255, 0, 255));
