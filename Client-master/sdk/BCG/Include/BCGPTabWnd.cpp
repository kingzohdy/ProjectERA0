...
void CBCGPTabWnd::OnPaint() 
{
		...
		m_rectTabsArea.Height () > 2 * nTabBorderSize + 1)
	{
		...
		if (m_iActiveTab >= 0)
		{
			...
			if (m_bFlat)
			{
				...
				if (m_location == LOCATION_BOTTOM)
				{
					pDC->MoveTo (pTabActive->m_rect.left + 1, pTabActive->m_rect.top);
					pDC->LineTo (pTabActive->m_rect.right, pTabActive->m_rect.top);
				}
				else
				{
					pDC->MoveTo (pTabActive->m_rect.left + 1, pTabActive->m_rect.bottom);
	...
	if (m_bFlat && m_nTabsHorzOffset > 0)
	{
		...
		if (m_location == LOCATION_BOTTOM)
		{
			pDC->MoveTo (m_rectTabsArea.left, m_rectTabsArea.top);
			pDC->LineTo (m_rectTabsArea.left, m_rectTabsArea.bottom - 2);
		}
		else
		{
			pDC->MoveTo (m_rectTabsArea.left, m_rectTabsArea.bottom);
			pDC->LineTo (m_rectTabsArea.left, m_rectTabsArea.top + 2);
		}
	}
...
void CBCGPTabWnd::AdjustTabs ()
{
	...
	else
	{
		...
		//-----------------------------------------
		int nTabWidth = m_rectTabsArea.Width () / nVisibleTabsNum - 1;
