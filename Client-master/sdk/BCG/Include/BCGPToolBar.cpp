...
//******************************************************************************************
int CBCGPToolBar::InsertButton (CBCGPToolbarButton* pButton, INT_PTR iInsertAt)
{
	ASSERT (pButton != NULL);
...
int CBCGPToolBar::OnToolHitTest(CPoint point, TOOLINFO* pTI) const
{
	...
	int nHit = (int) CBCGPBaseToolBar::OnToolHitTest(point, pTI);
	if (nHit != -1)
		return nHit;
...
BOOL CBCGPToolBar::SetCustomizeMode (BOOL bSet)
{
	...
	//-----------------------------------
	// Second step - adjust layput for all toolbars:
	//-----------------------------------
		...
		if (ar.IsLoading ())
		{
			...
			if (g_pWorkspace != NULL && 
				(g_pWorkspace->GetDataVersionMajor () == 6 &&
				 g_pWorkspace->GetDataVersionMinor () >= 2 ||
...
void CBCGPToolBar::ShowCommandMessageString (UINT uiCmdId)
{
	...
	if (uiCmdId == (UINT) -1 || uiCmdId == BCGPCUSTOMIZE_INTERNAL_ID)
	{
		GetOwner()->SendMessage(WM_SETMESSAGESTRING, AFX_IDS_IDLEMESSAGE);
		return;
	}
...
afx_msg LRESULT CBCGPToolBar::OnMouseLeave(WPARAM,LPARAM)
{
	...
	if (m_iHighlighted >= 0 && !bFocusHere)
	{
...
//****************************************************************************************
UINT CBCGPToolBar::OnNcHitTest(CPoint /*point*/) 
{
	return HTCLIENT;
}
//***************************************************************************************
...
BOOL CBCGPToolBar::SaveParameters (LPCTSTR lpszProfileName)
{
	...
	if (reg.CreateKey (strSection))
	{
		bResult =	reg.Write (REG_ENTRY_TOOLTIPS, m_bShowTooltips) &&
					...
					reg.Write (REG_ENTRY_ANIMATION, (int) CBCGPPopupMenu::GetAnimationType ()) &&
	...
	for (POSITION posTlb = gAllToolbars.GetHeadPosition (); posTlb != NULL;)
	{
		...
		if (CWnd::FromHandlePermanent (pToolBar->m_hWnd) != NULL)
		{
			...
			if (pToolBar->m_bLocked)
			{
				...
				if (m_bLargeIcons)
				{
					if (pToolBar->m_sizeCurButtonLocked.cx != pToolBar->m_sizeButtonLocked.cx * 2 ||
...
BOOL CBCGPToolBar::SmartUpdate (const CObList& lstPrevButtons)
{
	...
	//-----------------------------
	for (POSITION posPrev = lstPrevButtons.GetHeadPosition (); posPrev != NULL;)
	{
	...
	//-----------------------------
	int i = 0;
	for (POSITION posCurr = m_OrigButtons.GetHeadPosition (); posCurr != NULL; i++)
	{
		CBCGPToolbarButton* pButtonCurr = 
			DYNAMIC_DOWNCAST (CBCGPToolbarButton, m_OrigButtons.GetNext (posCurr));
		ASSERT_VALID (pButtonCurr);
...
void CBCGPToolBar::UpdateTooltips ()
{
	...
	for (int i = 0; i < m_Buttons.GetCount (); i++)
	{
		...
		if (pButton->m_nStyle != TBBS_SEPARATOR)
		{
			...
			if (!pButton->OnUpdateToolTip (this, i, m_ToolTip, strTipText))
			{
				m_ToolTip.AddTool (this, LPSTR_TEXTCALLBACK, pButton->Rect (), i + 1);
			}
...
BOOL CBCGPToolBar::OnNeedTipText(UINT /*id*/, NMHDR* pNMH, LRESULT* /*pResult*/)
{
	...
	m_ToolTip.SetFont (&globalData.fontRegular, FALSE);
	return TRUE;
}
//************************************************************************************
...
LRESULT CBCGPToolBar::OnPromptReset(WPARAM, LPARAM)
{
	...
	//Ask for reset
	if(IDOK == ::MessageBox(NULL, strPrompt, AfxGetApp()->m_pszAppName, MB_OKCANCEL|MB_ICONWARNING))
	{
		RestoreOriginalstate();
	}
...
void CBCGPToolBar::SetOneRowWithBrother()
{
	...
	if(pRowThis != pRowBrother)
	{
		if(m_bElderBrother)
		{
			...
			pRowThis->AddControlBar (m_pBrotherToolBar, DM_STANDARD);
		}
		else
		{
			pRowThis->RemoveControlBar (this);
			pRowBrother->AddControlBar (this, DM_STANDARD);
		}
	}
}
//*************************************************************************************
void CBCGPToolBar::SetTwoRowsWithBrother()
{
	...
	if(pRowThis == pRowBrother)
	{
		...
		if(m_bElderBrother)
		{
			...
			pNewRow->AddControlBar (this, DM_STANDARD);
		...
		else
		{
			...
			CBCGPDockBarRow* pNewRow = pDockBar->AddRow (pos, szBarSize.cy);
			pNewRow->AddControlBar(m_pBrotherToolBar, DM_STANDARD);
