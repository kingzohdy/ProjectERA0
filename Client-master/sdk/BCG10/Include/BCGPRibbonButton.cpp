	...
	if (bDrawText)
	{
		...
		else
		{
				...
				m_bIsLargeImage && HasMenu ())
			{
				...
				if (globalData.GetRibbonImageScale () > 1.)
				{
					cyMenu = (int)(.5 + globalData.GetRibbonImageScale () * cyMenu);
				}
...
CSize CBCGPRibbonButton::DrawBottomText (CDC* pDC, BOOL bCalcOnly)
{
	...
	if (nMenuArrowWidth != NULL && globalData.GetRibbonImageScale () > 1.)
	{
	...
	if (!rectMenuArrow.IsRectEmpty ())
	{
		...
		if (globalData.GetRibbonImageScale () > 1.)
		{
			nMenuArrowHeight = (int)(.5 + globalData.GetRibbonImageScale () * nMenuArrowHeight);
		}
