...
void CBCGPEditListBase::OnPaint() 
{
	...
	if (m_bVisualManagerStyle)
	{
		...
		dc.Draw3dRect (m_rectCaption, globalData.clrBarShadow, globalData.clrBarHilite);
	}
	else
	{
	...
	dc.SetTextColor (IsWindowEnabled () ? clrText : globalData.clrGrayedText);
...
BOOL CBCGPEditListBase::OnEraseBkgnd(CDC* pDC) 
{
	...
	pDC->FillRect (rectFill, m_bVisualManagerStyle ? &globalData.brBarFace : &globalData.brBtnFace);
	return TRUE;
}
//************************************************************************************
