...
//const UINT BCGM_GANTT_CHART_ITEMS_CHANGED   = ::RegisterWindowMessage (_T("BCGM_GANTT_ITEM_CHANGED"));
...
int GetFontHeight (CFont* pFont, CDC* pDC = NULL)
{
    if (pFont == NULL)
    {
...
IMPLEMENT_DYNCREATE (CBCGPGanttChart, CWnd)
...
BEGIN_MESSAGE_MAP(CBCGPGanttChart, CWnd)
...
void CBCGPGanttChart::DoPaint (CDC& dc)
{
	...
	{
		const CRect& rcRow = m_arrItemRects[i].m_rectRow;
		if (rcRow.top <= rcChart.bottom && rcRow.bottom >= rcChart.top)
		{
		...
		else if (rcRow.top > rcChart.bottom)
		{
			nLastVisibleRow = i - 1;
		}
	}
	...
	{
		DrawItemBackground (dc, m_arrItemRects[i].m_rectRow, m_arrItemRects[i].m_pItem);
	}
	...
	if (m_bShowEmptyRows)
	{
		...
		{
			rcRow = m_arrItemRects[m_nLastVisibleRow].m_rectRow;
		}
		while (rcRow.bottom < rcChart.bottom)
		{
			rcRow.top = rcRow.bottom;
			rcRow.bottom = rcRow.top + GetDefaultItemHeight ();
			DrawItemBackground (dc, rcRow, NULL);
		}
	}
	...
	for (i = nFirstVisibleRow; (i <= nLastVisibleRow) && (i >= 0) && (i < nItemRects); ++i)
	{
		const CBCGPGanttItem* pItem = m_arrItemRects[i].m_pItem;
		...
		ctxDraw.m_rectBar = m_arrItemRects[i].m_rectBar;
	...
	for (i = nFirstVisibleRow; (i <= nLastVisibleRow) && (i >= 0) && (i < nItemRects); ++i)
	{
        ...
        //if (pItem->IsSelected () m_nControlState != ecsMoveItems && m_nControlState != ecsResizeItems)
	...
	if (bDraggingItems)
	{
		for (i = nFirstVisibleRow; (i <= nLastVisibleRow) && (i >= 0) && (i < nItemRects); ++i)
		{
			...
			if (pItem->IsSelected ())
			{
				CRect rcBar = m_arrItemRects[i].m_rectBar;
...
void CBCGPGanttChart::AdjustLayout()
{
	...
	m_bInAdjustLayout = TRUE;
	{
		...
		CalculateRowsLayout (m_arrItemRects);
		...
		for (int i = 0; i < n; ++i)
		{
			...
			if (r.m_rectRow.top <= m_rectChart.bottom && r.m_rectRow.bottom >= m_rectChart.top)
			{
				...
				if (m_pToolTip != NULL && m_bShowTooltips)
				{
					m_pToolTip->AddTool (this, LPSTR_TEXTCALLBACK, &r.m_rectBar, nToolTipId ++);
				}
			}
			else if (r.m_rectRow.top > m_rectChart.bottom)
			{
...
void CBCGPGanttChart::CalculateRowsLayout (CArray<CRowLayout, CRowLayout&>& arrLayout)
{
	// Calculate Bar rectangles
	arrLayout.RemoveAll ();
	...
	while (pos != NULL)
	{
		...
		{
			CRowLayout r;
			r.m_pItem = pCurrentItem;

			r.m_rectRow.left = m_rectChart.left;
			r.m_rectRow.right = m_rectChart.right;
			r.m_rectRow.top = y;
			r.m_rectRow.bottom = y + h;

			r.m_rectBar.left = TimeToClient (pCurrentItem->GetStartTime ());
			r.m_rectBar.right = TimeToClient (pCurrentItem->GetFinishTime ());
			r.m_rectBar.top = r.m_rectRow.top + (h - nBarHeight) / 2;
			r.m_rectBar.bottom = r.m_rectBar.top + nBarHeight;

			if (pCurrentItem->IsMileStone ())
			{
				r.m_rectBar.left -= (nBarHeight / 2) + 1;
				r.m_rectBar.right = r.m_rectBar.left + nBarHeight + 1;
				r.m_rectBar.bottom += 1;
			}
...
CRect CBCGPGanttChart::GetItemRow(const CBCGPGanttItem* pItem) const
{
	...
	for (int i = 0; i < n; ++i)
	{
		if (m_arrItemRects[i].m_pItem == pItem)
		{
			return m_arrItemRects[i].m_rectRow;
		}
	}
...
CRect CBCGPGanttChart::GetBarRect(const CBCGPGanttItem* pItem) const
{
	...
	for (int i = 0; i < n; ++i)
	{
		if (m_arrItemRects[i].m_pItem == pItem)
		{
			return m_arrItemRects[i].m_rectBar;
		}
	}
...
void CBCGPGanttChart::GetVisibleTimeRange(COleDateTime* ptmStart, COleDateTime* ptmEnd) const
{
	if (ptmStart != NULL)
	{
		*ptmEnd = ClientToTime (GetVisibleChartArea ().left);
	}
	if (ptmEnd != NULL)
	{
...
void CBCGPGanttChart::CalculateConnectorLine (CBCGPGanttConnection& link, CRect rcSourceBar, CRect rcDestBar)
{
	...
	{
		ptDest.x = rcDestBar.left + 4;
	}
	else
	{
		ptDest.x = rcDestBar.right - 5;
	}
...
BOOL CBCGPGanttChart::QueryToolTipText(const CBCGPGanttItem* pItem, CString& strText, CString& strDescription) const
{
	...
	else
	{
		strDescription.Format (
			...
			strStart,
			strFinish,
			(int)(pItem->GetProgress () * 100 + 0.5f)
			);
	}
...
{
	return CWnd::Create (globalData.RegisterWindowClass (_T("BCGPGanttChart")), _T(""), dwStyle, rect, pParentWnd, nID);
}
...
void CBCGPGanttChart::UpdateScrollBars (BOOL bValuesOnly)
{
	...
	if (bHorzBarExists)
	{
        ...
        COleDateTimeSpan dtPage = m_dtPixelTimeDelta * GetVisibleChartArea ().Width ();

        const int xLeft = 0, xRight = 0x4000;
...
void CBCGPGanttChart::OnVScroll (UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	int y = m_wndScrollVert.GetScrollPos ();
	int yOld = y;
	...
	if (y != yOld && maxY > minY)
	{
		SetVScrollPos (y * (CalculateScrollHeight ()) / (maxY - minY));
	}
...
void CBCGPGanttChart::OnHScroll (UINT nSBCode, UINT nPos, CScrollBar* pScrollBar)
{
	...
	COleDateTimeSpan dtLineStep = m_hdrSmallHeader.dtCellTimeDelta;
	COleDateTimeSpan dtPageStep = m_hdrLargeHeader.dtCellTimeDelta / 2;
...
void CBCGPGanttChart::OnPaint ()
{
	...
	if (bHorzBarVisible && bVertBarVisible)
	{
		...
		dc.FillSolidRect (rcUnused, globalData.clrBtnFace);
	}
	if (bHorzBarVisible)
	{
...
CBCGPGanttItem* CBCGPGanttChart::ItemFromPoint (CPoint pt) const
{
	...
	{
		if (m_arrItemRects[i].m_rectBar.PtInRect(pt))
		{
			return m_arrItemRects[i].m_pItem;
		}
	}
...
BOOL CBCGPGanttChart::IsPointInItemResizeArea (CPoint point, CBCGPGanttItem** ppItem) const
{
    for (int i = (int)m_arrItemRects.GetSize () - 1; i >= 0; --i)
    {
        CRect rect = m_arrItemRects[i].m_rectBar;

        if (rect.PtInRect (point))
        {
            if (ppItem != NULL)
            {
                *ppItem = m_arrItemRects[i].m_pItem;
            }

            if (m_arrItemRects[i].m_pItem->IsMileStone ())
            {
                return FALSE;
            }
...
BOOL CBCGPGanttChart::OnSetCursor(CWnd* pWnd, UINT nHitTest, UINT message)
{
    ...
    BOOL bItemResize = IsPointInItemResizeArea (ptCursor, &pItem);
...
void CBCGPGanttChart::OnMouseMove (UINT nFlags, CPoint point)
{
	...
	case ecsSelectingItem:
		if (abs (m_ptDragStart.x - point.x) > 2 || abs (m_ptDragStart.y - point.y) > 2)
		{
	...
	case ecsMoveItems:
        if (bMouseButtonsPressed)
		{
			...
			if (szOffset != m_szDragOffset)
			{
				...
				{
					if (m_arrItemRects[i].m_pItem->IsSelected ())
					{
						InvalidateRect (m_arrItemRects[i].m_rectRow);
					}
				}
                bHandled = TRUE;
			}
