...
void CBCGPGanttItemStorageBase::Remove (CBCGPGanttItem* pItem)
{
	...
	{
        int i = IndexOf (pItem);
        NotifyUpdateSingleItem (BCGP_GANTT_STORAGE_BEFORE_REMOVE_ITEM, pItem, i, BCGP_GANTT_ITEM_PROP_NONE);

        int nRemoved = RemoveItemConnections (pItem);
        DoRemove (pItem);

        NotifyUpdateSingleItem (BCGP_GANTT_STORAGE_REMOVED_ITEM, pItem, i, BCGP_GANTT_ITEM_PROP_NONE);
		delete pItem;

        if (nRemoved > 0)
        {
            UpdateAll (BCGP_GANTT_ITEM_PROP_NONE);
        }
	}
}

void CBCGPGanttItemStorageBase::RemoveAt(int pos)
{
	CBCGPGanttItem* pItem = GetItem (pos);
	if (pItem != NULL)
	{
        NotifyUpdateSingleItem (BCGP_GANTT_STORAGE_BEFORE_REMOVE_ITEM, pItem, pos, BCGP_GANTT_ITEM_PROP_NONE);

        int nRemoved = RemoveItemConnections (pItem);
        DoRemoveAt (pos);

        NotifyUpdateSingleItem (BCGP_GANTT_STORAGE_REMOVED_ITEM, pItem, pos, BCGP_GANTT_ITEM_PROP_NONE);
		delete pItem;
...
void CBCGPGanttItemStorageBase::RemoveAll()
{
	...
	DoRemoveAll ();
    NotifyUpdateItems (BCGP_GANTT_STORAGE_REMOVED_ITEM, 0, nCount - 1, BCGP_GANTT_ITEM_PROP_NONE);
}
