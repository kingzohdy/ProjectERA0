...
BOOL CBCGPProgressDlg::OnInitDialog() 
{
	...
	else if (!m_Params.m_bShowPercentage)
	{
		CRect rectPerc;
	...
	if (IsWindowBackground () && m_Params.m_bShowCancel)
	{
		CRect rectLine;
		m_wndLine.GetWindowRect (rectLine);
...
BOOL CBCGPProgressDlg::OnEraseBkgnd(CDC* pDC) 
{
	...
	else
	{
		...
		if (m_yLine >= 0)
		{
			...
			rectBottom.top = m_yLine;

			pDC->FillRect (rectBottom, &globalData.brBtnFace);

			CBCGPDrawManager dm (*pDC);
			dm.DrawLine (rectBottom.left, m_yLine, rectBottom.right, m_yLine, globalData.clrBtnShadow);
		}
	}
