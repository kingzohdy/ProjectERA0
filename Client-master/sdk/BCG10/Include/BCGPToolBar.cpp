...
void CBCGPToolBar::OnChangeHot (int iHot)
{
	...
	if (pCurrPopupMenu == NULL && !CBCGPToolBar::IsCustomizeMode ())
	{
		...
		if (pMenuBar != NULL && globalData.IsAccessibilitySupport ())
		{
			if (GetFocus () == this)
            {
                if (m_iHot >= 0 && m_iHot < m_Buttons.GetCount())
                {
	...
	if (m_iHot >= 0 && m_iHot != m_iHighlighted)
	{
		...
		if (pMenuBar != NULL && globalData.IsAccessibilitySupport ())
		{
			if (m_iHot >= 0 && m_iHot < m_Buttons.GetCount() && m_iHighlighted==-1)
			{
				globalData.NotifyWinEvent (EVENT_OBJECT_FOCUS, GetSafeHwnd (), 
					OBJID_CLIENT , m_iHot);
			}
...
BOOL CBCGPToolBar::RemoveResetStateButton (UINT uiCmdId)
{
	...
	CBCGPToolbarButton* pButton = (CBCGPToolbarButton*) m_OrigResetButtons.GetAt (posButton);
	ASSERT_VALID (pButton);
...
//******************************************************************************
void CBCGPToolBar::AccNotifyObjectFocusEvent (int iButtton)
{
	if (!globalData.IsAccessibilitySupport ())
	{
		return;
	...
	CBCGPToolbarButton * pButton = GetButton (iButtton);
	if (pButton != NULL)
	{
		ASSERT_VALID (pButton);
		pButton->SetACCData (this, m_AccData);

		globalData.NotifyWinEvent (EVENT_OBJECT_FOCUS, GetSafeHwnd (), OBJID_CLIENT , iButtton + 1);
	}
}
//**************************************************************************
