...
void CBCGPPlannerViewMonth::OnDrawClient (CDC* pDC, const CRect& rect)
{
	...
	for (iRow = 0; iRow < nRows; iRow++)
	{
		for (int iDay = 0; iDay < 7; iDay++)
		{
			...
			if (m_nRowHeight <= rectDayCaption.Height () + 2)
			{
...
void CBCGPPlannerViewMonth::OnDrawWeekBar (CDC* pDC, const CRect& rectBar)
{
	...
	CString strDateFmt (_T("%d"));
	CString strMonthFmt (_T("%d"));
...
/*
	...
	if (CBCGPPlannerView::IsMonthLZero ())
	{
		strMonthFmt = _T("%.2d");
	}
*/

	BOOL bDateMonth = CBCGPPlannerView::IsDateBeforeMonth ();

	if (bDateMonth)
	{
		strMonthFmt = strDateFmt + CBCGPPlannerView::GetDateSeparator () + strMonthFmt;
	}
	else
	{
		strMonthFmt = strMonthFmt + CBCGPPlannerView::GetDateSeparator () + strDateFmt;
	}
	...
	for (int iRow = 0; iRow < nRows; iRow++)
	{
		...
		if (rect.top < rect.bottom)
		{
			...
			{
				CString strFormat;
				strFormat = strDateFmt + _T(" - ") + strMonthFmt;

				if (bDateMonth)
				{
					strText.Format (strFormat, day1.GetDay (), day2.GetDay (), day2.GetMonth ());
				}
				else
				{
					strText.Format (strFormat, day1.GetDay (), day2.GetMonth (), day2.GetDay ());
				}
			}
			else
			{
				CString strFormat;
				strFormat = strMonthFmt + _T(" - ") + strMonthFmt;

				if (bDateMonth)
				{
					strText.Format (strFormat, day1.GetDay (), day1.GetMonth (), day2.GetDay (), day2.GetMonth ());
				}
				else
				{
					strText.Format (strFormat, day1.GetMonth (), day1.GetDay (), day2.GetMonth (), day2.GetDay ());
				}
			}
