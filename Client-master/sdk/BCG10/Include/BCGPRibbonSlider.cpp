...
void CBCGPRibbonSlider::OnDraw (CDC* pDC)
{
	...
	{
		DrawText (pDC, m_strText, m_rectLabel, DT_CENTER | DT_SINGLELINE);
	}
}
//*****************************************************************************
...
void CBCGPRibbonSlider::OnAfterChangeRect (CDC* pDC)
{
	...
	if (m_bZoomButtons)
	{
		...
		if (IsVert ())
		{
			m_rectZoomOut.bottom = m_rectZoomOut.top + cySliderHeight;
		}
		else
		{
		...
		if (IsVert ())
		{
			m_rectZoomIn.top = m_rectZoomIn.bottom - cySliderHeight;
