/** @file $RCSfile: tbus.h,v $
  ...
  $Id: tbus.h,v 1.14 2009/01/23 06:45:56 jackyai Exp $
@author $Author: jackyai $
@date $Date: 2009/01/23 06:45:56 $
...
@param[in] a_iFlag -- 控制字段，各控制标志支持'与（|）'方式结合使用，目前已定义的标志位有：
  ...
  @pre a_iDataLen 必须大于0
*/
TSF4G_API int tbus_recv (IN const int a_iHandle,	INOUT int *a_piSrc,	INOUT int *a_piDst,
...
* @return  错误信息串的指针
*/
TSF4G_API  char const*tbus_error_string(IN int iErrorCode);
