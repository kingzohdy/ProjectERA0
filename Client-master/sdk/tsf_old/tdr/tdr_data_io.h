...
*@param[in] a_iIOVersion XML格式输入/输出的版本,目前支持两个版本
*	- 1	表示FO-DR定义输入/输出格式
*	- 2 表示TSF-G-DR定义的输入/输出格式
*	- 这两种格式的区别是: 对于复合数据类型数组,FO-DR格式中,XML节点以元数据类型作标示,而TSF-G-DR新格式以元数据名作标示
*
*@par 等价处理:
*  @code
	假定有元数据定义如下：
	...
	<struct name="type2" >
		....
		<entry name="Body" type="type1" count="2" />
	</struct>

	在输出type2元数据表示的数据时，按照FO-DR的格式，输出如下：
	<type2>
		....
		<Body>
			...
...
*/
TDR_API int tdr_output(IN LPTDRMETA a_pstMeta, INOUT LPTDRDATA a_pstXml, INOUT LPTDRDATA a_pstHost,
...
* @see	tdr_get_meta_by_id
*/
TDR_API int tdr_output_fp(IN LPTDRMETA a_pstMeta, IN FILE *a_fpXml, INOUT LPTDRDATA a_pstHost,
			   IN int a_iCutOffVersion, IN int a_iIOVersion);
...
* @see	tdr_get_meta_by_id
*/
TDR_API int tdr_output_file(IN LPTDRMETA a_pstMeta, IN const char *a_szFile, INOUT LPTDRDATA a_pstHost,
				  IN int a_iCutOffVersion, IN int a_iIOVersion);
...
在输入type2元数据表示的数据时，按照FO-DR定义的格式，输入格式如下：
	<type2>
	....
	<Body>
	...
...
*	- 1	表示旧的输入/输出格式
*	- 2 表示新的输入/输出格式
*	- 这两种格式的区别是: 对于复合数据类型数组,旧的格式中,XML节点以元数据类型作标示,而新格式以元数据名作标示
*
*@note 如果a_iCutOffVersion为0 ，则安装剪裁到最高版本
*@note 如果a_iIOVersion指定除1，2以外的值，则强制按照版本2进行处理
*
