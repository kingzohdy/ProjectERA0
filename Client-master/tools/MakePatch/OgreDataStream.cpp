...
size_t DataStream::readLine(char* buf, size_t maxCount, const String& delim)
{
	...
	char tmpBuf[OGRE_STREAM_TEMP_SIZE];
	size_t chunkSize = min(maxCount, (size_t)OGRE_STREAM_TEMP_SIZE-1);
	size_t totalCount = 0;
	size_t readCount; 
	while (chunkSize && (readCount = read(tmpBuf, chunkSize)))
	{
		...
		chunkSize = min(maxCount-totalCount, (size_t)OGRE_STREAM_TEMP_SIZE-1);
	}
...
//-----------------------------------------------------------------------
MemoryDataStream::MemoryDataStream(const String& name, const DataStream * sourceStream, 
								   bool freeOnClose)
								   : DataStream(name)
{
	// Copy data from incoming stream
	...
	mData = new uchar[mSize];
	mPos = mData;
	mEnd = mData + ((DataStream *)sourceStream)->read(mData, mSize);
	mFreeOnClose = freeOnClose;
}
//-----------------------------------------------------------------------
									  ...
									  const String& delim)
{
	if (delim.empty())
	{
		//LOG_SEVERE("No delimiter provided");
	}
	if (delim.size() > 1)
	{
		//LOG_WARNING("FileStreamDataStream::readLine - using only first delimeter");
	}
	// Deal with both Unix & Windows LFs
	bool trimCR = false;
	if (delim.at(0) == '\n') 
	...
	else if (mpStream->fail())
	{
		...
		{
			//LOG_SEVERE("Streaming error occurred");
		}
	}
	else 
	{
...
void *FileStreamDataStream::getMemoryImage()
{
	//assert(0);
	return NULL;
}
...
: DataStream(), mFileHandle(handle), m_pMemImage(NULL)
{
	// Determine size
...
: DataStream(name), mFileHandle(handle), m_pMemImage(NULL)
{
	// Determine size
	fseek(mFileHandle, 0, SEEK_END);
	mSize = ftell(mFileHandle);
...
//-----------------------------------------------------------------------
void *FileHandleDataStream::getMemoryImage()
{
	if(m_pMemImage == NULL)
	{
		m_pMemImage = malloc(mSize);
		fseek(mFileHandle, 0, SEEK_SET);
		fread(m_pMemImage, mSize, 1, mFileHandle);
	}
