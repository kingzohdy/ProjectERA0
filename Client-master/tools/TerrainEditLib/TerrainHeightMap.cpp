...
namespace TE
{
	TerrainHeightMap::TerrainHeightMap(void)
	{
		Destory();
	}
	...
	void    TerrainHeightMap::CreateMap(Ogre::Point2D nblock, Ogre::Point2D ngrid, Ogre::Vector3 startPos , float fSize , float fBaseHeight,float fScale)
	{
		Destory();
	...
	//更新法线;参数( 索引ID );
	void     TerrainHeightMap::UpdateNormal(int index)
	{
	...
	//更新法线;参数( X位置 , Y 位置 )
	void     TerrainHeightMap::UpdateNormal(int indexX, int indexY)
	{
		Ogre::Vector3  pos[4];
		...
		Ogre::Vector3 normal = m_vNormal[indexY*m_nWidth + indexX] ;
	...
	bool     TerrainHeightMap::SelectMapInfo(Ogre::Vector3 pos ,float fsize , HEIGHT_AREA area)
	{
		static Ogre::Vector3 savePos(0,0,0);
	...
	//计算高度图的法线
	void     TerrainHeightMap::CountNormal()
	{
		if( m_vVec.empty() )
			return;
	...
	void     TerrainHeightMap::ChangeSelectHeightRadio(float height , HEIGHT_CHANGE_MODE mode ,float fSize )
	{
		float changeheight ;
		for( size_t i = 0 ; i < m_vSelectInfo.size () ; i ++ )
		{
			...
			changeheight = height* cos ( ( fLength / fSize ) *Ogre::ONE_PI / 2.0f ) ;
	...
	void     TerrainHeightMap:: ChangeSelectHeigthLine(float height , HEIGHT_CHANGE_MODE mode)
	{
		float changeHeight ;
	...
	//平整选中的高度;参数( 修改高度 )	
	void     TerrainHeightMap::LevelOffSelectHeight(float height)
	{
	...
	//平滑选中区域高度
	void     TerrainHeightMap::SmoothSelectHeightMapData()
	{
		if ( m_vSelectInfo.empty() )
		{
			return ;
	...
	//平滑高度图数据
	void     TerrainHeightMap::SmoothHeightMapData()
	{
		if( m_vHeight.empty() )
		{
			return;
	...
	// 返回 ( 返回为计算过的法线 )
	Ogre::Vector3    TerrainHeightMap::CountNormal( Ogre::Vector3 &v0,Ogre::Vector3 &v1,Ogre::Vector3 &v2,Ogre::Vector3 &v3,Ogre::Vector3 &v4 )
	{
		Ogre::Vector3  nor1,nor2,nor3,nor4,nor;

		Ogre::Vector3 v11,v22;
		v11 = (v2 - v0 ) ;
		v22 = ( v1 - v0) ;
	...
	bool     TerrainHeightMap::GetHeight(float x, float z,float*  height)
	{	
		(*height) = 0.0f;
	...
	bool    TerrainHeightMap::SetHeightData( int width ,int height , unsigned char *data)
	{
	...
	bool    TerrainHeightMap::GetGridHeight(int x,int z , float* height )
	{
		(*height) = 0.0f;
	...
	bool TerrainHeightMap::ReadBmp(const  char* filename)
	{
		return false;
	};
	//功能: 处理高度图 
	...
	// 返回: ( 返回为真表示成功 )
	bool 
		TerrainHeightMap:: ProcessMap(HEIGHT_PROC_EVEN  even ,HEIGHT_CHANGE_MODE mode,HEIGHT_AREA area,  Ogre::Vector3  centerPos , float height ,float fsize,bool bChange)
	{
		if( m_vHeight.empty() )
			return false;
