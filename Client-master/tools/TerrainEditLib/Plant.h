...
public:
	 ...
	 //增加位置扩展
	 bool   AddPosEx_1(MVec3 pos , float fScale = 1.0f , unsigned int color = 0);
     //设置位置
	 ...
	 //设置植物的纹理贴图
	 bool   SetTexture(LMTex  tex);
     //重置
	 void   Reset();
	 //删除位置
	 void   DelPos( MVec3 pos , float fSize );
     //得到当前植物数量
	 int    GetCurPlantNum(){return m_nCurVecNum / 4;}
     //清除顶点颜色
	 void   ClearLightColor();
	 //计算顶点光照颜色；参数（ 灯光位置，灯光颜色，灯光范围 ）
	 void   CountLightColor(int lighttype, MVec3 pos, Ogre::ColourValue color, Ogre::ColourValue ambient, float range, float shadowdensity=0);
	 //更新灯光颜色
	 void   UpdateLightColor(); 

	 void	UpdateHeight(MChunk* pChunk);
public:
	MVec3                       m_SavePosSet[PLANT_MAX_NUM];
    unsigned  int               m_unSaveColorSet[PLANT_MAX_NUM];  

	float                       m_SaveScaleSet[PLANT_MAX_NUM];

	MVec3                       m_PlayerPos;
	MVec3                       m_PosSet[PLANT_VEC_MAX_NUM];
...
public:
	...
	std::vector < CPlantSet* >    m_PlantManager; 
...
extern TE::CPlantManager    *g_PlantMgr;