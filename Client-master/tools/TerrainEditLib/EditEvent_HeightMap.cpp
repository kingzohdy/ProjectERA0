...
namespace TE
{
	...
	float GetRampHeight( Ogre::Vector3  pos0,Ogre::Vector3 pos1,Ogre::Vector3  srcPos ,float height,float length)
	{
		float _height;
		Ogre::Vector2  _pos0,_pos1,_srcPos;
		_pos0.x = pos0.x ;
		_pos0.y = pos0.z ;
		_pos1.x = pos1.x ;
		_pos1.y = pos1.z ;
	...
	HeightMapEditHandler::HeightMapEditHandler(void)
	{     
	...
	void  HeightMapEditHandler::SaveData( TerrainEditMgr* pEditor )
	{
	...
	void  HeightMapEditHandler::Clear()
	{
		for( size_t i = 0 ; i < m_SaveMapHeightData.size () ; i++)
		{
			m_SaveMapHeightData[i].clear ();
	...
	void  HeightMapEditHandler::Undo( TerrainEditMgr *pEditor )
	{
		// 将高程数据恢复到之前的数据
	...
	bool HeightMapEditHandler::handleEditEvent( TerrainEditMgr* pEditor , ActionEvent *event )
	{
		static CHANGE_HEIGHT_MODE buttonStateBefore = g_TerrainEdit.GetCurHeightMode();
		static bool canGetShift = true;
		if (canGetShift)
		{
			buttonStateBefore = g_TerrainEdit.GetCurHeightMode();
		}

		if (g_MInputProcess.GetUserInput()->bLShift)
		{	
			canGetShift = false;
			g_TerrainEdit.SetCurHeightMode(SMOOTH_MODE);
		}
		else
		{
			canGetShift = true;
			g_TerrainEdit.SetCurHeightMode(buttonStateBefore);
		}

		if (g_TerrainEdit.GetHeightAddMode())
		{
		...
		if (g_MInputProcess.GetUserInput()->bLAlt)
		{
			g_TerrainEdit.SetModifyHeight(-10);
		}
	...
	void  HeightMapEditHandler::ModifyTerrainHeight( TerrainEditMgr* pEditor , int height,int mode)
	{
	...
	bool  HeightMapEditHandler::AddSmoothHeightPos(  TerrainEditMgr* pEditor ,int index)
	{
	...
	void HeightMapEditHandler::ModifyChunkHeight ( TerrainEditMgr* pEditor , float height,int mode)
	{	
		...
		if( pEditor->m_EditOption.mCurChangeHeightMode == RAMP_MODE )
		{
			pEditor->m_pCampSign->SetPos ( pEditor->m_saveSelectInfo.pos ,
				pEditor->m_curSelectInfo.pos , 
				pEditor->m_EditOption.nCurChangeHeightSize*( TERRBLOCK_SIZE / (pEditor->m_HeightMap.GerGridNum()*1.0f) ) );
			return;
		}
		...
		for( int i = mapIndexMinY ; i < mapIndexMaxY ; i++)
		{
			for( int j = mapIndexMinX ; j < mapIndexMaxX ; j++ )
			{
				...
				if( ( mmx + mmy ) <= lengthSqr )
				{
					...
					case SOFT_MODE:
						{
							pEditor->m_HeightMap.m_vHeight[i*pEditor->m_HeightMap.GetMapWidth()+j] += height*cos( (mmx+mmy)/(lengthSqr*1.0f) * Ogre::ONE_PI/2 ) ; 
						}
						break;
					case CURVE_MODE:
						{
							float fDefValue = 0.0f;
							float fInValue = Ogre::Sqrt((mmx+mmy))/(float)(pEditor->m_EditOption.nCurChangeHeightSize);
							float fValue = pEditor->GetHeightCurve()->Evaluate(fInValue,fDefValue,0);
							pEditor->m_HeightMap.m_vHeight[i*pEditor->m_HeightMap.GetMapWidth()+j] += height*fValue; 	
						}
						break;
					case RAMP_MODE:
						{
		...
		{
			TerrainEditMgr::SaveModifyVecInfo  saveInfo;
		...
		if(maxgx >= pEditor->GetEditData()->gw)
			maxgx = pEditor->GetEditData()->gw-1;
		mingy = pEditor->m_curSelectInfo.gy - 2;
		if(mingy < 0 )
			mingy = 0;
		maxgy = pEditor->m_curSelectInfo.gy + 2;
		if(maxgy >= pEditor->GetEditData()->gh)
			maxgy = pEditor->GetEditData()->gh-1;
	...
	void HeightMapEditHandler::SmoothHeightMap( TerrainEditMgr* pEditor )
	{
	...
	void  HeightMapEditHandler::UpdateHeightMapToTileHeight(  TerrainEditMgr* pEditor )
	{
		EditorTerrainTile*  pTile = g_Terrain.GetTitle ( 0 );
		if( pTile == NULL )
			return;
		...
		int p = 0;		
		EditorTerrainBlock* pchunk = &pTile->GetTileData ()->chunkSet [0][0];
			...
			for( int i = 0 ; i < pTile->GetTileData()->gh ; i++)
			{
				for ( int j = 0 ; j < pTile->GetTileData()->gw ; j++)
				{
					...
					if( pchunk->GetChunkOptionData ()->bChangeHeight != 0)
					{
						...
						{								
							int mapIndex = 
								i*pEditor->m_HeightMap.GerGridNum()*pEditor->m_HeightMap.GetMapWidth() + 
								j*pEditor->m_HeightMap.GerGridNum() + 
								( k / ( pEditor->m_HeightMap.GerGridNum()+1) )*pEditor->m_HeightMap.GetMapWidth() +
								k%( pEditor->m_HeightMap.GerGridNum()+1) ;
	...
	void HeightMapEditHandler::ModifyChunkHeightEx( TerrainEditMgr* pEditor , int gx,int gz , float height,int mode)
	{	
		if( ! pEditor->GetEditData() )
			return ;	
	...
	void  HeightMapEditHandler::ReCountNormal( TerrainEditMgr* pEditor )
	{
	...
	void HeightMapEditHandler::CreatRampMap( TerrainEditMgr* pEditor )
	{
		pEditor->m_SaveSmoothPos.clear ();
		...
		Ogre::Vector3   pos0 = pEditor->m_curSelectInfo.pos  , pos1 = pEditor->m_saveSelectInfo.pos  ;
		if( pos0.y > pos1.y )
		{
			pos0 = pEditor->m_saveSelectInfo.pos ;
			pos1 = pEditor->m_curSelectInfo.pos ;
		}
		Ogre::Vector2 _pos0,_pos1;
		_pos0.x = pos0.x ;
		_pos0.y = pos0.z ;
		_pos1.x = pos1.x ;
		_pos1.y = pos1.z ;
		Ogre::Vector2 length_0 = _pos1 - _pos0;
	...
	void  HeightMapEditHandler::UpdataNormalMap( TerrainEditMgr* pEditor )
	{
		pEditor->m_HeightMap.CountNormal();
	}


	bool HeightMapEditHandler::SaveCurHeightMap(  TerrainEditMgr* pEditor ,const char* filename)
	{
		EditorStream stream;
	...
	void HeightMapEditHandler::SetHeightMap(  TerrainEditMgr* pEditor ,int width,int height , unsigned char  *heightmap )
	{
		EditorTerrainTile*  pTile = g_Terrain.GetTitle ( 0 );
		if( pTile == NULL )
			return;
	...
	void  HeightMapEditHandler::UpdateHeightMapToAllTileHeight(  TerrainEditMgr* pEditor )
	{
		EditorTerrainTile*  pTile = g_Terrain.GetTitle ( 0 );
		if( pTile == NULL )
			return;
		...
		for( int i = 0 ; i < pTile->GetTileHead()->gridY ; i++)
		{
			...
			{	
				EditorTerrainBlock* pchunk = &pTile->GetTileData ()->chunkSet [i][j];
	...
	void  HeightMapEditHandler::UpdateTileHeightDataToHeightMap( TerrainEditMgr* pEditor )
	{
		EditorTerrainTile*  pTile = g_Terrain.GetTitle ( 0 );
		if( pTile == NULL )
			return;

		{
			...
			pEditor->m_HeightMap.CreateMap(Ogre::Point2D(phead->gridX, phead->gridY), Ogre::Point2D(phead->gridVecNum, phead->gridVecNum), pos, fSize , 100.0f );
	...
	void  HeightMapEditHandler::CreateHeightMapEX(  TerrainEditMgr* pEditor, Ogre::Point2D nblock, Ogre::Point2D ngrid, int minheight ,int maxheight, int randsize,int smoothNum)
	{
	...
	void  HeightMapEditHandler::SmoothHeightMap( TerrainEditMgr* pEditor ,int width ,int height )
	{
