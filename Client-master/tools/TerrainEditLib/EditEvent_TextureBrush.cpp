...
namespace TE
{
	...
	TextureBrushEditHandler::TextureBrushEditHandler()
	{
	...
	bool TextureBrushEditHandler::handleEditEvent( TerrainEditMgr* pEditor , ActionEvent *event )
	{
	...
	Ogre::Vector3  TextureBrushEditHandler::PaintTexture( int x ,int y )
	{
		Ogre::Vector3  pos ;

		Ogre::Vector3  centerpos = g_Camera.m_Pos ;
	...
	void TextureBrushEditHandler::PaintTexture ( TerrainEditMgr* pEditor ,Ogre::Vector3 &dir, Ogre::Vector3 &pos ,int mode ,int x,int y)
	{
		...
		if( g_Terrain.TestAndRay (dir,pos,pos0) || g_bUseOtherCamera)
		{
			...
			case LAYER_0:
				{
					g_UndoManager.SaveCurData(gx, gz, 0);
					...
					for(int w = -1 ; w <= 1 ; w++)
					{
						for(int q = -1 ; q <= 1; q++)
						{
							if(bDraw[w+1][q+1])
							{
								...
								{
									g_UndoManager.SaveCurData(gx+q, gz+w, 0);
	...
	void TextureBrushEditHandler::duplateAlphaTexEdge(TerrainEditMgr* pEditor, int gx, int gz, int layer)
	{
		TerrBlockData* pchunk =  pEditor->GetEditData()->chunkSet[gz][gx].GetChunkData();

		if(gx > 0)
		{
			TerrBlockData* pchunk1 = pEditor->GetEditData()->chunkSet[gz][gx-1].GetChunkData();
			for(int z=0; z<64; z++)
			{
				pchunk->alpahMap[layer].data[z*64] = pchunk1->alpahMap[layer].data[z*64+63];
			}
		}

		if(gz > 0)
		{
			TerrBlockData* pchunk1 = pEditor->GetEditData()->chunkSet[gz-1][gx].GetChunkData();
			for(int x=0; x<64; x++)
			{
				pchunk->alpahMap[layer].data[x] = pchunk1->alpahMap[layer].data[63*64+x];
			}
		}

		if(gx>0 && gz>0)
		{
			TerrBlockData* pchunk1 = pEditor->GetEditData()->chunkSet[gz-1][gx-1].GetChunkData();
			pchunk->alpahMap[layer].data[0] = pchunk1->alpahMap[layer].data[64*64-1];
		}
	}

	void  TextureBrushEditHandler::ModifyAlphaTex( TerrainEditMgr* pEditor ,int gx,int gz ,int layer,int x,int z,unsigned char* data,int texid)
	{
		//注意 layer 已经在外部被减 1
		...
		{	
			...
			duplateAlphaTexEdge(pEditor, gx, gz, layer);
	...
	int   TextureBrushEditHandler::AddTextureFileToTitle( TerrainEditMgr* pEditor , std::string name)
	{
		if( pEditor->GetEditData() == NULL)
			return -1;
	...
	void   TextureBrushEditHandler::ResetPaintTexture( TerrainEditMgr* pEditor )
	{
	...
	void  TextureBrushEditHandler::CreateAlphaPaint( TerrainEditMgr* pEditor  )
	{
		...
		{	
			...
			for(i = 0 ; i < 64 ; i++)
			{
				for(j = 0 ; j < 64 ; j++)
				{
					testda = (int)( 255 - (sqrt( (float)( (i-r0)*(i-r0) + (j-r0)*(j-r0)) ) )*da );
	...
	void  TextureBrushEditHandler::Undo( TerrainEditMgr* pEditor )
	{
		...
		{		    
			UndoSaveData *pData = pgroup->m_SaveSet[i];
	...
	void  TextureBrushEditHandler::CreateQuadPaint( TerrainEditMgr* pEditor )
	{
		int i,j;
	...
	void  TextureBrushEditHandler::CreateRandPaint (  TerrainEditMgr* pEditor )
	{
		int i,j;
	...
	void  TextureBrushEditHandler::CreateRandCirclePaint(TerrainEditMgr* pEditor )
	{
		int i,j;
	...
	int       TextureBrushEditHandler::AddTextureFileToTile( TerrainEditMgr* pEditor ,const char* filename)
	{
		int texID = 0 ;
	...
	void      TextureBrushEditHandler::UpdateAlphaData( TerrainEditMgr* pEditor ,const char* filename, int layer)
	{
		if( pEditor->GetEditData() == NULL )
			return;
	...
	void    TextureBrushEditHandler::SaveChunkTextureInfo( TerrainEditMgr* pEditor )
	{
		if( pEditor->GetEditData() == NULL )
			return;
	...
	void   TextureBrushEditHandler::LoadChunkTextureInfo( TerrainEditMgr* pEditor )
	{
		EditorStream   stream;
	...
	void      TextureBrushEditHandler::UpdateColorData( TerrainEditMgr* pEditor ,const char* filename )
	{
		TextureData *ptex = new TextureData();
		ptex->loadFromImageFile(filename);

		Ogre::Texture *  pTex = ptex;
		if( pTex == NULL )
			return;
	...
	void      TextureBrushEditHandler::UpdateShadowData( TerrainEditMgr* pEditor ,const char* filename )
	{
		if( pEditor->GetEditData() == NULL )
			return;
	...
	void   TextureBrushEditHandler::SaveAlphaTex(  TerrainEditMgr* pEditor ,const char* filename, unsigned char* pData,int layer)
	{
		if( pEditor->GetEditData() == NULL )
			return;
	...
	void      TextureBrushEditHandler::LoadTerainLayerInfo( TerrainEditMgr* pEditor )
	{
		if( pEditor->GetEditData() == NULL )
			return;  
	...
	void  TextureBrushEditHandler::SetCurSelectPaintTexName( TerrainEditMgr* pEditor ,const char* filename)
	{
		int index;
