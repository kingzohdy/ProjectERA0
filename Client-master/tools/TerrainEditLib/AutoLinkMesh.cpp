...
{                     
	...
	MSAFE_DELETE_ARRAY( m_LinkMap );
	MSAFE_DELETE_ARRAY( m_NewMap );
	MSAFE_DELETE_ARRAY( m_UsedModelMap );
	MSAFE_DELETE_ARRAY( m_ModelMapInfo );
	MSAFE_DELETE_ARRAY( m_SignCullInfo );
	MSAFE_DELETE_ARRAY ( m_bLoadedInfo );
...
//设置开始引导ID的位置;参数( 位置 )	
void  LinkMeshManager::SetBeginLoadIDPos( MVec3  pos )
{
	 int mx,my;
...
//初始化LINK模型;参数( 地图大小 )
void  LinkMeshManager::Init(int unitsize, MPoint2D nblock, MPoint2D ngrid)
{
	Destory();
...
//设置地图的标记;参数( 位置 , 标记 ) 		
void  LinkMeshManager::SetMapSign(MVec3 pos , unsigned char sign )
{
	int x ,y ;
...
const char* LinkMeshManager::GetMapSingFileName(int x,int y , bool btest , bool *bUpdate)
{
	if ( btest )
	{
		...
		sign_ex[0] = m_LinkMap[x+1+(y+1)*m_nMapSize];
		sign_ex[1] = m_LinkMap[x-1+(y+1)*m_nMapSize];
		sign_ex[2] = m_LinkMap[x-1+(y-1)*m_nMapSize];
		sign_ex[3] = m_LinkMap[x+1+(y-1)*m_nMapSize];
		...
		{
			{
				...
				sign[SIGN_UP] = m_LinkMap[i+(j+1)*m_nMapSize] ;
				sign[SIGN_LEFT] = m_LinkMap[i-1+j*m_nMapSize] ;
				sign[SIGN_CENTER] = m_LinkMap[i+j*m_nMapSize] ;
				sign[SIGN_RIGHT] = m_LinkMap[i+1+j*m_nMapSize] ;
				sign[SIGN_DOWN] = m_LinkMap[i+(j-1)*m_nMapSize] ;
...
{	
	...
	for ( int j = y ; j < y + height ; j ++ )
	{
		for ( int i = x ; i < x + width ; i ++ )
		{
			...
			curData.saveData[index].sign = m_LinkMap[i+j*m_nMapSize] ;
...
bool  LinkTextureManager::UpdateMesh(int x,int y,const char* filename ,bool *bUpdate)
{
	...
	if( !strname.empty() )
	{
		...
		if( pMayModel )
		{
			LMTex ptex = g_TexManager.LoadTex( g_TerrainEdit.GetEditData()->texData.texFileSet[m_nCurTextureID].c_str());
...
void   LinkOverlayManager::SetMapSign(MVec3 pos , unsigned char sign )
{
	int x ,y ;
