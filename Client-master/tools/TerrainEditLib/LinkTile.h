...
#define MAP_RD_LU        MAP_LU_SIGN | MAP_RD_SIGN 
	...
	public:
		...
		void  Init(int unitsize, Ogre::Point2D nblock, Ogre::Point2D ngrid);//unitsize=1,2一个拼接单位对应几格地图， 地图有nblock块, 每个block有ngrid格
		void    Release();

		void  SetMapSign( int x,int y, unsigned char sign );
		virtual  void  SetMapSign(Ogre::Vector3 pos , unsigned char sign );
	...
	protected:
		...
		unsigned char getLinkMapUnit(int x, int y);
		...
		Ogre::Point2D m_nBlock;
		Ogre::Point2D m_nGrid;
		int m_UnitSize;
	...
	public:
		void                                           SetBeginLoadIDPos( Ogre::Vector3  pos );
		//当前选择坐标( X,Y )
