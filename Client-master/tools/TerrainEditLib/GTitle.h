...
public:
	...
	void Init();
    //设置模型 
	bool SetShowModel( MShowModel  * pModel );
    //重置矩阵
	void ResetMat();
	//重置BOX
	void ResetBox();
	...
	//测试是否与射线相交
	bool  TestIntersect( MVec3 pos , MVec3 dir );
	//更新当前位置
	void  UpdatePos();
	//计算模型顶点颜色
...
struct MTileData
{
	...
	bool  bShowFog; //是否显示雾
	MChunk                      chunkSet[TERRTILE_NUMBLOCK][TERRTILE_NUMBLOCK];//地形块数据
};
...
public:
	...
	bool    SaveMayFile(const char* name);
    //引导ADT文件；参数（ 文件名）；返回（ 为真表示成功）
	bool    LoadMayFileEX(const char* name);
    ...
    //测试与TILE相交
	bool    InsterectRay(MVec3 dir , MVec3 pos , MVec3 &reslut );
    //设置环绕次数；参数（ 次数 )
...
private:
	//TILE的数据
	MTileData              m_TitleData;
	//TILE的头信息
	MTileHead               m_TileHead;
