...
MAutoLinkMeshProcess::MAutoLinkMeshProcess(void)
{
...
//后退操作
void MAutoLinkMeshProcess::Undo( MTerrainEdit* pEditor )
{
	pEditor->GetAutoLinkMesh()->UnDo();
}    
//处理过程; 参数( 编辑器指针, 响应事件 ) ; 返回 ( 为真表示成功 )
bool MAutoLinkMeshProcess::Process( MTerrainEdit* pEditor , ActionEvent *event )
{	
...
void  MAutoLinkMeshProcess::SetAutoLinkMeshSign(  MTerrainEdit* pEditor ,MVec3 &dir ,MVec3 &pos)
{
 	MVec3  pos0;
