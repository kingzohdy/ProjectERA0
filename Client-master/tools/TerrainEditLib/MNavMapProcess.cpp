...
MNavMapProcess   g_NavMapProcess;

MNavMapProcess::MNavMapProcess(void)
{
}
...
void  MNavMapProcess::calNaviMapData( MTerrainEdit* pEditor, int nmask_block, float height)
{
	...
	float basex,basez;
	MTitle* pTitle = (MTitle*)g_Terrain.GetTitle (0);
	...
	for( int i = 0 ; i < pEditor->GetEditData()->gh ; i++ )
	{
		for( int j = 0 ; j < pEditor->GetEditData()->gw ;j++ )
		{
			...
			{
				MVec3 pos = pEditor->GetEditData()->chunkSet[i][j].GetChunkData()->vecData[0].pos  ;
...
unsigned char MNavMapProcess::GetMapSign(float x,float z,float sx,float sz,float height)
{
    float _height[4];
...
void  MNavMapProcess::SetModelSign( MovableObject *pNode,float height , float basex,float basez, unsigned char *pdestdata, int ngridx, int ngridz, int gridsizex, int gridsizez)
{
	int  minx,minz;
	int  maxx,maxz;
...
bool MNavMapProcess::isPointWalkable(MTerrainEdit* pEditor, float x, float z)
{
	if(!pEditor->GetEditData())
		return false;
...
bool MNavMapProcess::saveNaviDataToMaskMap(MNavigationMap &destmap, MTerrainEdit* pEditor, int nmask_block)
{
	if(!pEditor->GetEditData())
		return false;
	...
	MNavHead  *pHead = destmap.GetHeadData();
	...
	pHead->ngridz = gz;
	pHead->version = 100;
	pHead->headsize = sizeof(MNavHead);
	pHead->filesize = pHead->headsize + gx*gz;

	MNavData *pData = destmap.GetNavData();
