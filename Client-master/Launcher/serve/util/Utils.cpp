...
unsigned int string_id(const char *source)
{
    ...
    return hash((unsigned char *)(str), (unsigned int)strlen(str), 0 );
}
...
const char *string_adjust(const char *source)
{
    ...
    strncpy_s(buf, sizeof(buf), source, strlen( source ) );
...
const char *strip_path(const char *full_path)
{
    ...
    static char buf[1024];
    strncpy_s(buf, sizeof(buf), full_path, strlen( full_path ));
...
void ShowError( const WCHAR* pszMsg, HWND hWnd )
{
 	//::MessageBoxW( hWnd, pszMsg, L"零纪元", MB_OK | MB_ICONSTOP );
...
void GetFilePath( const char* szFileName, char* szPath, int nDestLen )
{
	...
	_snprintf_s( szPath, nDestLen,nDestLen, "%s", szFileName );
}
...
void DrawFont( HDC hDC, COLORREF crColor, LPCTSTR strCaption, RECT& rect, int nFontWidth, bool bCenter /*= false*/ )
{
	...
	/*DrawText( hDC, "", 0, &rect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS );
	DrawText( hDC, strCaption, lstrlen( strCaption ), &rect, DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS );*/
	
	static wchar_t s_UnicodeBuffer[MAX_CONVERT];
	AnsiToUnicode( s_UnicodeBuffer, MAX_CONVERT, strCaption );
	DWORD style = 0;
	if( bCenter )
	{
		style = DT_CENTER | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS;
	}else{
		style = DT_LEFT | DT_VCENTER | DT_SINGLELINE | DT_END_ELLIPSIS;
	}
	DrawTextW( hDC, L"", 0, &rect, style );
	DrawTextW( hDC, s_UnicodeBuffer, wcslen( s_UnicodeBuffer ), &rect, style );
...
{
	int nchar;
	//static const int nCharType = CP_ACP;
	static const int nCharType = 936;
#ifdef _DEBUG
	nchar = MultiByteToWideChar( nCharType, MB_PRECOMPOSED, psrc, nsrcchar, pdestbuf, 0 );
